...
- **サードパーティ action は必ずコミット SHA で固定する**: `@main` / `@vN` 等の可動参照は生成しない。上流のタグ付け替え・ブランチ改変が未検証のまま流れ込むサプライチェーンリスクを避けるため（SHA 更新時は差分を確認してから更新する）。**`Fandhe-AI/actions/*`（内製 action）は `@latest` 参照とする**（組織内で管理・レビューされるため。オーナー決定）
