...
#
# 【読み取り専用 Agent の既定】（research/ または quality/ カテゴリ）
# → Edit / Write / Bash を含めない。ネットワーク系（WebFetch / WebSearch）も既定では含めない
tools:
  - Glob
  - Grep
  - Read

# 【外部ドキュメント調査を担う Agent の追加例】（research/ カテゴリ限定）
# 公式ドキュメント・外部仕様の取得が責務に含まれる場合のみ、以下を追加する。
# quality/（レビュー・監査）には付与しない。
# tools:
#   - Glob
#   - Grep
#   - Read
