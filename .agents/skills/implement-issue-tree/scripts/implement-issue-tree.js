...
//       enforce_admins 有効。PR #222 codex P0 第 2 ラウンド対応: bypass 可能な構成では admin
...
//     有効」まで確認する。これによりマージ可否の実強制は GitHub の branch protection になり、
  ...
  return [
    ...
    allowMerge
      ? [
          ...
          `   (ii) (i) が 0 件またはエラーの場合のみ classic branch protection を確認する: gh api "repos/{owner}/{repo}/branches/${encodeURIComponent(baseBranch)}/protection/required_status_checks" --jq '.checks | length' が 1 以上、かつ gh api "repos/{owner}/{repo}/branches/${encodeURIComponent(baseBranch)}/protection/enforce_admins" --jq '.enabled' が true の両方を満たす場合のみ次へ進む（enforce_admins が false だと管理者権限のこの gh 認証が required checks を迂回してマージできるため辞退する）。`,
          ...
          ...(apps.length
            ...
            : []),
          `   G0 を通過できない場合（(i-b) の bypass 検証不合格・(ii) の不合格・(iii) のスレッド解消強制なし・(iv) の外部チェック required 化なし・取得不能を含む）はマージせず merged: false / reason: server-enforcement-missing を返す（summary に「ベースブランチ ${JSON.stringify(baseBranch)} のサーバー側強制を確認できない」と、どの判定（存在 / ruleset bypass / org ruleset / enforce_admins / スレッド解消強制 / 外部チェック required 化）で不合格になったかを書く。エラー出力の本文は転記しない）。本手順に記載したコマンド以外の branch protection API は実行しない。`,
        ].join('\n')
