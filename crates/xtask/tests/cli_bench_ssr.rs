//! `xtask bench-ssr` の CLI 契約に対する回帰テスト（イシュー #1317）。
//!
//! 本サブコマンドは実行環境依存の計測値を出力する report-only ハーネスであり、
//! 性能数値そのもの（`mean_ms`/`p50_ms`/`p95_ms`/`min_ms`）は環境依存のため
//! 検証しない（`bench_binding_update` の既存 CLI 契約テストと同じ判断軸）。
//! 本テストは**形式・契約**のみを検証する:
//! - 終了コード 0: 検証（`escape_ok`/`row_count_ok`）PASS
//! - 終了コード 1: 検証 FAIL・環境エラー（`--baseline` 読み取り/パース失敗を含む）
//! - 終了コード 2: 引数不備
//! - stdout に JSON 1 行（`framework`/`version`/`mode`/`rows1k`/`rows10k`/
//!   `html_bytes_1k`/`escape_ok`/`row_count_ok`/`notes` を含む）
//! - `--baseline <FILE>` 指定時は `bench-ssr-compare:` 行群が追加出力される
//!
...
fn run_bench_ssr(extra_args: &[&str]) -> Output {
...
fn unique_fixture_path(label: &str) -> PathBuf {
    ...
    scratch_root().join(format!(
        "xtask-bench-ssr-{label}-{}-{nanos}-{n}.json",
        std::process::id()
    ))
}

#[test]
fn bench_ssr_exits_zero_and_reports_expected_json_shape() {
    let output = run_bench_ssr(&[]);
    let stdout = String::from_utf8_lossy(&output.stdout);

    assert_eq!(
        output.status.code(),
        Some(0),
        "自己検証（escape_ok/row_count_ok）は常に PASS のはず。stdout: {stdout}\nstderr: {}",
...
#[test]
fn bench_ssr_with_unknown_argument_exits_two() {
    let output = run_bench_ssr(&["--rows", "5"]);
    assert_eq!(
        output.status.code(),
        Some(2),
...
#[test]
fn bench_ssr_with_baseline_missing_value_exits_two() {
    let output = run_bench_ssr(&["--baseline"]);
    assert_eq!(
        output.status.code(),
        Some(2),
...
#[test]
fn bench_ssr_with_invalid_baseline_json_exits_one() {
    ...
    let output = run_bench_ssr(&["--baseline", &baseline_arg]);
...
#[test]
fn bench_ssr_with_baseline_missing_file_exits_one() {
...
#[test]
fn bench_ssr_with_valid_baseline_emits_compare_lines_and_exits_zero() {
    // 1 回目の実行結果を baseline として保存する。
    let first = run_bench_ssr(&[]);
    ...
    let baseline_arg = baseline_path.to_string_lossy().into_owned();
    let second = run_bench_ssr(&["--baseline", &baseline_arg]);
    ...
    let compare_lines: Vec<&str> = second_stdout
        .lines()
        .filter(|l| l.starts_with("bench-ssr-compare: "))
        .collect();
    assert_eq!(
        compare_lines.len(),
        9,
        "9 指標分の比較行が出力されるはず: {second_stdout}"
    );
