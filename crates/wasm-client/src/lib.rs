//! `fandhe-frontend-wasm-client`: 最小ハイドレーション方式のクライアントランタイム（TASK-6.2b）。
//!
...
//!
//! # 2 層構成（`fandhe-frontend-wasm-full` の先例を踏襲）
//!
//! 1. **純粋ロジック層**（本ファイル直下、ネイティブテスト可能）:
//!    `fandhe_frontend_app::Loader`（#347・イシュー #346 設計確定書
//!    `docs/design/loader-trait-design.md`）経由でデータを解決し
//!    `fandhe_frontend_core::render` を呼ぶ CSR 用 HTML 生成（[`render_list_page_html`] /
//!    [`render_detail_page_html`])、および `fandhe_frontend_core::find_attr_values` /
//!    `fandhe_frontend_core::find_nav_targets` を用いたハイドレーション対象の特定
//!    （[`find_hydrate_target_kinds`] / [`find_list_nav_targets`]）。実 DOM 型
//!    （`web-sys` 各型）には一切依存しないため、wasm ビルドを介さず
//!    `cargo test -p fandhe-frontend-wasm-client` で検証できる
//!    （`wasm-client/tests/hydration_targets.rs`）。イシュー #375 で
//!    `fandhe_frontend_app::demo_items()` の直接呼び出しを排し、`fandhe-frontend-server`（#348）・
//!    `fandhe-frontend-wasm-full`（#349）と同一の Loader 契約へ統一した
...
//!
//! 1. DOM への HTML 挿入は [`fandhe_frontend_core::render`] の出力（既定エスケープ済み）
...
/// keyed list（`fandhe_frontend_core::keyed`、#344）の DOM 適用: 純粋 diff 層（イシュー
...
pub use keyed_dom::{
    ...
    find_keyed_list_node, find_list_element, KeyedListApplyResult,
};
...
/// ハイドレーション対象を示す属性名（`fandhe_frontend_app::detail_page` が「いいね」
...
/// （[`fandhe_frontend_core`]）のみで組み立て、`format!` によるタグ文字列の直接組み立て
...
pub fn loader_error_view() -> Node {
    fandhe_frontend_core::div(
        vec![("data-fandhe-frontend", "csr-error")],
        vec![fandhe_frontend_core::p(
            vec![],
...
/// 一覧画面向け CSR loader 解決（イシュー #375）。`fandhe-frontend-wasm-full` は
...
/// 詳細画面向け CSR loader 解決（イシュー #375）。`fandhe-frontend-wasm-full` は
/// `wasm-full/src/csr.rs` から本関数を再エクスポートして共有する
/// （Bugbot 指摘対応、重複コピーの解消）。
///
...
/// [`resolve_list_node`] 経由で解決し `fandhe_frontend_core::render` を呼ぶ。
/// SSR/SSG（`fandhe-frontend-server`、TASK-6.1c）が同一の `fandhe_frontend_app::Loader` 契約
...
///
/// ```
/// use fandhe_frontend_wasm_client::render_list_page_html;
/// use fandhe_frontend_app::{list_page, demo_items};
...
pub fn render_list_page_html() -> String {
    fandhe_frontend_core::render(&resolve_list_node(&DemoItemsLoader))
}

/// CSR 用の詳細ページ HTML を生成する純粋関数。[`render_list_page_html`] と
/// 同様、[`DemoItemDetailLoader`] を [`resolve_detail_node`] 経由で解決し
/// `fandhe_frontend_core::render` を呼ぶ。該当 `id` が存在しない場合は
/// `fandhe_frontend_app::detail_page` の 404 相当ノードを返す（呼び出し元での
/// `panic!` を避ける、`.claude/rules/coding-rust.md`）。
///
/// # Examples
///
/// ```
/// use fandhe_frontend_wasm_client::render_detail_page_html;
///
/// let html = render_detail_page_html("1");
/// assert!(html.contains("Rust 製フロントエンド基盤の構想"));
/// ```
pub fn render_detail_page_html(id: &str) -> String {
    fandhe_frontend_core::render(&resolve_detail_node(&DemoItemDetailLoader, id))
}

/// 指定 `id` の詳細ページのノード木から、[`HYDRATE_ATTR`] を持つ要素の値を
/// [`fandhe_frontend_core::find_attr_values`] で列挙する。
///
...
///
/// ```
/// use fandhe_frontend_wasm_client::{find_hydrate_target_kinds, LIKE_HYDRATE_VALUE};
///
...
pub fn find_hydrate_target_kinds(id: &str) -> Vec<String> {
    ...
    fandhe_frontend_core::find_attr_values(&tree, HYDRATE_ATTR)
}

/// 一覧ページのノード木から `data-nav` 属性値を [`fandhe_frontend_core::find_nav_targets`]
/// で列挙する。`wasm-client` 側のクライアントルーティング配線はイシュー #405
/// で非採用確定（`docs/policy/intentional-non-adoption.md` §3.19）。配線は
/// `fandhe-frontend-wasm-full` の `nav`（`wasm-full/src/nav.rs`）のみが実装し、本関数は
/// 将来の再評価トリガー充足時に備えた対象特定契約として維持する。
///
/// # Examples
///
/// ```
/// use fandhe_frontend_wasm_client::find_list_nav_targets;
///
...
pub fn find_list_nav_targets() -> Vec<String> {
    ...
    fandhe_frontend_core::find_nav_targets(&tree)
}

// ---------------------------------------------------------------------
// wasm32 配線本体: 実 DOM（web-sys）を操作するのはこのモジュールに限定する。
// native の `cargo test -p fandhe-frontend-wasm-client` にはコンパイル対象外
...
mod hydrate_dom {
    ...
    /// `fandhe-frontend-wasm-full`（`wasm-full/src/nav.rs::render_route`、イシュー
...
// ---------------------------------------------------------------------
// イシュー #345: `fandhe-frontend-wasm-full` が本クレートを rlib として依存し
