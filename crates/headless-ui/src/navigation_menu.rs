...
//!
//! Radix Primitives `Navigation Menu`
//!（`docs/design/component-coverage-map.md` §5 Part D / §9）を参照し、
//! Root / List / Item / Trigger / Content / Link の 6 anatomy パーツと、
//! [`crate::state::SingleSelect`] を埋め込んだ「高々 1 項目が開く」状態機械
//! [`NavigationMenu`] を提供する。
//!
//! # `nav_list` との使い分け
//!
...
//! [`content`]/[`link`]、いずれも純粋関数で完結）を直接呼んで組み立てる。
...
use crate::aria::{
    aria_controls, aria_current, aria_expanded, aria_label, aria_labelledby, AriaCurrent,
};
use crate::data_attrs::{data_current, data_disabled, data_state};
...
/// [`item`]/[`trigger`]/[`content`] へ注入する利便メソッドを提供する
/// （[`root`]/[`list`]/[`link`] は状態非依存またはリンク単位のため利便
/// メソッドを持たない）。SSR での自由関数直接利用も引き続き可能。
...
impl NavigationMenu {
    ...
    pub fn content<'a>(
        ...
        content(self.item_state(value), id, labelled_by, attrs, children)
    }
}
...
mod tests {
    ...
    fn list_and_item_output_expected_tags_without_role() {
        let list_html = render(&list(
            vec![],
            vec![item(OpenState::Closed, false, vec![], vec![])],
        ));
    ...
    fn item_disabled_true_adds_data_disabled() {
        let html = render(&item(OpenState::Closed, true, vec![], vec![]));
    ...
    // --- root > list > item > trigger + content(link) の組み立てテスト ---
    ...
    fn navigation_menu_convenience_methods_reflect_state() {
        ...
        let content_products = render(&m.content("products", None, None, vec![], vec![]));
        assert!(!content_products.contains("hidden"));

        let content_solutions = render(&m.content("solutions", None, None, vec![], vec![]));
    ...
    // --- XSS 回帰: label/id/controls/labelled_by/href/呼び出し側 attrs/children ---
