...
//! 11. Menubar（イシュー #1073）: closed 時のトリガー間水平/垂直移動 +
//!     roving tabindex 更新・disabled スキップ、closed 時の ArrowDown/Enter
//!     で open + 初期 highlight、open 時の highlight 移動、open-follows-focus
//!     （open 中の未消費 ArrowRight/ArrowLeft でトリガー間移動 + 新 Menu が
//!     開き旧 Menu が閉じる）、`aria-controls` 欠落時の `content_owner`
