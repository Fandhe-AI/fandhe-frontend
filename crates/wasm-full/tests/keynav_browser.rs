...
/// イシュー #1605 codex-review P1 是正の回帰: `data-readonly`（`ComboboxProps::readonly`
/// が root へ付与する属性）が付いた combobox では、closed の input 上で
/// ArrowDown を押しても listbox を開かない（`handle_combobox_input_keydown`
/// の [`has_readonly_ancestor`](wiring::has_readonly_ancestor) 判定、
/// モジュール doc §Combobox「`data-readonly`（祖先方向）」節参照）。open/
/// Enter/Escape/Home/End のいずれも claim しないため、`prevent_default`
/// されず（キャレット移動等ブラウザの既定動作を奪わない）、`hidden`
/// 状態も変化しない。
#[wasm_bindgen_test]
