...
//!    配線しないだけで正しい静止表示になる）
//!
//! # 参考: `fandhe-frontend-animation::text_animation` との再生時間契約
//!
//! `play_typewriter`/`play_scramble` は呼び出し時点から常に `elapsed_s =
//! 0.0` で再生する（ページのナビゲーション経過時間に依存しない、
//! codex-review P1・cursor Bugbot 是正: ナビゲーション経過時間を初期値に
//! 使うと、duration 超過後の遅延マウント/ハイドレーションで最初のフレーム
//! から完了扱いになり一切再生されない、または duration 未満でも SSR の
//! 全文表示を途中までの文字列へ即座に巻き戻してしまう問題があった）。
//! この結果、本モジュールが配線を呼ぶ時点で `.fd-text-reveal__display`
//! が既に SSR の全文を表示済みでも、常に空文字から改めて再生される
//! （遅延ハイドレーション時に一瞬全文が消えて打ち直されるちらつきが
//! 起きる既知の簡略化——upgrade path: 「SSR からどれだけ時間が経過したか」
//! を著者側マークアップへタイムスタンプとして埋め込む等の設計が要るが、
//! 依存追加なしで実装できる範囲では duration 超過時の無反応・巻き戻りと
//! いう P1 バグの方が実害が大きいため、本 PR ではこのちらつきの解消は
//! 見送る）。
//!
