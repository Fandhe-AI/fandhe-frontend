...
//!
//! ラップ本体（`with_view_transition`）はイシュー #2400 で
//! [`crate::view_transition`] へ切り出し、`Runtime::apply_with_view_transition`
//! （任意の状態更新向け新規公開 API、feature `view-transitions`）と共有する。
//! 本モジュールの呼び出しは feature に関わらず無条件のまま（新規公開 API と
//! 既存配線の責務分離、イシュー #2400 受け入れ条件）。
...
mod wiring {
    ...
    /// `document.startViewTransition()` ラップ本体。イシュー #2400 で
    /// [`crate::view_transition`] へ切り出し、`Runtime::apply_with_view_transition`
    /// （任意の状態更新向け新規公開 API、feature `view-transitions`）と共有
    /// する。本モジュールの呼び出しは feature に関わらず無条件のまま
    /// （router 経由の View Transitions は新規公開 API と責務分離されて
    /// おり、feature 無効でも動作し続ける）。
        ...
        P: FnOnce() + 'static,
    {
        ...
        // `_is_real_vt`（イシュー #2578）: router 遷移は実 VT の有無を
        // 区別しないため未使用（`with_view_transition` doc 参照）。
        with_view_transition(document, None, move |_is_real_vt: bool| {
