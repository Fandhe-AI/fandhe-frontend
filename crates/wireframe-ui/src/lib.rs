...
//! `props::Orientation` の最初の消費者）が続き、Phase 2 の [`link`]
//! （イシュー #2618、`Option<Node>` アイコンスロット規約 §11.4 の実例、
//! `a[href]` 非出力）が続いた。残りは Phase 1 の他部品（frame/stack）・
//! Phase 2 の他部品（text/paragraph/rich-text/tag 等）および Phase 3 以降
//! （#2608〜）で順次追加する。
//!
//! # class 命名規約
//!
