...
//! 外とする）が続き、2 番目の部品 [`media`]（イシュー #2661、blocks.pm
//! 上の表示名は Placeholder。`content: Option<Node>` が `None` のとき
//! [`icon::play`] へフォールバックする §11.4 からの意図的な逸脱。動画か
//! 静止画かは bool ではなくスロット差し替えで表し、枠は 16:9 固定で
//! `<video>`/`<iframe>` は出力しない）が続いた。残りは Phase 7 の他部品・
//! Phase 8 の他部品で順次追加する。
//!
//! # class 命名規約
//!
