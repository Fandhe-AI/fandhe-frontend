//! `avatar::AVATAR_CSS` の全文をバイト一致で固定する golden。
//!
//! 生成元 CSS 定数を手で書き写すと差分検知の意味がなくなるため、実装時に
//! `avatar::AVATAR_CSS` の出力をそのままダンプして作成した（機械的な複製）。
//! 更新手順は `docs/internal/wireframe-ui-golden-test-update-guide.md` を参照。

const EXPECTED_CSS: &str = r#".fw-wire-avatar {
  display: inline-flex;
  align-items: center;
...
#[test]
fn avatar_css_matches_golden() {
    assert_eq!(
        fandhe_frontend_wireframe_ui::avatar::AVATAR_CSS,
        EXPECTED_CSS
    );
}
