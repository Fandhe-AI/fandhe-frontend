...
const TAGS_INPUT: ComponentPageSpec = ComponentPageSpec {
    ...
    arguments: &[
        ...
        ArgRow { name: "item_input(item, value)", kind: "&TagItem, &str", default: "", description: "編集中の暫定値。非編集時は `hidden` + `tabindex=\"-1\"` を出力する。" },
        ArgRow { name: "item_delete_trigger(item)", kind: "&TagItem", default: "", description: "`aria-label` を `\"Delete tag {value}\"`（zag 既定訳）として動的生成する。`tabindex=\"-1\"` を常に付与する。" },
