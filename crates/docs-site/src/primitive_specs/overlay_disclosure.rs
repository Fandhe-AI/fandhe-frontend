...
//! 個別分岐して固定する）。
//!
//! **base 再取り込み（イシュー #1636 マージ, 2026-09-06）**: 上記 toast
//! （#1643、7 例目）と accordion（#1636）は別ブランチで独立に追加され、本
//! マージで統合された。accordion は 8 例目の例外に当たり、`ACCORDION.keyboard`
//! は「のみ非空」ではなく collapsible/dialog/popover/hover-card/toggle-tip/
//! tooltip/toast と並ぶ 8 件目である。現状 `keyboard: &[]` のまま残るのは
//! drawer/floating-panel の 2 部品のみであり、tooltip 節の「現状は accordion/
//! drawer/floating-panel の 3 部品」・toast 節の「他 3 部品〔accordion/drawer/
//! floating-panel〕」・上記 accordion 節の「他 9 件」はいずれもマージ前の
//! 状態を指す（`specs_have_non_empty_sections_except_demo` が 8 件の分岐で
//! 固定する）。
//!
...
mod tests {
    ...
    fn specs_have_non_empty_sections_except_demo() {
        for (path, spec) in SPECS {
            ...
            // accordion（イシュー #1636）・collapsible（イシュー #1637）・
            // dialog（イシュー #1638）・popover（イシュー #1642）・
            // hover-card（イシュー #1641）・toggle-tip（イシュー #1644）・
            // tooltip（イシュー #1645）・toast（イシュー #1643）のみ例外:
            // accordion は fandhe-frontend-wasm-full の keynav.rs が実際に
            // キーボードイベントを処理するため、collapsible は trigger が
            ...
            // 例外を追加した」参照）。他 2 部品〔drawer/floating-panel〕は
                ...
                || *path == "/primitives/toast/"
            {
                assert!(
                    ...
                    "{path}: keyboard should hold the documented rows (#1636/#1637/#1638/#1641/#1642/#1643/#1644/#1645)"
                );
            } else {
                assert!(
                    spec.keyboard.is_empty(),
