...
/// 積んで 1.25 MiB（`1_048_576 + 262_144`）へ引き上げた。
/// その後本 PR（イシュー #2637 Menu）の base 取り込みでさらに Wireframes
/// セクションへ 1 部品ページが加わり実測が再び上限へ接近したため、
/// 個別ページの索引テキストを切り詰める対処は索引精度を恒久的に損なうため
/// 取らず（設計文書 §10 の既定方針を踏襲）、1.5 MiB（`1_572_864`）へ
/// 引き上げる（設計文書 §3-4 追記節参照）。
pub const MAX_INDEX_BYTES: usize = 1_572_864;
