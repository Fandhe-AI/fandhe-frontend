...
/// その後 Wireframes セクションの部品追加が続き、イシュー #2637（Menu）と
/// イシュー #2644（Tooltip）の 2 部品が同時に main へ合流したことで実測が
/// 再び上限（1,179,648 バイト）を超過した（超過分 451 バイト）。
/// 個別ページの索引テキストを切り詰める対処は索引精度を恒久的に損なうため
/// 取らず（設計文書 §10 の既定方針を踏襲）、1.5 MiB（`1_572_864`）へ
/// 引き上げる。引き上げ後の実測（約 1,180,099 バイト）は新上限の約 75% で
/// あり、§10 の再評価トリガー（新上限の 80% 超過）に対して余裕を持たせた
/// （設計文書 §3-4 追記節参照）。
