...
/// イシュー #2851（`footer-link-columns` block 追加）を base 取り込みで
/// 合流させた時点では実測はこの 4032 の範囲内に収まっていた。
///
/// イシュー #2856（`header-flyout-menu` block 追加、PR #3276）の CI で
/// `SearchIndex(TooLarge { bytes: 1705161, limit: 1703936 })`（超過量
/// 1,225 バイト）を再検知したため、本定数を 4032 → 4000（−32 バイト）へ
/// 再度引き下げて対処した（設計文書 §10-12）。恒久対処ではない緊急避難で
/// あることは変わらず、恒久対処はセクション粒度インデックスへの分割
