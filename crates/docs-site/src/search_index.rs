...
/// セクション粒度インデックスへの分割（追跡: イシュー #3173）であり、本
/// 定数の再引き下げの繰り返しでは対処しない。イシュー #2851
/// （`footer-link-columns` block 追加）を base 取り込みで合流させた時点
/// でも実測はこの 4032 の範囲内に収まっており、追加の引き下げは不要
/// だった。イシュー #2862（`pricing-comparison-table` block 追加）で
/// 再度 `MAX_INDEX_BYTES` 超過（5,632 バイト）を検知し、同じ緊急避難で
/// 4032 → 3968（−64）へ再引き下げた（設計文書 §10-12）。引き下げ後の
/// 索引全体は約 99.3% に達しており、次回の Blocks 追加でも再発しうる。
