...
///
/// #957 設計時点の 1 MiB から 3 回引き上げて現在値（1.375 MiB）に至る
/// （#2552 → 1.125 MiB、#2645 → 1.25 MiB、#2750 → 1.375 MiB）。3 回とも
/// 「引き上げ直後から §8 トリガー 1（新上限の 80% 超過）に抵触する」
/// 結果に終わっており、**これ以上の引き上げは行わない**（ハードルール、
/// 設計文書 §10-4）。次回 [`SearchIndexError::TooLarge`] が発生した場合の
/// 恒久対処はセクション粒度インデックスへの分割であり、本定数の値を
/// 変更することでは対処しない。判断の実測根拠・却下した代替案（per-page
/// 上限引き下げ・索引対象精査）は設計文書 §10-4 を正とし、変更履歴の
/// 逐次追記はここでは行わない。
