...
/// の範囲内であり追加の引き上げは不要だった。本 PR（イシュー #2637 Menu）の
/// base 取り込みでも実測（下記コミット時点の `cargo test -p
/// fandhe-frontend-docs-site` 実測値）は 1,310,720 バイトの範囲内であり、
/// 追加の引き上げは不要と判断した（再評価トリガー未到達、設計文書 §3-4
/// 追記節参照）。
