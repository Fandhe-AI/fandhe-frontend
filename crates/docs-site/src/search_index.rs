...
///
/// イシュー #2858（`header-mega-menu`）と #2855（`header-flyout-menu`）が
/// 並行して Marketing / Header カテゴリへ新規ページを追加し、両者を
/// `main` 上で合流させた時点で再び `TooLarge`（超過 1,052 バイト）を
/// 検知したため、本定数を 4032 → 3968 へ再度引き下げて対処した
/// （設計文書 §10-11）。#2849 の教訓（「本定数の再引き下げの繰り返しでは
/// 対処しない」）を踏まえ、今回は #2849 実施時と同じ引き下げ幅（−64）を
/// 適用し必要量（1,052 バイト）の約 8 倍の安全余裕（索引 JSON 全体で
/// 1,687,354 バイト、`MAX_INDEX_BYTES` の約 99.0%）を確保した。ただし
/// 依然として恒久対処ではない緊急避難であり、次回 `TooLarge` が発生した
/// 場合に本定数を三度引き下げることは推奨しない。恒久対処はセクション
/// 粒度インデックスへの分割（追跡: イシュー #3173）であり、優先度を
/// 上げて着手することが望ましい。イシュー #2851（`footer-link-columns`
/// block 追加）は本引き下げ前の 4032 の範囲内に収まっており、本定数を
/// 対象にした追加の変更は不要だった（設計文書 §10-12）。
pub const MAX_PAGE_TEXT_BYTES: usize = 3968;
