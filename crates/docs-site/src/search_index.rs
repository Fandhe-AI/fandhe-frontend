...
/// ただし恒久対処ではない緊急避難であり、余裕（引き下げ後で全体の約 99.1%）
/// は薄いままである。恒久対処はセクション粒度インデックスへの分割
/// （追跡: イシュー #3173）であり、本定数の再引き下げの繰り返しでは
/// 対処しない。イシュー #2851（`footer-link-columns` block 追加）を
/// base 取り込みで合流させた時点でも実測はこの 4032 の範囲内に収まって
/// おり、追加の引き下げは不要だった。
///
/// イシュー #2852（`footer-newsletter-band` block 追加）を base 取り込みで
/// 複数回合流させる過程で `MAX_INDEX_BYTES` 超過（1,707,604 バイト、
/// 1,703,936 バイト上限比 +3,668 バイト）を検知したため、同じ緊急避難として
/// 本定数を再度 4032 → 3968 へ引き下げた。
