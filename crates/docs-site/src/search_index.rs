...
/// （再評価トリガー未到達、設計文書 §3-4 追記節参照）。その後 Blocks
/// セクションの拡充（イシュー #2730 配下、bento-asymmetric-rows 等の
/// base 取り込みが careers-split-accordion（イシュー #2816）PR と合流）
/// により実サイトが 1,313,090 バイトへ達し、この上限（1,310,720 バイト）
/// を超過したため、同じ +131,072 バイト刻みで
/// 1.375 MiB（`1_048_576 + 393_216`）へ引き上げた（設計文書 §10-4 参照）。
///
/// **この値が最終値（ハードルール、イシュー #2816 §10-4 の決定）**。
/// 引き上げ後の実測は新上限の約 91.1% であり §8 トリガー 1（80% 超過）に
/// 既に抵触しているため、次回 `SearchIndexError::TooLarge` が発生しても
/// 本値をこれ以上引き上げてはならない。恒久対応（セクション別インデックス
/// ファイルへの分割）はイシュー #3173 で追跡する。本値が動いていないことは
/// `crates/docs-site/tests/search_index.rs` の
