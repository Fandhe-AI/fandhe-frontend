...
/// その後 Wireframes セクションの並行 PR 2 件（Breadcrumbs／イシュー
/// #2639・Tooltip／イシュー #2644）が同時に main へ取り込まれ、単独では
/// 収まる各変更が合算で 1.125 MiB を約 344 バイト超過した（PR #2702 の
/// base 取り込み時に実測で判明）。個別ページの索引テキストを切り詰める
/// その場しのぎは取らない方針（§10 既定）を踏襲し、1.25 MiB
/// （`1_048_576 + 262_144`）へ再度引き上げる。
