...
/// バイト、全体の約 99.7%。設計文書 §10-13）。イシュー #2858
/// （`header-mega-menu`）自体はレビュー指摘（PR #3273）を受け、本定数の
/// 三度目の引き下げでは対処しない方針とした。`header-mega-menu` はコード
/// フェンスを含む本文が既にこの上限で切り詰め済みのため同ページの
/// プロースを削っても索引全体には無効果であることを実測で確認し、代わりに
/// 同時に合流した `site/blocks/header-floating-pill.md`（切り詰められて
/// おらず実測が上限を下回っていた）から冗長な実装詳細の 1 文を削って
/// 実測を上限内へ戻した。`MAX_PAGE_TEXT_BYTES`・`MAX_INDEX_BYTES` いずれも
/// 変更していない（設計文書 §10-11 参照）。
