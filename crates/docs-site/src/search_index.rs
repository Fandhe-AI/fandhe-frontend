...
/// PR #3276）は上記 4000 へ既に引き下げ済みの状態を base 取り込みで引き継いだ
/// うえで実装し、`MAX_INDEX_BYTES` 超過が生じないことを実測確認した
/// （設計文書 §10-13）。**本定数のこれ以上の引き下げは行わない**（同一
/// ハードルールの機械的な繰り返しは索引精度を際限なく落とすだけで恒久対処
/// にならないため）。恒久対処（セクション粒度インデックスへの分割、追跡:
/// イシュー #3173）が必要になった場合はそちらへ着手する。
