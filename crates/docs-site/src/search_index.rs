...
/// （`1_048_576 + 131_072`）へ引き上げた（設計文書 §3-4 追記節参照）。
/// その後 PR #2705（イシュー #2642 Cursor と #2644 Tooltip の並行 PR を
/// base 取り込みで合流させた際、Wireframes セクションへ 2 部品ページが
/// 同時に加わり実サイトが再度上限を超過）を受け、さらに 128 KiB の余裕を
/// 積んで 1.25 MiB（`1_048_576 + 262_144`）へ引き上げた。その後イシュー
/// #2645（`modal` 部品ページ追加）を base 取り込みで合流させた時点で実
/// サイトが約 1,180,096 バイトに達したが、この上限（1,310,720 バイト）
/// の範囲内であり追加の引き上げは不要だった。イシュー #2637（Menu）の
/// base 取り込みでも実測は 1,310,720 バイトの範囲内であり、追加の引き上げは
/// 不要と判断した。イシュー #2639（Breadcrumbs）の base 取り込みでも
/// 実測は 1,310,720 バイトの範囲内であり、追加の引き上げは不要と判断した。
/// その後 Blocks セクションの継続的な拡充（イシュー #2730 配下、約 300
/// block 規模を見込む並行 PR 群）により、イシュー #2814
/// （`blog-split-header-grid` block 追加）の base 取り込み時点で実サイトが
/// 1,313,108 バイトに達し、1.25 MiB 上限を約 2,388 バイト超過して
/// `SearchIndexError::TooLarge` が発火した。§3-4 追記節 §10-1 の予告
/// （次回超過時は per-page 上限見直し・セクション粒度分割を実際に検討
/// すること）を踏まえて評価した結果、Blocks セクションが block 単位で
/// 独立ページのまま増え続ける構造上、セクション粒度分割は既存 IA
/// （`site/nav.toml`）を横断する大改修になり 1 block ページ追加への
/// 対処として引き続き不釣り合いに大きく、per-page 上限
/// （`MAX_PAGE_TEXT_BYTES`）の引き下げは既存ページの索引品質を一律に
/// 損なうため、今回も `MAX_INDEX_BYTES` の引き上げを採る。ただし
/// 約 300 block 規模の並行拡充が見込まれるため、前回までの固定
/// +131,072 バイト刻みではなく、実測値（1,313,108 バイト）が新上限の
/// 80% 未満に収まるよう +393,216 バイト（1.25 MiB + 0.375 MiB）を積み、
/// 1.625 MiB（`1_048_576 + 655_360` = `1_703_936`）へ引き上げる
/// （引き上げ後の実測は新上限の約 77.1%、設計文書 §3-4 追記節 §10-4
/// 参照）。イシュー #2751（`content-article` block 追加、Marketing /
/// Content カテゴリの最初の block）を本 PR の base 取り込みで合流させた
/// 時点でも実測は 1.625 MiB の範囲内（80% 未満）に収まっており、
/// `MAX_INDEX_BYTES` の追加引き上げは不要だった（設計文書 §3-4 追記節
/// §10-5 参照）。
