...
/// 実際に超過したため 1.125 MiB（`1_048_576 + 131_072`）へ引き上げた
/// （設計文書 §10）。その後も Wireframes セクションの部品ページ追加
/// （Phase 5「Navigation」〜Phase 6「Overlay・Feedback」）が蓄積し、
/// イシュー #2645（`modal` 部品ページ追加、`tooltip`〔#2644〕との並行
/// マージ）で実サイトが 1,180,096 バイトへ達し再び旧上限を超過した。
/// 前回と同じ +131,072 バイト刻みで 1.25 MiB（`1_048_576 + 262_144`）へ
