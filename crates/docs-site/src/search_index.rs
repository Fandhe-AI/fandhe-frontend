...
///
/// イシュー #2851（`footer-link-columns` block 追加）の base 取り込み時点で
/// `MAX_INDEX_BYTES` を実測 2,044 バイト超過したため、4,096 から 4,064 へ
/// 引き下げた（設計文書 §10-10）。`MAX_INDEX_BYTES` 自体の追加引き上げは
/// §10-5・§10-6 のハードルールで禁止されているため対象にできず、本値は
/// per-page 上限に張り付くページ（約 260 件）1 件あたり最大 32 バイトの
/// 削減で全体の超過分を吸収する、恒久対処（セクション粒度インデックス分割、
/// イシュー #3173）までの最小限の暫定調整である。
pub const MAX_PAGE_TEXT_BYTES: usize = 4064;
