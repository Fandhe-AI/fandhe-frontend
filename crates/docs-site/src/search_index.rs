...
/// その後イシュー #2750（`bento-two-column` block ページ追加）で実サイトが
/// 1,313,033 バイトへ達し、1.25 MiB（1,310,720 バイト）を約 2,313 バイト
/// 超過して実際に `SearchIndexError::TooLarge` が発火した。§8 トリガー 1
/// の観点では per-page 上限の引き下げ・セクション粒度分割が本筋だが、
/// 1 block ページ追加への対処として不釣り合いに大きい過去の判断
/// （設計文書 §10・§10-1）を踏襲し、同じ +131,072 バイト刻みで
