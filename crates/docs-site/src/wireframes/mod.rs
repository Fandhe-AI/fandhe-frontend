...
/// （イシュー #2624）・[`input::WIREFRAME`]（イシュー #2622）が続いた。
/// Phase 1・3 以降（#2608〜#2665）の残りの
...
mod tests {
    ...
    fn wireframe_for_path_finds_nothing_in_empty_registry() {
        // `/wireframes/textarea/` は Phase 3「Forms A」の未実装部品（イシュー
        // #2622 時点では button/select/input のみ登録済み）であり、恒久的に
        // 未登録のパスとして使える。
