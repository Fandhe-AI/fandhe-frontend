...
pub const PRIMITIVES: &[PrimitiveEntry] = &[
    ...
    PrimitiveEntry {
        // イシュー #2072 で headless-ui 層を実装。Themes ページと title
        // 一致（イシュー #2075、`site/themes/sidebar.md` / `site/nav.toml`
    ...
    PrimitiveEntry {
        // イシュー #2121 で headless-ui 層を実装。Themes 層（styled
        // recipe・golden・`site/themes/message-scroller.md`）は後続イシュー
        // #2123 のスコープであり、`PRIMITIVES_WITHOUT_THEMES_PAGE` へ
        // `questionnaire` と同様の暫定登録をしている。
...
/// 作らない」の限定用途の 1 つ）。`questionnaire` はイシュー #2119 で
/// Themes 層を実装済みのため除外済みで、現在は `message_scroller` と
/// `data_table` が該当する。`data_table` も同型の暫定登録であり、
/// イシュー #2125 で headless-ui 層のみ先行実装した（Themes 層・
/// styled recipe・golden・`site/themes/data-table.md` は後続イシュー
/// #2127 のスコープ）。
pub const PRIMITIVES_WITHOUT_THEMES_PAGE: &[&str] = &["message_scroller", "data_table"];
