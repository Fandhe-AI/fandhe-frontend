//! `dashboard-01` block（イシュー #2089。shadcn/ui Blocks の `dashboard-01`
//! に相当する合成例で、`crate::blocks` モジュール doc の契約を `login_01`
//! に続いて 2 件目に実装する）。
//!
//! # 使用部品
//!
//! `sidebar`（左ナビゲーション）/ `card` + `stat` + `badge`（統計カード）/
//! `area_chart`（グラデーション面グラフ）/ `toggle_group`（期間切替）/
//! `tabs`（Outline/Past Performance/Key Personnel/Focus Documents）/
//! `table` + `checkbox`（行選択付き data table 合成）/ `avatar` + `menu`
//! （footer ユーザーメニュー）/ `separator` / `link` / `button` の
//! 部品を合成する（`crate::blocks::Block::parts` に一致させる契約）。
//!
//! # data-table（#2124〜#2127）未実装のため `table` + `checkbox` で代替
//!
//! shadcn/ui の `dashboard-01` は TanStack Table 相当の data-table（列表示
//! 切替・ページネーション・行ドラッグ）を使うが、本リポジトリの
//! `data-table` 系部品は本イシュー時点で未実装（#2124〜#2127 が open）。
//! 既存の `table`（構造）+ `checkbox`（選択列）の合成で静的な行選択の
//! 見た目のみを再現し、ページネーション・列表示切替・行ドラッグは持たない
//! （shadcn 側との差分メモに記録する）。
//!
//! # `<form>` を使わない・認証処理を持たない・全データが架空
//!
//! `crate::blocks` モジュール doc の不変条件どおり、本 Demo は `<form>` を
//! 出力しない。ブランド名・ユーザー名・メールアドレス・統計値はすべて
//! 架空のものであり、実企業名・実在人物・実クレデンシャル・PII を含まない。
//! 期間切替（toggle-group）・タブ切替・行選択（checkbox）はいずれも静的な
//! 初期状態を表示するのみで、値の送信・永続化・認証処理は行わない。
//!
//! # CSS フックの選び方（`drop_class_attr` の契約）
//!
//! `sidebar` の全パーツ・`card::root`・`stat::root`・`table::root`・
//! `badge::badge`・`avatar::root`・`checkbox::root`・
//! `toggle_group::root_with_props`・`separator::separator`・
//! `link::root`・`menu::root` は呼び出し側 `attrs` の `class` を
//! `drop_class_attr` により黙って除去する契約を持つ（`crate::blocks::mod`
//! モジュール doc「CSS フック」節参照）。これらへの Demo 固有 CSS フックは
//! `data-blocks-dashboard-01-*` 属性で渡し、[`super::LAYOUT_CSS`]（本
//! モジュールの [`LAYOUT_CSS`]）側も同じ属性セレクタで対応する。一方
//! `card::header`/`body`/`footer`/`title`/`description`/`action`・
//! `stat::label`/`value_text`/`help_text`・`table::header`/`body`/`row`/
...
pub const BLOCK: Block = Block {
    ...
    rust_source: "crates/docs-site/src/blocks/application/dashboard/dashboard_01.rs",
...
/// レイアウト CSS（設計判断: 並列進行する他 block（#2090〜#2095）との
/// `mod.rs::LAYOUT_CSS` 追記衝突を避けるため、本モジュール側に定数を
/// 分離する。`docs/design/docs-site-blocks-section.md` §10 追記参照）。
...
.blocks-dashboard-01-stat-label {\n  font-size: var(--fandhe-font-font-size-sm, 0.875rem);\n  font-weight: var(--fandhe-font-font-weight-normal, 400);\n}\n\
[data-blocks-dashboard-01-chart] {\n  width: 100%;\n}\n\
[data-blocks-dashboard-01-chart] svg {\n  width: 100%;\n  height: auto;\n}\n\
[data-blocks-dashboard-01-table] {\n  width: 100%;\n}\n\
.blocks-dashboard-01-brand {\n  font-weight: var(--fandhe-font-font-weight-medium, 500);\n}\n\
.blocks-dashboard-01-user {\n  display: flex;\n  align-items: center;\n  gap: 0.5rem;\n  padding: 0.5rem;\n}\n\
.blocks-dashboard-01-user-text {\n  display: flex;\n  flex-direction: column;\n  font-size: var(--fandhe-font-font-size-sm, 0.875rem);\n  line-height: 1.2;\n  overflow: hidden;\n}\n\
[data-blocks-dashboard-01-user-trigger] {\n  margin-inline-start: auto;\n}\n";
