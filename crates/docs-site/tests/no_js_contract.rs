...
//!    は `<button>` で `:hover`/`:focus-within` により CSS のみで開閉する
//!    ため「ブロック内の遷移要素がすべて `<a>`」ではない。リンク解決性
//!    自体は `build_site` 内蔵の linkcheck が fail-closed で保証済みであり、
//!    ここでは「JS なしで辿れる形が存在する」ことのみを固定する）
