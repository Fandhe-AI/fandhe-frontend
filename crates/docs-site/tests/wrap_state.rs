...
/// headless 部品（module 名、2 件）。
const HEADLESS_UNWRAPPED: &[&str] = &["collapsible", "fieldset"];
...
fn unwrapped_ledger_is_consistent_with_primitives_without_themes_page() {
    ...
    assert_eq!(
        diff, expected_diff,
        "PRIMITIVES_WITHOUT_THEMES_PAGE ∖ HEADLESS_UNWRAPPED は {{\"field\"}} \
         のみであるはずです（`field` は別名ラップ済みのため \
