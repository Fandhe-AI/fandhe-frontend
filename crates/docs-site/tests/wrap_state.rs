...
/// `motion` と同じ理由で追加した。イシュー #2538 で `button_motion`
...
fn every_pre_styled_module_is_either_a_page_or_declared_non_page() {
    ...
    assert_eq!(
        scan.top_level.len(),
        129,
        "src/*.rs の総数が想定と異なります（イシュー #2540 で \
         marquee_motion.rs を新設し 128 → 129。`motion` feature 配下の \
