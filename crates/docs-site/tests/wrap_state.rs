...
/// ため本リストから除外した。イシュー #2125 で同様に headless-ui 層の
/// みを実装した `data_table` が新設され本リストへ加わった。pre-styled-ui
/// recipe・Themes ページは後続イシュー #2127 のスコープであり、実装され
/// 次第 `WRAPPED_SAME_NAME` へ移す。
