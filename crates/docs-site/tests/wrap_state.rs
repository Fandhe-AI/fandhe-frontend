...
//!
//! - [`WRAPPED_SAME_NAME`]（68）: 同名の Primitives 部品が存在し、かつ同名
...
/// コード委譲している Themes ページ（kebab、ソート済み、70 件。
...
/// #2072 で一時的に headless-ui 層のみを実装した `sidebar` が新設され
...
/// リストから除外した。イシュー #2117 で同様に headless-ui 層のみを実装
/// した `questionnaire` が一時的に本リストへ加わっていたが、イシュー
/// #2119 で pre-styled-ui 側（`crates/pre-styled-ui/src/questionnaire.rs`・
/// `/themes/questionnaire/`）を新設し `WRAPPED_SAME_NAME` へ分類された
/// ため本リストから除外した。イシュー #2121 で同様に headless-ui 層の
/// みを実装した `message_scroller` が新設され本リストへ加わった。
/// pre-styled-ui recipe・Themes ページは後続イシュー #2123 のスコープで
/// あり、実装され次第 `WRAPPED_SAME_NAME` へ移す。イシュー #2125 で
/// 同様に headless-ui 層のみを実装した `data_table` が新設され本リストへ
/// 加わった。pre-styled-ui recipe・Themes ページは後続イシュー #2127 の
/// スコープであり、実装され次第 `WRAPPED_SAME_NAME` へ移す。
const HEADLESS_UNWRAPPED: &[&str] = &["message_scroller", "data_table"];
...
/// §3.5: nav 登録済み Themes ページ 121 件（イシュー #2119 で
/// `questionnaire` 追加、旧 120）すべてが `resolve_page` で panic せず
/// 解決できること。
#[test]
...
fn unwrapped_ledger_is_consistent_with_primitives_without_themes_page() {
    ...
    let expected_diff: BTreeSet<&str> = THEMES_RECIPE_WITHOUT_PAGE.iter().copied().collect();
    assert_eq!(
        diff, expected_diff,
        "PRIMITIVES_WITHOUT_THEMES_PAGE ∖ HEADLESS_UNWRAPPED は \
         THEMES_RECIPE_WITHOUT_PAGE と一致するはずです（`fieldset` は \
         イシュー #1687 で `/themes/fieldset/` ページを登録し \
...
fn every_pre_styled_module_is_either_a_page_or_declared_non_page() {
    ...
    let non_page_top_actual: BTreeSet<String> = scan
        ...
        .cloned()
        .collect();
    let expected_non_page_top: BTreeSet<String> = NON_PAGE_TOP_LEVEL
        .iter()
        .chain(THEMES_RECIPE_WITHOUT_PAGE.iter())
        .map(|s| s.to_string())
        .collect();
    assert_eq!(
    ...
    assert_eq!(
        ...
        "src/*.rs の総数が想定と異なります（イシュー #1684 で field.rs \
         ...
         WRAPPED_SAME_NAME バケットへ移った。イシュー #2063 で \
         input_group.rs を新設し 110 → 111。`/themes/input-group/` \
         ...
         イシュー #2119 で questionnaire.rs を新設し 120 → 121。 \
         `/themes/questionnaire/` ページ登録により `questionnaire` も \
         HEADLESS_UNWRAPPED から WRAPPED_SAME_NAME バケットへ移った）"
    );
    assert_eq!(
        scan.charts.len(),
