...
//! Utilities 17 部品。イシュー #2125 で `data_table`・イシュー #2121 で
//! `message_scroller`・イシュー #2114 で `marker`・イシュー #2111 で
//! `attachment`・イシュー #2108 で
//! `bubble`・イシュー #2105 で
//! `message`・イシュー #2065 で `item` 追加、当初 10）を検証する
//! 統合テスト。
//!
//! 対象は本カテゴリの 17 パスのみ（`tests/component_pages.rs`/
...
/// 本イシューの対象 17 パス（`site/primitives/*.md` のファイル名 = path 末尾）。
