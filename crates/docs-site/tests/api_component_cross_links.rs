...
//! - 部品ページ → API ページ: 全 116 部品ページが
//!   `../../docs/api/pre-styled-ui-api.md` へのリンクを持ち、その集合は
//!   `pre-styled-ui-api.md` 側が指す 116 件と完全一致すること（イシュー
...
fn api_links_to_component_pages_are_all_nav_registered() {
    ...
    assert_eq!(
        nav_kebabs.len(),
        116,
        "expected 116 nav-registered component pages, got {}",
        nav_kebabs.len()
    );
...
fn every_component_page_links_back_to_pre_styled_ui_api() {
    ...
    assert_eq!(
        ...
        "docs/api/pre-styled-ui-api.md component links must exactly match the 116 nav-registered \
...
fn headless_backed_component_pages_link_bidirectionally_with_headless_ui_api() {
    ...
    assert_eq!(
        headless_backed.len(),
        68,
        "expected 68 headless-ui-backed component pages (nav ∩ headless-ui src mods), got {}: \
         {headless_backed:?}",
        headless_backed.len()
    );
