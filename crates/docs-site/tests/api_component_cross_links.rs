...
fn api_links_to_component_pages_are_all_nav_registered() {
    ...
    assert_eq!(
        nav_kebabs.len(),
        99,
        "expected 99 nav-registered component pages, got {}",
        nav_kebabs.len()
    );
...
fn every_component_page_links_back_to_pre_styled_ui_api() {
    ...
    assert_eq!(
        ...
        "docs/api/pre-styled-ui-api.md component links must exactly match the 99 nav-registered \
...
fn headless_backed_component_pages_link_bidirectionally_with_headless_ui_api() {
    ...
    assert_eq!(
        headless_backed.len(),
        56,
        "expected 56 headless-ui-backed component pages (nav ∩ headless-ui src mods), got {}: \
         {headless_backed:?}",
        headless_backed.len()
    );
