...
//! - 部品ページ → API ページ: 全 123 部品ページが
//!   `../../docs/api/pre-styled-ui-api.md` へのリンクを持ち、その集合は
//!   `pre-styled-ui-api.md` 側が指す 123 件と完全一致すること（イシュー
...
//!   イシュー #2063 で Input Group が加わり 110 → 111、イシュー #2066 で
//!   Item が加わり 111 → 112、イシュー #2060 で Button Group が加わり
//!   112 → 113、イシュー #2070 で Command が加わり 113 → 114、イシュー
//!   #2080 で Radial Chart が加わり 114 → 115、イシュー #2106 で Message
//!   が加わり 115 → 116、イシュー #2075 で Sidebar が加わり 116 → 117、
//!   イシュー #2109 で Bubble が加わり 117 → 118、イシュー #2112 で
//!   Attachment が加わり 118 → 119、イシュー #2115 で Marker が加わり
//!   119 → 120、イシュー #2119 で 120 → 121、イシュー #2123 で Message Scroller が加わり
//!   121 → 122、イシュー #2127 で Data Table が加わり 122 → 123）。
...
fn api_links_to_component_pages_are_all_nav_registered() {
    ...
    assert_eq!(
        nav_kebabs.len(),
        123,
        "expected 123 nav-registered component pages, got {}",
        nav_kebabs.len()
    );
...
fn every_component_page_links_back_to_pre_styled_ui_api() {
    ...
    assert_eq!(
        ...
        "docs/api/pre-styled-ui-api.md component links must exactly match the 123 nav-registered \
...
fn headless_backed_component_pages_link_bidirectionally_with_headless_ui_api() {
    ...
    assert_eq!(
        headless_backed.len(),
        75,
        "expected 75 headless-ui-backed component pages (nav ∩ headless-ui src mods), got {}: \
         {headless_backed:?}",
        headless_backed.len()
    );
