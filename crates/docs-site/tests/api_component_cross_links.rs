...
//! - 部品ページ → API ページ: 全 114 部品ページが
//!   `../../docs/api/pre-styled-ui-api.md` へのリンクを持ち、その集合は
//!   `pre-styled-ui-api.md` 側が指す 114 件と完全一致すること（イシュー
...
//!   112 → 113、イシュー #2070 で Command が加わり 113 → 114）。
...
fn api_links_to_component_pages_are_all_nav_registered() {
    ...
    assert_eq!(
        nav_kebabs.len(),
        114,
        "expected 114 nav-registered component pages, got {}",
        nav_kebabs.len()
    );
...
fn every_component_page_links_back_to_pre_styled_ui_api() {
    ...
    assert_eq!(
        ...
        "docs/api/pre-styled-ui-api.md component links must exactly match the 114 nav-registered \
