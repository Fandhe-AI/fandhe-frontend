...
//! - 部品ページ → API ページ: 全 99 部品ページが
//!   `../../docs/api/pre-styled-ui-api.md` へのリンクを持ち、その集合は
//!   `pre-styled-ui-api.md` 側が指す 99 件と完全一致すること。
...
/// headless-ui 裏付けを持つ部品ページ kebab（nav 登録 99 件と headless-ui
...
fn api_links_to_component_pages_are_all_nav_registered() {
    ...
    assert_eq!(
        nav_kebabs.len(),
        101,
        "expected 101 nav-registered component pages, got {}",
        nav_kebabs.len()
    );
...
fn every_component_page_links_back_to_pre_styled_ui_api() {
    ...
    assert_eq!(
        ...
        "docs/api/pre-styled-ui-api.md component links must exactly match the 101 nav-registered \
...
fn headless_backed_component_pages_link_bidirectionally_with_headless_ui_api() {
    ...
    assert_eq!(
        headless_backed.len(),
        58,
        "expected 58 headless-ui-backed component pages (nav ∩ headless-ui src mods), got {}: \
         {headless_backed:?}",
        headless_backed.len()
    );
