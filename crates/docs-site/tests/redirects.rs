...
/// 伴い `/components/input-group/` を追加し、113 件になった。イシュー
/// #2066 で Item の Themes ページ新設に伴い `/components/item/` を追加し、
...
/// `/components/message-scroller/` を追加し、124 件になった。イシュー
/// #2127 で Data Table の Themes ページ新設に伴い
/// `/components/data-table/` を追加し、現在は 125 件。本値の更新が
...
fn every_themes_page_has_exactly_one_matching_components_redirect() {
    ...
    // (kebab, path) 集合（117 件、イシュー #1683 で 107 → 108、
    ...
    // イシュー #2112 で 118 → 119、イシュー #2115 で 119 → 120、
    // イシュー #2119 で 120 → 121）。
    ...
    assert_eq!(
        themes_pages.len(),
        123,
        "expected 123 site/themes/ pages, got {}: {themes_pages:?}",
        themes_pages.len()
    );
