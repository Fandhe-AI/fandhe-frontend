...
/// 114 件になった。イシュー #2070 で Command の Themes ページ新設に伴い
/// `/components/command/` を追加し、現在は 115 件。本値の更新が要る変更は
...
fn every_themes_page_has_exactly_one_matching_components_redirect() {
    ...
    // (kebab, path) 集合（113 件、イシュー #1683 で 107 → 108、
    ...
    assert_eq!(
        themes_pages.len(),
        113,
        "expected 113 site/themes/ pages, got {}: {themes_pages:?}",
        themes_pages.len()
    );
