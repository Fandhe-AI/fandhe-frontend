...
/// `.rs` 総数と一致すること。75 をハードコードせず走査結果から導出する
/// （手動同期点を作らないため）。イシュー #2062 で `input_group`・イシュー
/// #2059 で `button_group` が加わり PRIMITIVES は 63 → 65、総数は
/// 73 → 75 になった。
#[test]
