...
/// `.rs` 総数と一致すること。76 をハードコードせず走査結果から導出する
...
/// なった。イシュー #2121 で `message_scroller` が加わり PRIMITIVES は
/// 73 → 74、総数は 83 → 84 になった。イシュー #2125 で `data_table` が
/// 加わり PRIMITIVES は 74 → 75、総数は 84 → 85 になった。
#[test]
fn module_counts_are_consistent_with_the_source_tree() {
    ...
    assert_eq!(
        ...
        scan.total_rs_files,
        "PRIMITIVES(75) + FOUNDATION_MODULES(9) + lib.rs(1) が \
         crates/headless-ui/src/*.rs の実測総数({})と一致しません",
        scan.total_rs_files
    );
}

/// 受け入れ条件 4: 6 グループ 13/12/10/10/13/17 = 75（イシュー #2125 で
/// Data Display / Utilities に `data_table` を追加、旧 13/12/10/10/13/16 = 74。
/// イシュー #2121 で
/// Data Display / Utilities に `message_scroller` を追加、旧
/// 13/12/10/10/13/15 = 73。イシュー #2117 で
