...
/// （手動同期点を作らないため）。イシュー #2062 で `input_group`・イシュー
/// #2059 で `button_group`・イシュー #2068 で `command` が加わり
/// PRIMITIVES は 63 → 66、総数は 73 → 76 になった。
#[test]
...
/// 受け入れ条件 4: 6 グループ 13/11/10/10/12/10 = 66（イシュー #2062 で
/// Forms A に `input_group` を、イシュー #2059 で Navigation に
/// `button_group` を、イシュー #2068 で Forms A に `command` を追加、
/// 旧 11/11/10/10/11/10 = 63）、カテゴリ出現順・グループ内順序が設計 §7
/// 逐語（+ #2059/#2062/#2068 追記）であること。
#[test]
