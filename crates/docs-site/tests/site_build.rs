...
fn build_site_succeeds_for_the_real_repository_site() {
    ...
    // 225 → 226 になった。イシュー #2094 で signup-01 が加わり、
    // 226 → 227 になった。イシュー #2119 で Questionnaire の Themes
    // ページが加わり、227 → 228 になった。
    assert_eq!(
        report.written.len(),
        228,
        "実サイトの生成ページ数が期待値と異なる: {:?}",
    ...
    // `/themes/` 配下に部品 118 件（イシュー #1683 で Collapsible が加わり
    ...
    assert_eq!(
        primitive_pages, 74,
        "/primitives/ 配下の生成ページ数（部品 73 件 + 索引 1 件）"
    );

    // アセットは site.css / admonition.css / skip-nav.css / site.js /
    // pre-styled-ui.css / primitives-showcase.css / search-index.json /
    // assets/image-demo.svg の 8 件（部品ページが showcase CSS を要求する
...
fn real_site_sidebar_is_scoped_to_the_current_section() {
    ...
    // （現在グループが開いている・部品 73 + 索引 1 = 74 件のリンクが
    ...
    // イシュー #2117 で Questionnaire が
    // それぞれ加わり
    // 63 → 65 → 66 → 67 → 69 → 70 → 71 → 72 → 73 部品）。
    ...
    let primitives_link_count = primitives_window
        ...
        .count();
    assert_eq!(
        primitives_link_count, 74,
        "Primitives サイドバーのリンク数が索引 1 + 部品 73 = 74 件と一致しない: {primitives_window}"
    );
}
