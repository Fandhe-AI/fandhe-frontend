...
fn build_site_succeeds_for_the_real_repository_site() {
    ...
    // 加わり、202 → 203 になった。イシュー #2068 で Command（Primitives）が
    // 加わり、203 → 204 になった。
    assert_eq!(
        report.written.len(),
        204,
    ...
    // それぞれ加わり部品 65 件 + 索引 1 件 = 66 件になった。イシュー #2068 で
    // Command が加わり部品 66 件 + 索引 1 件 = 67 件になった。
...
fn real_site_sidebar_is_scoped_to_the_current_section() {
    ...
    // Button Group・イシュー #2062 で Input Group・イシュー #2068 で
    // Command がそれぞれ加わり 63 → 66 部品）。
