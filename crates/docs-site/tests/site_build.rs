...
fn build_site_succeeds_for_the_real_repository_site() {
    ...
    // 上の 128 は「その時点の実測値」であり、Phase 6/7/8 でページが増減したら
    ...
    // /components/ 配下は部品ページ 106 件（イシュー #991 で Toolbar が
    ...
    // 103 → 105、イシュー #996 で Tab Nav が加わり 105 → 106）
    // + 索引ページ /components/pre-styled-ui/ 1 件の計 107 件（イシュー #943）。
    ...
    assert_eq!(
        component_pages, 107,
        "/components/ 配下の生成ページ数（部品 106 + 索引 1）"
    );
