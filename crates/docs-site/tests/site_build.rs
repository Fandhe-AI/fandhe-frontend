...
fn build_site_succeeds_for_the_real_repository_site() {
    ...
    // /components/ 配下は部品ページ 103 件（イシュー #991 で Toolbar が
    // 加わり 99 → 100、イシュー #992 で Menubar が加わり 100 → 101、
    // イシュー #993 で Navigation Menu が加わり 101 → 102、イシュー #996 で
    // Tab Nav が加わり 102 → 103） + 索引ページ /components/pre-styled-ui/
    // 1 件の計 104 件（イシュー #943）。
    ...
    assert_eq!(
        component_pages, 104,
        "/components/ 配下の生成ページ数（部品 103 + 索引 1）"
    );
