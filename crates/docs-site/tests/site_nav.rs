...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // イシュー #2646 で Wireframes セクションへ Alert 部品ページ（同 Phase
    ...
    // #2646 で Alert（同 Phase の 2 番目の部品）が加わり 31 件になった。
    ...
    assert_eq!(
        ...
        31,
        "expected 31 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + accordion + pagination + cursor + tooltip + alert), got {wireframes_pages:?}"
    );
    assert!(
