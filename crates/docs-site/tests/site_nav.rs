...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // になった。イシュー #2068 で Command（Primitives）が加わり 203 → 204
    // になった。
    ...
    // イシュー #2068 で Command が加わり 66 → 67 件になった。
    ...
    assert_eq!(
        ...
        67,
        "expected 67 /primitives/ pages (66 部品 + 1 索引), got {primitives_pages:?}"
    );
    let source_based_primitive_pages = pages
        ...
        .count();
    assert_eq!(
        source_based_primitive_pages, 66,
        "expected 66 pages sourced from site/primitives/"
    );
    assert!(
    ...
    // `/themes/` 配下は部品ページ 111 件 + 索引ページ（`/themes/` 自身）1 件
    // の 112 件（イシュー #1018。イシュー #1683 で部品ページが 107 → 108、
    ...
    assert_eq!(
        themes_pages.len(),
        112,
        "expected 112 /themes/ pages (111 部品 + 1 索引), got {themes_pages:?}"
    );
    ...
    let source_based_component_pages = pages
        ...
        .count();
    assert_eq!(
        source_based_component_pages, 111,
        "expected 111 pages sourced from site/themes/"
    );
