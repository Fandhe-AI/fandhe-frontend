...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 加わり、262 → 263 になった。イシュー #2622 で Wireframes セクションへ
    // Input 部品ページが加わり、263 → 264 になった。
    ...
    // イシュー #2622 で Input が加わり 12 件になった。
    ...
    assert_eq!(
        ...
        12,
        "expected 12 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + button + select + switch + checkbox + input), got {wireframes_pages:?}"
    );
    assert!(
