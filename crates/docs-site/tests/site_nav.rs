...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Nav item 部品ページが加わり、277 → 278 になった。イシュー #2640 で
    // Wireframes セクションへ Pagination 部品ページが加わり、278 → 279
    // になった。
    assert_eq!(pages.len(), 279, "expected 279 pages, got {pages:?}");
    ...
    // なった。イシュー #2640 で Pagination が加わり 27 件になった。
    ...
    assert_eq!(
        ...
        27,
        "expected 27 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + pagination), got {wireframes_pages:?}"
    );
    assert!(
