...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 197 → 198 になった。
    assert_eq!(pages.len(), 198, "expected 198 pages, got {pages:?}");
    ...
    // `/themes/` 配下は部品ページ 108 件 + 索引ページ（`/themes/` 自身）1 件
    // の 109 件（イシュー #1018。イシュー #1683 で部品ページが 107 → 108）。
    ...
    assert_eq!(
        themes_pages.len(),
        109,
        "expected 109 /themes/ pages (108 部品 + 1 索引), got {themes_pages:?}"
    );
    ...
    let source_based_component_pages = pages
        ...
        .count();
    assert_eq!(
        source_based_component_pages, 108,
        "expected 108 pages sourced from site/themes/"
    );
