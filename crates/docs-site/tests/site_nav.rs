...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // イシュー #2612 で Divider が加わり 4 件になった。イシュー #2615 で
    // Paragraph が加わり 5 件になった。
    ...
    assert_eq!(
        ...
        5,
        "expected 5 /wireframes/ pages (index + annotation + grid + divider + paragraph), got {wireframes_pages:?}"
    );
    assert!(
