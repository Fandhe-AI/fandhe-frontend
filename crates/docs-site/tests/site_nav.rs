...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Question 部品ページが加わり、270 → 271 になった。イシュー #2614 で
    // Wireframes セクションへ Text 部品ページが加わり、271 → 272 になった
    // （これで Phase 2「テキスト・注釈」の全部品が出揃った）。
    assert_eq!(pages.len(), 272, "expected 272 pages, got {pages:?}");
    ...
    // 19 件になった。イシュー #2614 で Text が加わり 20 件になった
    ...
    assert_eq!(
        ...
        20,
        "expected 20 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + text), got {wireframes_pages:?}"
    );
    assert!(
