...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Wireframes セクションへ Alert 部品ページ（同 Phase の 3 番目の部品）
    // が加わり、283 → 284 になった。
    ...
    // イシュー #2646 で Alert（同 Phase の 3 番目の部品）が加わり 32 件に
    // なった。
    ...
    assert_eq!(
        ...
        32,
        "expected 32 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + accordion + pagination + cursor + tooltip + toast + alert), got {wireframes_pages:?}"
    );
    assert!(
