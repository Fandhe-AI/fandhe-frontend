...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // イシュー #2649 で Wireframes セクションへ Spinner 部品ページ
    // （Phase 6 の 2 番目の部品）が加わり、282 → 283 になった。
    ...
    // #2649 で Spinner（Phase 6 の 2 番目の部品）が加わり 31 件になった。
    ...
    assert_eq!(
        ...
        31,
        "expected 31 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + accordion + pagination + cursor + tooltip + spinner), got {wireframes_pages:?}"
    );
    assert!(
