...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // sidebar-03 が加わり、224 → 225 になった。イシュー #2095 で signup-05
    // が加わり、225 → 226 になった。
    ...
    // sidebar-03 が加わり 5 件。イシュー #2095 で signup-05 が加わり 6 件。
    ...
    assert_eq!(
        blocks_pages.len(),
        6,
        "expected 6 /blocks/ pages (index + login-01 + dashboard-01 + sidebar-07 + sidebar-03 + signup-05), got {blocks_pages:?}"
    );
    assert!(
