...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // なった。イシュー #2609 で Wireframes セクションへ Frame 部品ページが
    // 加わり、256 → 257 になった。
    ...
    // イシュー #2612 で Divider が加わり 4 件になった。イシュー #2609 で
    // Frame 部品ページが加わり 5 件になった。Phase 1・3 以降
    ...
    assert_eq!(
        ...
        5,
        "expected 5 /wireframes/ pages (index + annotation + grid + divider + frame), got {wireframes_pages:?}"
    );
    assert!(
