...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 253 → 254 になった。イシュー #2610 で Wireframes セクションへ
    // Stack 部品ページが加わり、254 → 255 になった。
    assert_eq!(pages.len(), 255, "expected 255 pages, got {pages:?}");

    // イシュー #2607: `/wireframes/` 配下は索引ページ（`/wireframes/` 自身）
    // 1 件のみだった（同イシュー時点では個別部品ページを同梱しない、設計
    // 文書 §12 D2）。イシュー #2617 で最初の部品ページ（Annotation）が
    // 加わり 2 件になった。イシュー #2610 で Stack 部品ページが加わり
    // 3 件になった。Phase 1・3 以降（#2608〜#2665）の各部品イシューが
    // 増分する。
    ...
    assert_eq!(
        ...
        3,
        "expected 3 /wireframes/ pages (index + annotation + stack), got {wireframes_pages:?}"
    );
    assert!(
