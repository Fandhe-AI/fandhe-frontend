...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Link 部品ページが加わり、257 → 258 になった。イシュー #2619 で
    // Wireframes セクションへ Tag 部品ページが加わり、258 → 259 になった。
    assert_eq!(pages.len(), 259, "expected 259 pages, got {pages:?}");
    ...
    // なった。イシュー #2619 で Tag が加わり 7 件になった。Phase 1・3 以降
    ...
    assert_eq!(
        ...
        7,
        "expected 7 /wireframes/ pages (index + annotation + grid + divider + stack + link + tag), got {wireframes_pages:?}"
    );
    assert!(
