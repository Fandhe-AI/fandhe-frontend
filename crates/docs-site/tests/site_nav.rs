...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    assert_eq!(
        ...
        8,
        "expected 8 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph), got {wireframes_pages:?}"
    );
    assert!(
