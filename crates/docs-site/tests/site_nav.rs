...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // セクションへ Button 部品ページが加わり、260 → 261 になった。
    ...
    // #2615 で Paragraph が加わり 8 件になった。イシュー #2621 で Button
    // が加わり 9 件になった。
    ...
    assert_eq!(
        ...
        9,
        "expected 9 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button), got {wireframes_pages:?}"
    );
    assert!(
