...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // ページが加わり、272 → 273 になった。イシュー #2638 で Wireframes
    // セクションへ Tabs 部品ページが加わり、273 → 274 になった。
    ...
    // イシュー #2633 で File drop が加わり 21 件になった。イシュー #2638 で
    // Tabs が加わり 22 件になった。
    // Phase 1・3・5 以降（#2608〜#2665）の各部品イシューが増分する。
    ...
    assert_eq!(
        ...
        22,
        "expected 22 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + file-drop + tabs), got {wireframes_pages:?}"
    );
    assert!(
