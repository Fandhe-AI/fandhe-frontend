...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Checkbox 部品ページが加わり、263 → 264 になった。イシュー #2619 で
    // Wireframes セクションへ Tag 部品ページが加わり、264 → 265 になった。
    ...
    // イシュー #2625 で Checkbox が加わり 12 件になった。イシュー #2619 で
    // Tag が加わり 13 件になった。
    ...
    assert_eq!(
        ...
        13,
        "expected 13 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + button + select + radio + switch + checkbox + tag), got {wireframes_pages:?}"
    );
    assert!(
