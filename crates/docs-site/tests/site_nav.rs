...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // へ File drop 部品ページが加わり、273 → 274 になった。イシュー #2638 で
    // Wireframes セクションへ Tabs 部品ページが加わり、274 → 275 になった。
    ...
    // File drop が加わり 22 件になった。イシュー #2638 で Tabs が加わり
    // 23 件になった。
    ...
    assert_eq!(
        ...
        23,
        "expected 23 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + tabs), got {wireframes_pages:?}"
    );
    assert!(
