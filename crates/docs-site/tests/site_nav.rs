...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // cta-signup-celebrate の 2 ページが加わり、239 → 241 になった。
    ...
    // testimonials-stack が加わり 10 → 11 件になった。イシュー #2550 で
    // cta-banner-magnetic・cta-signup-celebrate が加わり 11 → 13 件になった。
    ...
    assert_eq!(
        blocks_pages.len(),
        13,
        "expected 13 /blocks/ pages (index + login-01 + login-04 + dashboard-01 + sidebar-07 + sidebar-03 + signup-01 + signup-05 + pricing-tiers-morph + pricing-usage-slider + testimonials-stack + cta-banner-magnetic + cta-signup-celebrate), got {blocks_pages:?}"
    );
    assert!(
