...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 10 件になった。イシュー #2622 で Input が加わり 11 件になった。
    ...
    assert_eq!(
        ...
        11,
        "expected 11 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + button + select + switch + input), got {wireframes_pages:?}"
    );
    assert!(
