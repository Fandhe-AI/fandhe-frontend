...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // なった。イシュー #2619 で Wireframes セクションへ Tag 部品ページが
    // 加わり、259 → 260 になった。イシュー #2621 で Wireframes セクションへ
    // Button 部品ページが加わり、260 → 261 になった。
    assert_eq!(pages.len(), 261, "expected 261 pages, got {pages:?}");
    ...
    // #2619 で Tag が加わり 8 件になった。イシュー #2621 で Button が
    // 加わり 9 件になった。Phase 1・3 以降
    // （#2608〜#2665）の各部品イシューが増分する。
    ...
    assert_eq!(
        ...
        9,
        "expected 9 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + tag + button), got {wireframes_pages:?}"
    );
    assert!(
