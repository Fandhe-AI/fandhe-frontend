...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 253 → 254 になった。イシュー #2609 で Wireframes セクションへ
    // Frame 部品ページが加わり、254 → 255 になった。
    ...
    // 加わり 2 件になった。イシュー #2609 で Frame 部品ページが加わり
    ...
    assert_eq!(
        ...
        3,
        "expected 3 /wireframes/ pages (index + annotation + frame), got {wireframes_pages:?}"
    );
    assert!(
