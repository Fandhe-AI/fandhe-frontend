...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // なった。イシュー #2634 で Wireframes セクションへ Stepper 部品ページが
    // 加わり、272 → 273 になった。
    ...
    // イシュー #2634 で Stepper が加わり 21 件になった。
    ...
    assert_eq!(
        ...
        21,
        "expected 21 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + stepper), got {wireframes_pages:?}"
    );
    assert!(
