...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Select 部品ページが加わり、260 → 261 になった。イシュー #2627 で
    // Wireframes セクションへ Switch 部品ページが加わり、261 → 262 になった。
    // イシュー #2625 で Wireframes セクションへ Checkbox 部品ページが
    // 加わり、262 → 263 になった。イシュー #2630 で Wireframes セクションへ
    // Question 部品ページが加わり、263 → 264 になった。
    assert_eq!(pages.len(), 264, "expected 264 pages, got {pages:?}");
    ...
    // Select が加わり 9 件になった。イシュー #2627 で Switch が加わり
    // 10 件になった。イシュー #2625 で Checkbox が加わり 11 件になった。
    // イシュー #2630 で Question が加わり 12 件になった。
    ...
    assert_eq!(
        ...
        12,
        "expected 12 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + button + select + switch + checkbox + question), got {wireframes_pages:?}"
    );
    assert!(
