...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Link 部品ページが加わり、257 → 258 になった。イシュー #2615 で
    // Wireframes セクションへ Paragraph 部品ページが加わり、258 → 259
    // になった。
    ...
    // なった。イシュー #2615 で Paragraph が加わり 7 件になった。
    ...
    assert_eq!(
        ...
        7,
        "expected 7 /wireframes/ pages (index + annotation + grid + divider + stack + link + paragraph), got {wireframes_pages:?}"
    );
    assert!(
