...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // イシュー #2059 で Button Group（Primitives）が加わり 200 → 201 になった。
    assert_eq!(pages.len(), 201, "expected 201 pages, got {pages:?}");

    // イシュー #1021/#2059: `/primitives/` 配下は部品ページ 64 件 + 索引ページ
    // （`/primitives/` 自身）1 件の 65 件。
