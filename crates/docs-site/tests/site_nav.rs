...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Question 部品ページが加わり、270 → 271 になった。イシュー #2633 で
    // Wireframes セクションへ File drop 部品ページが加わり、271 → 272 に
    // なった。
    ...
    // 19 件になった。イシュー #2633 で File drop が加わり 20 件になった。
    ...
    assert_eq!(
        ...
        20,
        "expected 20 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + file-drop), got {wireframes_pages:?}"
    );
    assert!(
