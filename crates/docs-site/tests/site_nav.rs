...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 表示」の 4 番目の部品）が加わり、297 → 298 になった。イシュー #2652 で
    // Wireframes セクションへ Icon 部品ページ（Phase 7「Data display」の
    // 7 番目の部品）が加わり、298 → 299 になった。
    ...
    // 加わり 46 件になった。イシュー #2652 で Icon（Phase 7「Data display」の
    // 7 番目の部品）が加わり 47 件になった。
    ...
    assert_eq!(
        ...
        47,
        "expected 47 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + accordion + pagination + cursor + menu + breadcrumbs + tooltip + toast + alert + progress + spinner + modal + avatar + counter + emoji + stat + chart + image + card-basic + map + media + icon), got {wireframes_pages:?}"
    );
    assert!(
