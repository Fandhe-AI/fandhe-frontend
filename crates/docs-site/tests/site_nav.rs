...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // なった。イシュー #2614 で Wireframes セクションへ Text 部品ページが
    // 加わり、272 → 273 になった（これで Phase 2「テキスト・注釈」の
    // 全部品が出揃った）。
    assert_eq!(pages.len(), 273, "expected 273 pages, got {pages:?}");
    ...
    // イシュー #2614 で Text が加わり 21 件になった（Phase 2
    // 「テキスト・注釈」の全部品が出揃った）。
    ...
    assert_eq!(
        ...
        21,
        "expected 21 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + text), got {wireframes_pages:?}"
    );
    assert!(
