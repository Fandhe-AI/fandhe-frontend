...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // なった。イシュー #2658 で Wireframes セクションへ Card basic 部品
    // ページ（Phase 7「Data display」の 5 番目の部品）が加わり、
    // 294 → 295 になった。
    assert_eq!(pages.len(), 295, "expected 295 pages, got {pages:?}");
    ...
    // データ表示」の最初の部品）が加わり 42 件になった。イシュー #2658 で
    // Card basic（Phase 7「Data display」の 5 番目の部品）が加わり
    // 43 件になった。
    ...
    assert_eq!(
        ...
        43,
        "expected 43 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + accordion + pagination + cursor + menu + breadcrumbs + tooltip + toast + alert + progress + spinner + modal + avatar + counter + emoji + stat + chart + card-basic), got {wireframes_pages:?}"
    );
    assert!(
