...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Checkbox 部品ページが加わり、263 → 264 になった。イシュー #2630 で
    // Wireframes セクションへ Question 部品ページが加わり、264 → 265 に
    // なった。
    ...
    // イシュー #2625 で Checkbox が加わり 12 件になった。イシュー #2630 で
    // Question が加わり 13 件になった。
    ...
    assert_eq!(
        ...
        13,
        "expected 13 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + button + select + radio + switch + checkbox + question), got {wireframes_pages:?}"
    );
    assert!(
