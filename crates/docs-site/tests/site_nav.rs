...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // なった。イシュー #2622 で Wireframes セクションへ Input 部品ページが
    // 加わり、259 → 260 になった。
    ...
    assert_eq!(
        ...
        8,
        "expected 8 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + input), got {wireframes_pages:?}"
    );
    assert!(
