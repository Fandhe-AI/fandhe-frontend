...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 加わり、256 → 257 になった。イシュー #2615 で Wireframes セクション
    // へ Paragraph 部品ページが加わり、257 → 258 になった。
    assert_eq!(pages.len(), 258, "expected 258 pages, got {pages:?}");
    ...
    // Stack が加わり 5 件になった。イシュー #2615 で Paragraph が加わり
    // 6 件になった。Phase 1・3 以降（#2608〜#2665）の各部品イシューが
    // 増分する。
    ...
    assert_eq!(
        ...
        6,
        "expected 6 /wireframes/ pages (index + annotation + grid + divider + stack + paragraph), got {wireframes_pages:?}"
    );
    assert!(
