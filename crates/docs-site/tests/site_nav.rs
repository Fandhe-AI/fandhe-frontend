...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Select 部品ページが加わり、260 → 261 になった。イシュー #2622 で
    // Wireframes セクションへ Input 部品ページが加わり、261 → 262 になった。
    ...
    // Select が加わり 9 件になった。イシュー #2622 で Input が加わり
    // 10 件になった。
    ...
    assert_eq!(
        ...
        10,
        "expected 10 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + button + select + input), got {wireframes_pages:?}"
    );
    assert!(
