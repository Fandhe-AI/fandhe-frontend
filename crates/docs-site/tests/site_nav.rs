...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // イシュー #2651 で Wireframes セクションへ Avatar 部品ページ
    // （Phase 7「Data display」の最初の部品）が加わり、282 → 283 に
    // なった。
    assert_eq!(pages.len(), 283, "expected 283 pages, got {pages:?}");
    ...
    // #2651 で Avatar（Phase 7「Data display」の最初の部品）が加わり
    // 31 件になった。
    ...
    assert_eq!(
        ...
        31,
        "expected 31 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + accordion + pagination + cursor + tooltip + avatar), got {wireframes_pages:?}"
    );
    assert!(
