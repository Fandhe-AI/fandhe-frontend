...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 表示」の 4 番目の部品）が加わり、297 → 298 になった。
    ...
    assert_eq!(
        ...
        46,
        "expected 46 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + accordion + pagination + cursor + menu + breadcrumbs + tooltip + toast + alert + progress + spinner + modal + avatar + counter + emoji + stat + chart + image + card-basic + map + media), got {wireframes_pages:?}"
    );
    assert!(
