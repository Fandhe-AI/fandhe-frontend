...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Grid 部品ページが加わり、254 → 255 になった。イシュー #2616 で
    // Wireframes セクションへ Rich text 部品ページが加わり、255 → 256 に
    // なった。
    assert_eq!(pages.len(), 256, "expected 256 pages, got {pages:?}");
    ...
    // イシュー #2616 で Rich text が加わり 4 件になった。Phase 1・3 以降
    ...
    assert_eq!(
        ...
        4,
        "expected 4 /wireframes/ pages (index + annotation + grid + rich-text), got {wireframes_pages:?}"
    );
    assert!(
