...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 213 → 214 になった。
    assert_eq!(pages.len(), 214, "expected 214 pages, got {pages:?}");
    ...
    assert_eq!(
        ...
        71,
        "expected 71 /primitives/ pages (70 部品 + 1 索引), got {primitives_pages:?}"
    );
    let source_based_primitive_pages = pages
        ...
        .count();
    assert_eq!(
        source_based_primitive_pages, 70,
        "expected 70 pages sourced from site/primitives/"
    );
    assert!(
    ...
    // `/themes/` 配下は部品ページ 117 件 + 索引ページ（`/themes/` 自身）1 件
    // の 118 件（イシュー #1018。イシュー #1683 で部品ページが 107 → 108、
    ...
    // イシュー #2075 で Sidebar が加わり 116 → 117）。
    ...
    assert_eq!(
        themes_pages.len(),
        118,
        "expected 118 /themes/ pages (117 部品 + 1 索引), got {themes_pages:?}"
    );
    ...
    let source_based_component_pages = pages
        ...
        .count();
    assert_eq!(
        source_based_component_pages, 117,
        "expected 117 pages sourced from site/themes/"
    );
