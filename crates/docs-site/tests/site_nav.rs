...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // なった。イシュー #2616 で Wireframes セクションへ Rich text 部品
    // ページが加わり、256 → 257 になった。
    assert_eq!(pages.len(), 257, "expected 257 pages, got {pages:?}");
    ...
    // イシュー #2612 で Divider が加わり 4 件になった。イシュー #2616 で
    // Rich text が加わり 5 件になった。Phase 1・3 以降（#2608〜#2665）の
    // 各部品イシューが増分する。
    ...
    assert_eq!(
        ...
        5,
        "expected 5 /wireframes/ pages (index + annotation + grid + divider + rich-text), got {wireframes_pages:?}"
    );
    assert!(
