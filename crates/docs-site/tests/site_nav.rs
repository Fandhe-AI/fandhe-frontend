...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // pricing-usage-slider の 2 ページが加わり、236 → 238 になった。
    // イシュー #2550 で Blocks セクションへ cta-banner-magnetic・
    // cta-signup-celebrate の 2 ページが加わり、238 → 240 になった。
    ...
    // pricing-usage-slider が加わり 8 → 10 件になった。イシュー #2550 で
    // cta-banner-magnetic・cta-signup-celebrate が加わり 10 → 12 件になった。
    ...
    assert_eq!(
        blocks_pages.len(),
        12,
        "expected 12 /blocks/ pages (index + login-01 + login-04 + dashboard-01 + sidebar-07 + sidebar-03 + signup-01 + signup-05 + pricing-tiers-morph + pricing-usage-slider + cta-banner-magnetic + cta-signup-celebrate), got {blocks_pages:?}"
    );
    assert!(
