...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Grid 部品ページが加わり、254 → 255 になった。イシュー #2609 で
    // Wireframes セクションへ Frame 部品ページが加わり、255 → 256 になった。
    ...
    // イシュー #2609 で Frame 部品ページが加わり 4 件になった。Phase 1・
    // 3 以降（#2608〜#2665）の各部品イシューが増分する。
    ...
    assert_eq!(
        ...
        4,
        "expected 4 /wireframes/ pages (index + annotation + grid + frame), got {wireframes_pages:?}"
    );
    assert!(
