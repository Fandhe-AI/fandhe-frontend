...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // Grid 部品ページが加わり、254 → 255 になった。イシュー #2610 で
    // Wireframes セクションへ Stack 部品ページが加わり、255 → 256 に
    // なった。
    ...
    // イシュー #2610 で Stack が加わり 4 件になった。Phase 1・3 以降
    ...
    assert_eq!(
        ...
        4,
        "expected 4 /wireframes/ pages (index + annotation + grid + stack), got {wireframes_pages:?}"
    );
    assert!(
