...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // へ File drop 部品ページが加わり、273 → 274 になった。
    assert_eq!(pages.len(), 274, "expected 274 pages, got {pages:?}");
    ...
    assert_eq!(
        ...
        22,
        "expected 22 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop), got {wireframes_pages:?}"
    );
    assert!(
