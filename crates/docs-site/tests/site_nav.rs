...
fn site_nav_registers_all_pages_with_expected_paths() {
    ...
    // 最初の部品）が加わり、280 → 281 になった。イシュー #2645 で
    // Wireframes セクションへ Modal 部品ページ（Phase 6 の 2 番目の部品）
    // が加わり、281 → 282 になった。
    ...
    // #2640 で Pagination が加わり 28 件になった。イシュー #2644 で
    // Tooltip（Phase 6「Overlay・Feedback」の最初の部品）が加わり 29 件に
    // なった。イシュー #2645 で Modal（Phase 6 の 2 番目の部品）が加わり
    // 30 件になった。
    ...
    assert_eq!(
        ...
        30,
        "expected 30 /wireframes/ pages (index + annotation + grid + divider + stack + link + rich-text + paragraph + button + select + radio + switch + checkbox + textarea + slider + frame + tag + input + question + ratings + calendar + file-drop + text + tabs + stepper + nav-item + accordion + pagination + tooltip + modal), got {wireframes_pages:?}"
    );
    assert!(
