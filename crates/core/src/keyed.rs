...
//! 条件に影響しない。
//!
//! サブモジュール [`template`] は、Insert 対象アイテム群の構造同型判定と
//! 束縛点パス導出（イシュー #1384）を担う。DOM 適用（`clone_node` 方式、
//! 後続イシュー #1385）の前段にあたる純 Rust 処理であり、本モジュール同様
//! JS 境界呼び出しを持たない。詳細は [`template`] モジュール doc 参照。
