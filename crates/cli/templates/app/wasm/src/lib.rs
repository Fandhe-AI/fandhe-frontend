...
//! カスタムセクション）は `fandhe-frontend-wasm-client` 側のコンパイル単位で生成され、
...
//! `.wasm` を後処理し、`static/wasm/fandhe_frontend_wasm_client.js` /
//! `fandhe_frontend_wasm_client_bg.wasm` を生成する（`--out-name fandhe_frontend_wasm_client` で
...
// `fandhe-frontend-wasm-client` の `hydrate`/`mount_csr` は wasm32 配線層
...
pub use fandhe_frontend_wasm_client::{hydrate, mount_csr};
