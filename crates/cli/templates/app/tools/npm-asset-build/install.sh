...
#
# テンプレート同梱（イシュー #316）: 本ファイルは templates/default/tools/npm-asset-build/
# へバイト同一のままコピー同梱される。正本はこのファイル（tools/npm-asset-build/）であり、
# 変更時は tools/npm-asset-build/tests/test_template_sync.sh がドリフトを検知する。
