#!/usr/bin/env bash
# tools/wasm/build.sh — CSR wasm ビルド手順（イシュー #503。
# templates/app/tools/wasm/build.sh を下敷きにした examples 版）。
#
# 役割: `examples/interactive-view-transitions/wasm`（独立ワークスペース、
# glue クレート `interactive-vt-wasm`）を wasm32-unknown-unknown へビルドし、
# wasm-bindgen CLI で `static/wasm/fandhe_frontend_wasm_full.js` /
# `fandhe_frontend_wasm_full_bg.wasm` を生成する。`static/embed.html` の
...
# （`.claude/rules/security.md` A01/A03）。
set -euo pipefail
...
if [ ! -f "${wasm_manifest}" ]; then
  echo "error: ${wasm_manifest} not found. Run this script from examples/interactive-view-transitions/." >&2
  exit 1
fi
...
wasm_artifact="${target_dir}/wasm32-unknown-unknown/release/interactive_vt_wasm.wasm"
