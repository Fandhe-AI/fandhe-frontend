...
//!
//! `routes.rs`（`fandhe-frontend-router-v1` 抽出器）と同じ方針を踏襲する:
...
mod tests {
    ...
    fn extracts_real_app_crate_loader_impls() {
        // 統合的な回帰テスト: このリポジトリの実 `crates/app/src/lib.rs` から
        // 実際に Loader 実装を抽出できること（`docs/design/loader-trait-design.md`
        // の実装、`DemoItemsLoader` / `DemoItemDetailLoader`）。このテスト
        // バイナリは `crates/cli/` 配下でビルドされるため、2 段の親ディレクトリで
        // ワークスペースルートを得る（イシュー #436）。
        let workspace_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(|p| p.parent())
            .expect("crates/cli/ has a workspace root two levels up");
        let content = std::fs::read_to_string(workspace_root.join("crates/app/src/lib.rs"))
            .expect("crates/app/src/lib.rs should be readable");
