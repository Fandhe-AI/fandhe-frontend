...
mod dev_fs {
    ...
    ///
    /// `build.rs`（`crates/dist-server/build.rs`）と同一の基準
    /// （`CARGO_MANIFEST_DIR` の 2 段上（ワークスペースルート）+ `static`、
    /// イシュー #436 の `crates/` 配下移設で 1 段から 2 段へ変更）を用いる。
    /// 開発ビルドはソースツリー上での実行が前提であり、`CARGO_MANIFEST_DIR` は
    /// コンパイル時に埋め込まれる定数のためリクエスト入力に依存しない
    /// （`parent()` チェーンが `None` になるのは `CARGO_MANIFEST_DIR` が
    /// ファイルシステムルート付近の場合のみで、cargo ワークスペース構成では
    /// 実質到達不能）。ただし `coding-rust.md`「ライブラリコードでの
