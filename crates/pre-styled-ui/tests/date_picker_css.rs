...
//! から新規生成した期待値である。
//!
//! イシュー #2013（shadcn/ui 突合）で `input`/`trigger` の `data-invalid`、
//! `input` の `data-readonly` を新規消費するようになったため、期待値を
//! 再生成した（追加分はモジュール rustdoc「スタイル調整（イシュー #2013）」
//! 節参照）。
...
[data-scope="date-picker"][data-part="input"][data-readonly] {
  cursor: default;
}
