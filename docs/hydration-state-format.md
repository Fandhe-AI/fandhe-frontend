...
`docs/hydration-api.md`（TASK-6.2a）・`docs/interactive-api.md`（TASK-11.1a）・
`docs/wasm-full-architecture.md`（TASK-11.2a）と同じ書式（ステータス・トレーサビ
...
- `rws-interactive` の `Hydrate` トレイト・`HYDRATE_ATTR_PREFIX`・`HydrateError`・
  ...
  `docs/interactive-api.md` 第 3〜4 節の凍結表と一致）
...
- `rws-wasm-full` クレートの現状（`wasm-full/src/lib.rs`・`events.rs`・`dom.rs`。
  ...
  `docs/wasm-full-architecture.md` 第 3.1 節）
...
並列進行中**であり、`docs/wasm-full-architecture.md` 第 3.2 節が凍結したシグネチャ
...
- ラウンドトリップ整合性（区切り文字・エスケープ文字混入時を含む）は
  `rws-interactive` 側で保証済み（`docs/interactive-api.md` 第 6 節・不変条件 5）
...
| # | 判断 | 根拠 |
|---|------|------|
| 1 | 属性命名は `data-hydrate-<field>`（`<field>` は ASCII 小文字英数字・ハイフンのみ）とし、`HYDRATE_ATTR_PREFIX` は `rws-interactive` 側の定数を単一の真実として再定義しない | `docs/interactive-api.md` 第 4 節・判断 3 の踏襲。属性名の生成主体（アプリ側 `Hydrate` 実装）と読み取り主体（`wasm-full`）が同一プレフィックスを共有する必要があり、複数箇所へのハードコードは規約の分岐（単一の真実の原則違反）を招く |
...
| 3 | 数値パース失敗は `HydrateError::InvalidValue` として扱い、`unwrap()`/`panic!` を使わない | `.claude/rules/coding-rust.md` のエラーハンドリング規約・`docs/interactive-api.md` 第 4 節・判断 4 の踏襲。`data-hydrate-*` 属性値は改ざんされうるクライアント入力であり、パース失敗時に panic すると DoS（クラッシュ）につながる |
...
| 5 | 属性値は改ざんされうるクライアント入力として扱い、復元は `from_hydration_attrs` → `Result<_, HydrateError>` の経路のみを許容する。未知の `data-hydrate-*` 属性は無視する | `docs/interactive-api.md` 第 6 節・不変条件 3 の踏襲。DOM は攻撃者（ブラウザ拡張・DevTools・XSS 経由の別スクリプト等）が改変可能な信頼境界外の入力源であり、パース・復元処理は防御的に書く |
| 6 | `HydrateError` 発生時は panic せず、初期状態での CSR 再描画へフォールバックする | `docs/wasm-full-architecture.md` 第 4 節・判断 5 の再掲・確定。本書はこのフォールバック方針をフォーマット層の不変条件としても明記し、`hydration.rs`（#83）の実装契約とする |
| 7 | 巨大な属性値・リスト長の上限（DoS 耐性）は本タスクでは規定せず、TASK-11.4b（#83）の実装検討事項として引き継ぐ | `docs/interactive-api.md` 第 5 節・第 6 節・不変条件 8 の記録を継承する。放置せず引き継ぎ先を明示することで `.claude/rules/out-of-scope-tracking.md` の規約を満たす |
...
- `docs/wasm-full-architecture.md` 第 3.2 節で凍結済みのシグネチャ
...
`core/src/lib.rs` 冒頭・`docs/interactive-api.md` 第 6 節・
`docs/wasm-full-architecture.md` 第 7 節に記載された不変条件（REQ-1・REQ-2）を、
...
6. **エラー・ログの機微情報非露出（A09 相当）**: `HydrateError` の
   ...
   （`docs/wasm-full-architecture.md` 第 7 節・不変条件 5 と同一方針）。
...
| WASM 完全方式でのイベント処理・DOM 操作が `unsafe` を使用せず safe Rust の範囲に収まること | `read_hydration_attrs`/`restore_state` は `rws-wasm-full` の `#![deny(unsafe_code)]` 方針（`docs/wasm-full-architecture.md` 第 2 節）の範囲内で実装する自作コード（`unsafe` は `wasm-bindgen` 生成コードに限定） | TASK-11.4b（#83） |

## 10. 関連文書との整合確認

- `docs/interactive-api.md` 第 3〜4 節・第 6 節で凍結済みの `Hydrate`・
  `HYDRATE_ATTR_PREFIX`・`HydrateError`・`codec::encode_list`/`decode_list` の
  シグネチャをそのまま引用し、本書側で再定義・変更していない。
- `docs/wasm-full-architecture.md` 第 3.1 節「`hydration` モジュールは
...
- `docs/unsafe-boundary.md` は本タスク（docs-only・依存追加なし）では更新
