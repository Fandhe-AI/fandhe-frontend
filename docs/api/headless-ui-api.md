...
| Item（[Primitives](../../site/primitives/item.md)、イシュー #2065。Themes ページは #2066 で追加予定） | `item` | Root/Media/Content/Title/Description/Actions/Header/Footer/Group/Separator | なし（自由関数のみ。root は href 指定で a 描画・external で target/rel 不可分付与。group は role="group"〔shadcn の role="list" からの意図的差分〕。参照実体は shadcn/ui のみ〔#2001〕。予約キー除去あり） |
