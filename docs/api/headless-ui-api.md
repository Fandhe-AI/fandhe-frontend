...
| Command（[Primitives](../../site/primitives/command.md)、イシュー #2068。Themes ページは #2070 で追加予定） | `command` | Root/Input/List/Empty/Group/GroupHeading/Item/Shortcut/Separator/Dialog | `state::Disclosure`（dialog 開閉）+ `state::TextInput`（検索クエリ）+ `state::SingleSelect`（選択中 item の value）。combobox/listbox の既存 ARIA 実装（`role="combobox"`/`role="listbox"`/`role="option"`）を再利用し、`aria-controls`/`aria-label` を必須引数（`Option` opt-in にしない）にすることで `docs-site` の R1〜R4 契約を型で構造的に満たす。`item` は `data-selected`（presence）で選択有無を表し `data-highlighted`/`data-state` は出力しない（combobox/listbox との意図的な差分）。`Input` は dialog を開閉せず、`Select` は dialog を閉じない（cmdk のコマンドパレット意味論、実行フックは UI コンポーネント層の範囲外）。`filter_items` は `combobox::filter_options` へ全委譲する。shadcn/ui のみに存在する部品（参照軸 #2001） |
