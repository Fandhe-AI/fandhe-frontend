...
| Button Group（[Primitives](../../site/primitives/button-group.md)、イシュー #2059。Themes ページは #2060 で追加予定） | `button_group` | Root/Separator/Text | なし（自由関数のみ。`role="group"` の静的なグループ化で `toolbar` の roving tabindex 状態機械とは異なる。`role="group"` へ `aria-orientation` は付与せず `data-orientation` のみで向きを表現。参照実体は shadcn/ui のみ〔参照軸 #2001〕。呼び出し側 `attrs` の予約キー除去あり） |
