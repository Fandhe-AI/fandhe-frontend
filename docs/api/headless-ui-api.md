...
| [Input Group](../../site/primitives/input-group.md)（Themes ページは #2063 で登録予定。リンク先が暫定的に `/primitives/` を指す） | `input_group` | Root/Addon/Text/Button | なし（`field`/`fieldset` と同じ判断で `disabled`/`invalid` は呼び出し側が決める SSR 静的な props。実際の `<input>`/`<textarea>` は本モジュールが出力せず `field::input`/`field::textarea` を呼び出し側が合成する契約。`merge_field_props` は `disabled`・`invalid` の双方を OR 伝播（「1 group = 1 control」の構造のため `invalid` も伝播する点が `fieldset` と異なる）。イシュー #2062） |
