...
| data-list | ✓（`Size` 5 段、既定 `Md`） | 提供しない | `orientation`（`Vertical`/`Horizontal`）・`DataListVariant`（`Subtle`/`Bold`）の追加軸を持つ（イシュー #1559） |
| toast | ✓（`placement`、`group` slot） | ✓（`status`、`root` slot、`ColorPalette` 6 役割淡色面配色、イシュー #1544。`alert` と宣言の中身は共有しない） | 各軸が別 slot のため `variant_class` をスロットごとに個別呼び出し |
| tour | 提供しない | ✓（`root` slot） | `size` は overlay 系の寸法を呼び出し側の CSS カスタムプロパティ上書きに委ねるため初版非提供。`palette` は `action-trigger` の背景・文字色（`--fandhe-palette-fg`）・両トリガーのフォーカスリング・スポットライト縁取りの強調色に反映（イシュー #1551 で `action-trigger` の文字色反映を追加） |
