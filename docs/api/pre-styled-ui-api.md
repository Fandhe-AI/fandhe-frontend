...
| headless ラッパー | `timer`（`clipboard` と同型の判断で variant は非提供。#1577 で ark-ui 基準へ調整済み（`root` 中央揃え・`action-trigger` の outline ボタン化とホバー/トランジション付与）。`item-value` に `font-variant-numeric: tabular-nums` を付与し桁の増減時のレイアウトシフトを防ぐ。`completed` 状態の切り替えは `root` の `[data-state="completed"]` が custom property 経由で `item-value` の色を強調色へ切り替える（headless が `data-state` を root にしか出さないため）。`action-trigger` に focus-visible リングを付与する。実 tick 駆動（`setInterval`）は `fandhe-frontend-wasm-full::headless_timer` が提供する） | [timer](../../site/themes/timer.md) |
