...
| headless ラッパー | `tree_view`（`popover`/`tooltip` と同型の判断で `size`/`color-palette` のいずれも非提供。branch のインデントは CSS custom property（`--fandhe-tree-view-indent`）で表現し、DOM ネストにより深さ分が自然に累積する。密度調整用の `--fandhe-tree-view-row-padding-block`/`-inline`・`--fandhe-tree-view-icon-size` custom property を提供（イシュー #1578）。`branch-indicator` が子を持たない（`:empty`、`StateCondition::Empty`）場合のみ border 2 本でシェブロンを描画し、独自グリフを渡す消費者と二重表示しない） | [tree-view](../../site/themes/tree-view.md) |
