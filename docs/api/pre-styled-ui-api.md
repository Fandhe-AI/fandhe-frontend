...
（#738）・RatingGroup styled ラッパー追加（#742、いずれも公開時点未反映）を
経て 22 の公開モジュールを持つ。内訳は次の通り。
...
| headless ラッパー第 7 弾 | `rating_group` | #742（`size`/`color-palette` 両軸、星形 indicator は `clip-path` インライン表現） |
