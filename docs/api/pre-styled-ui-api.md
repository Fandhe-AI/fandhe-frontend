...
| 状態機械を持たない静的表示部品 | `table` / `data_list`（`card` と同型。headless-ui 側に対応する anatomy を持たず本クレートで新規 anatomy `table`/`data-list` を定義する。`table` は `variant`（`Line`/`Outline`）/`size`（`Xs`〜`Xl` の 5 段）/`striped`/`sticky_header` の 4 軸 variant を持ち、striped は新設の `StateCondition::NthChildEven` で表現、`sticky_header`（イシュー #1571）は `column-header` を `position: sticky` にする。`Outline` は内側の行罫線・muted なヘッダー背景・最終行罫線なし・`tfoot` 上罫線を持つ（イシュー #1572）。`scroll_area`（`div`、イシュー #1572）で `chakra-ui Table.ScrollArea` 相当のスクロール枠を提供し、`sticky_header` と組み合わせられる。`data_list` は `orientation`（`Vertical`/`Horizontal`）/`variant`（`Subtle`/`Bold`）/`size`（`Size` 5 段、既定 `Md`）の 3 軸を持つ、イシュー #1559） | [table](../../site/themes/table.md) / [data-list](../../site/themes/data-list.md) |
...
| table | ✓（`Size` 5 段、既定 `Md`） | 提供しない | 選択・チェック状態を示す部品ではないため color-palette は非提供。`TableVariant`（`Line`/`Outline`）・`striped`（`bool`）・`sticky_header`（`bool`）の追加軸を持つ。striped は `StateCondition::NthChildEven` で表現。`Outline` は内側行罫線・muted ヘッダー背景・最終行罫線なし・`tfoot` 上罫線を持つ（イシュー #1572） |
...
| card | ✓ | 提供しない | 中立コンテナのため palette 非提供（イシュー #606）。size（xs〜xl、既定 md）は root の `--fandhe-card-*` へ一本化し padding/角丸/title の文字サイズへ連動する。size 軸新設に伴い `root` の第 1 引数を `impl Into<CardProps>` へ拡張した（イシュー #1557）。`From<CardVariant> for CardProps` により従来の `root(CardVariant::Elevated, …)` はそのまま有効で、size を指定する場合は `root(CardProps { variant, size }, …)` を渡す（後方互換の純追加） |
