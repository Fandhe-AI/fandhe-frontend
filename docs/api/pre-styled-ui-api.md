...
PieChart/DonutChart styled 静的部品追加（#850、charts 基盤（#846）を用いた
初のチャート部品。`docs/policy/intentional-non-adoption.md` §7 の
chakra-ui charts 保留を pie-chart/donut-chart の 2 件で解除。いずれも
公開時点未反映）を経て 86 の公開モジュール + `charts` サブモジュール群を
持つ。内訳は次の通り。
