...
| headless ラッパー | `link` / `link_overlay` / `nav_list`（状態機械なし。`link_overlay` は `::before` 疑似要素の代わりに `overlay` 自身を `position: absolute; inset: 0;` で展開する。イシュー #1580 で `overlay` に `:focus-visible` の共通フォーカスリング（`border-radius: inherit` で `root` の角丸に追従）を追加。`nav_list` は `fandhe-frontend-docs-site::nav.rs::sidebar` が直接使う想定のため、`root` 以外（`heading`/`list`/`item`/`link`）は headless 自由関数をそのまま選択的に再エクスポートする） | [link](../../site/themes/link.md) / [link-overlay](../../site/themes/link-overlay.md) / [nav-list](../../site/themes/nav-list.md) |
