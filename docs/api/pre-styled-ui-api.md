...
| 単純 styled 部品 | `marquee`（ark-ui の `Root`/`Viewport`/`Content`/`Item`/`Edge` anatomy を `root`/`content`/`item` の 3 パーツへ縮約（`Viewport` は `root` が兼ね、`Edge` は root への `mask-image` variant（`MarqueeEdge::Fade`、イシュー #1582）で代替）。`content` を内部で 2 回複製しシームレスループを実現し、2 個目は常時 `aria-hidden`。`direction`（`Start`/`End`）・`edge`（`None`/`Fade`）の 2 軸 variant を root へ付与し `content` への伝搬は `--fandhe-marquee-direction` custom property の継承で行う。`color-palette`/`size` 軸は非提供。速度・間隔・delay・loop-count・両端フェード幅はいずれも `--fandhe-marquee-duration`/`-delay`/`-loop-count`/`-gap`/`-edge-size` の CSS custom property で調整する契約。CSS のみ（JS ゼロ）・`prefers-reduced-motion: reduce` でのアニメーション停止・`hover`/`focus-within` での常時一時停止という決定的設計） | [marquee](../../site/themes/marquee.md) |
