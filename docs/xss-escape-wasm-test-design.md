...
本書は `docs/plain-html-output-test-design.md`（TASK-5.2a）・
`docs/wasm-full-architecture.md`（TASK-11.2a）・`docs/hydration-state-format.md`
...
- **根拠**:
  ...
  2. `wasm-client/`（`rws-wasm-client`）クレートの新設は TASK-6.2b（#48）の
     スコープである。`docs/browser-testing.md` 第 2 節が同様の判断
...
- **承認根拠**: PoC-5（`docs/spec/03-poc/`）が実機テストで実績のある構成であり、
  `docs/browser-testing.md` 第 3 節が `wasm-pack test --headless --chrome`
...
| `wasm-client/` クレート経由のハイドレーション実ブラウザ検証 | TASK-6.2b（#48）・TASK-6.3b（#66）。`docs/browser-testing.md` 第 7 節の引き継ぎ表に「WASM 経路 XSS テストの本環境への統合」として既に記載済み |
| `wasm-thin`（オプトイン薄い JS グルー方式）経路の XSS 検証 | v1 スコープ外。`docs/opt-in-thin-js-glue.md` が既に制約を明記済み。本タスクでは新規 Issue 化を提案しない（既存文書での担保で十分と判断） |
