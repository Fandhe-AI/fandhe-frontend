...
本書は `docs/wasm-full-architecture.md`（TASK-11.2a）・`docs/hydration-state-format.md`
（TASK-11.4a）・`docs/xss-escape-wasm-test-design.md`（TASK-1.3a）と同じ書式
...
- CI: `.github/workflows/ci.yml` は `RUSTFLAGS='-F unsafe_code' cargo check --workspace`
  ...
  `MAX_DEPTH = 6` が正）を強制。運用ポリシーは `docs/dependency-graph-policy.md`。
...
| 参考: PoC-3 の `rws-server`（axum+tokio、既存実測 52/5 の再検証） | axum + tokio（PoC-3 と同一） | 50 | **9** | 既存の「52 件/深さ 5」という PoC-3 実測（`docs/dependency-graph-policy.md` 第 2 節）と**本書の再計測（同一アルゴリズム）は不一致** |
...
`docs/dependency-graph-policy.md` の「採用上限」根拠は PoC-3 実測（52 件/深さ **5**、
...
全体としての依存グラフ上限運用（REQ-3・`docs/dependency-graph-policy.md`）は
...
`MAX_DEPTH` 値・`docs/dependency-graph-policy.md` の再検証）は本タスク
（docs-only の設計確定）の範囲では行えない（`MAX_DEPTH` は tooling-builder
領域の定数変更・ユーザー承認事項、`docs/dependency-graph-policy.md` の確定は
...
- **TASK-6.1c（#44、rws-server SSR/SSG エントリ）との関係**: #44 は `rws-server`
  ...
  依存グラフ測定（`docs/dependency-graph-policy.md` 第 2 節「`rws-server` は
...
- **【最優先で Issue 化を提案】`MAX_DEPTH` 算出根拠（PoC-3 `cargo tree` 目視:
  ...
  そのまま製品化しようとすると必ず本齟齬に突き当たる。`docs/dependency-graph-policy.md`
...
- 4.2 節の実測は scratchpad 上の試作クレート（workspace 外）によるものであり、
  ...
  実測での再確認を必須の完了条件とする。
- 4.3 節の計測基準齟齬は本タスクの発見事項であり、`docs/dependency-graph-policy.md`
