...
扱う原則。2026-09-07 に #2153 で主基準の 1 つへ改訂されたが、golden 純追加
原則により既存 55 色トークンの優先は改訂後も有効）に従い、既存 55 色
トークン（本書 §3）を優先し、shadcn 側の命名を機械的に輸入しない。`https://ui.shadcn.com/docs/theming` の実測値
