...
> `docs/design/impact-analysis-design.md` / TASK-13.3a `docs/design/gate-design.md` と
> 同型の「設計文書 + スケルトン」構成）。
...
は実測時点のスキーマで保存されており、製品スキーマ（`docs/design/impact-analysis-design.md`
...
- `fw impact <symbol> --project <dir>` の JSON
  （`docs/design/impact-analysis-design.md` §3.5 スキーマ）に対し
...
- 影響ルート判定の AST 精密化は `docs/design/impact-analysis-design.md` §7 で
...
- シナリオ 1〜3 の回帰テスト本体の実装 → #145 / #146 / #147（既存
  イシューで追跡済み、新規起票不要）
- 影響ルート判定の AST 精密化 →
  `docs/design/impact-analysis-design.md` §7 で将来スコープとして文書化済み
  （新規起票不要）
