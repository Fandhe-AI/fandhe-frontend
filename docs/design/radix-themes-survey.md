...
| breakpoints | `initial`/`xs`/`sm`/`md`/`lg`/`xl`（6 段、§4.5） | `DEFAULT_BREAKPOINTS`（theme.rs、イシュー #2197 で新設）4 段: `sm`(640px)/`md`(768px)/`lg`(1024px)/`xl`(1280px)。`crates/pre-styled-ui/src/recipe.rs` の `Breakpoint` enum・`SlotRecipe::breakpoint` builder による `@media (min-width: ...)` 条件と対で導入（4 段トークン化済み、#2197） | Radix は `initial`/`xs` を含む 6 段の名前付き値（値も 1 段ずれる: Radix `sm`=768px、fandhe `sm`=640px）。fandhe は shadcn/ui（Tailwind v4 既定）・chakra-ui v3 と `sm` 以外で完全一致する 4 段を採用し、Radix の名前付きスケールは不採用（`2xl`/`xs`/`initial` も見送り。判断根拠は `docs/design/pre-styled-ui-scale-tokens.md` §3.6） |
