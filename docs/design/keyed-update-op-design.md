...
| c. **浅い in-place 更新（深さ 1 打ち切り）** | アイテムルート要素の同一性（DOM ノード参照）を維持したまま、(1) ルート要素の属性差分を `setAttribute`/`removeAttribute` で適用し、(2) 子ノード列は新旧で不一致なら子ノード列全体を破棄・`build_dom_node` 相当のプログラム的構築で再構築する | **採用** |
...
- **ルート属性の差分適用**: 旧ノードの属性集合と新ノードの属性集合を比較し、
  ...
  （検証規則を複製せず共有する）。
- **子ノード列の再構築**: 子ノード列（テキスト・要素混在を含む）が新旧で
  一致しない場合、既存の子ノードをすべて削除してから
  `build_dom_node` によるプログラム的構築（`create_element` →
  `set_attribute` → `set_text_content` → `append_child`）で再構築する。
  テキストのみの変更（例: `<span>1</span>` → `<span>2</span>`）は
  結果として `set_text_content` 相当の 1 回の DOM 操作に帰着する。子要素の
  内部に HTML パーサを一切経由しない（更新設計書 §9 不変条件 1 の延長）。
...
- `Update` の発行位置は、既存の 2 パス（Remove パス → Insert/Move パス）
  ...
  対して、位置が変わらず（`Move` が発行されなかった）かつ新旧両方の
  キー列に存在するキーそれぞれについて、対応する旧アイテム `Node` と
  新アイテム `Node` を比較し、不一致であれば `KeyedOp::Update` を発行する。
  発行順序はキーの新しい並び順（`new_keys` の順序）に固定する。
- `Move` されたアイテムについても内容変更が同時に起きうる（キーは同じだが
  位置も内容も変わるケース）。この場合は `Move` 発行に加えて同一キーへの
  `Update` も発行する（1 キーに対し `Move` と `Update` の両方が起きうる
  ことを許容する。位置決定と内容適用は独立した関心であり、`Move` 1 種類に
  内容更新の意味を混在させない）。
