...
- **ルート属性の差分適用（detached 複製での事前検証、イシュー #1330
  レビュー指摘 P0 を受けて確定）**: 旧ノードの属性集合と新ノードの属性
  集合を比較し、削除属性・追加変更属性からなる操作列（`removeAttribute`
  / `setAttribute` の呼び出し列）を算出する。`data-key` /
  ...
  適用する（検証規則を複製せず共有する）。この操作列は、**ライブ root
  要素へ適用する前に**、`cloneNode(false)`（子ノードを含まない浅い複製）
  で得た detached な複製要素へ**まず全操作を適用して検証する**。複製への
  適用がいずれか 1 件でも例外（不正な属性名等）で失敗した場合は複製を
  破棄し、ライブ root 要素には一切触れず、当該アイテムの Update 適用全体
  （後述の子ノード再構築を含む）を諦める。複製要素は元の root 要素と同一
  タグ・同一 document コンテキストを持ち、`setAttribute`/`removeAttribute`
  の成否は入力（属性名・属性値）のみに依存し要素の DOM 接続状態には
  依存しないため、複製での適用成功はライブ root 要素へ同一操作列を適用
  した場合の成功を保証する。この事前検証により、ライブ root 要素への
  属性適用は「複製で確認済みの操作列を再生するだけ」となり、ライブ DOM
  上で操作が部分的にしか成功しない状態は構造的に発生しない（旧属性値の
  保存・失敗時ロールバックという追加の状態管理を持ち込まずに済む）。
- **子ノード列の再構築（detached 構築 → 検証 → 交換の順序、イシュー #1330
  ...
  確認より先に行わない）。
  **子ノード列の detached 構築とルート属性の複製検証（上記）が両方とも
  成功した場合にのみ**、ライブ DOM への変更（コミットフェーズ）へ進む。
  コミットフェーズは (1) 検証済みの属性操作列をライブ root 要素へ適用
  （複製での検証成功により失敗しないことが保証されている）、(2) 続けて
  既存の子ノードをすべて削除して detached コンテナの子ノード列を
  `append_child` で移し替える、の順で行う（構築と検証が完了した内容だけ
  を旧 DOM と交換する。「削除してから構築」ではなく「構築・検証してから
  削除して交換」の順序を常に守る）。子ノード構築の検証と属性操作の複製
  検証のいずれか一方でも失敗した場合は、ライブ DOM には一切触れず
  （detached コンテナ・複製要素をそれぞれ破棄するのみ）、当該アイテムの
  Update 適用全体を諦めて他アイテムへの適用へ進む（§6 不変条件 6 の
  fail-closed 契約）。テキストのみの変更（例: `<span>1</span>` →
...
6. **fail-closed（属性適用・子ノード再構築いずれも部分適用を許さない、
   イシュー #1330 レビュー指摘 P0 を受けて確定）**: 新旧 `Node` の比較・
   属性適用に失敗した場合（想定外の属性名・DOM API 呼び出し失敗等）は
   当該アイテムの Update 適用を諦め、他アイテムへの適用を妨げない
   （更新設計書 §9 不変条件 6 と同じ設計、`unwrap()`/`panic!` を使わない）。
   ライブ DOM への変更は §3.2 の 2 つの事前検証（(a) ルート属性の操作列を
   `cloneNode(false)` で得た detached 複製要素へ適用して検証する、
   (b) 子ノード列を detached コンテナ上で `build_dom_node` により構築して
   検証する）の**両方**が完了した後のコミットフェーズでのみ行う。
   いずれか一方でも検証に失敗した場合はライブ root 要素・既存子ノードの
   いずれにも一切変更を加えず（detached の複製要素・コンテナを破棄する
   のみ）、Update 適用全体を諦める。これにより、属性適用の途中（複数回の
   `setAttribute`/`removeAttribute` 呼び出しの一部のみ成功した状態）で
   終了することも、子ノード交換の途中で終了することも構造的に発生しない
   （旧 DOM を消してから作り直す実装・ライブ要素へ検証なしで逐次属性適用
   する実装はいずれも許容しない）。
...
- **wasm-client（#1324）**:
  ...
  - Update 実測のオーバーヘッドが既存操作（#1315 実測 1.32ms 相当）の
    ...
    経由）。
  - 属性適用が複数件（削除・追加・変更混在）となるケースで、いずれか
    1 件でも検証失敗を注入した場合にライブ root 要素の属性が一切変更
    されず（1 件も適用されない）、既存の子ノードも保たれることを確認する
    テスト（§6 不変条件 6 の atomicity 契約の固定）。
