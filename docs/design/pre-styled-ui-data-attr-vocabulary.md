...
| `data-tone`（marker.rs 出力分） | `crates/pre-styled-ui/src/marker.rs`（イシュー #2115） | headless-sourced（`neutral`/`info`/`warning`/`danger` の 4 値、`fandhe-frontend-pre-styled-ui` `recipe::ColorPalette` の同名 4 値の部分集合） | headless `marker.rs::root` が固定出力する（headless-sourced、役割 B。本リポジトリ初出の属性名、イシュー #2114） | `crates/pre-styled-ui/src/marker.rs::recipe()` が `StateCondition::AttrEq` で**参照のみ**する（自前で出力しない） | headless-sourced 語彙として維持（§2.2 役割 B、`crate::attachment` の `data-variant`/`data-state`/`data-disabled` と同型） |
