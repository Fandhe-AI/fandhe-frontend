...
| `data-state`（sidebar 語彙、`"expanded"`/`"collapsed"`） | `sidebar.rs`（イシュー #2073、`root`/`rail`/`trigger` slot への state 規則。他部品の `data-state`〔`open`/`closed` 等〕とは値域が異なる） | headless `sidebar.rs`（`SidebarState::as_data_state` が固定出力） |
