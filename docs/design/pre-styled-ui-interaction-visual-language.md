...
3. **disabled**: `Attr("data-disabled")` + `disabled_declarations()` を登録する。`:disabled` ネイティブ擬似クラスや独自の `opacity`/`cursor` 値は使わない。`steps.rs` は `Attr("disabled")` → `Attr("data-disabled")` への語彙統一も行う。**補足（イシュー #2195、opacity 単一階層）**: `disabled_declarations()`（opacity 0.5 + cursor）を適用してよいのは部品ごとに opacity を所有する 1 階層（葉パーツ or ルートコンテナのどちらか）のみである。祖先/子孫の別スロットが同時に `data-disabled` を持つ場合、opacity 所有スロット以外は `cursor: not-allowed` のみを登録し `disabled_declarations()` は使わない（重ねると実効 opacity が二乗され過減光になる、`tags_input.rs::control` が先例）。所有スロットの判定・部品別対応表の正は `docs/design/pre-styled-ui-forms-disabled-required-matrix.md` を参照する（本節はレビュー時の「新規 state 追加は §5 違反」という誤検知を避けるための補足であり、既存規則を変更しない）
4. **transition**: 既存リテラル duration をトークン参照へ置換する。写像表: `0.15s` → `MotionDuration::Fast`、`0.2s`（`ease` 有無問わず）→ `MotionDuration::Normal`、`0.3s` → `MotionDuration::Slow`。未導入モジュールは `transition_declarations` を base へ追加する。**対象プロパティの選定基準（イシュー #2215 で明文化）**: `transition_declarations` の `properties` には、当該 slot の状態（hover / active / selected / disabled 等）間で実際に値が変わるプロパティを列挙する。状態で `box-shadow` が変化する slot は `box-shadow` を含める（`button` root が参照実装、`calendar`/`slider`/`splitter`/`angle_slider`/`image_cropper`/`message_scroller`/`color_picker` が先例）。値が変化しない slot・variant（例: `tabs` の `TabsVariant::Line`）では対象プロパティに含めても computed style・アニメーションは変化しない（golden のテキストバイトのみ変わる純追加）。tabs の Enclosed（#2039 で選択中 trigger へ `box-shadow` を追加、#2215 で transition 対象へ追加）がこの基準を明文化する契機になった
