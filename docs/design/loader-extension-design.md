...
| 2 | 判断は `docs/policy/intentional-non-adoption.md` の記録書式（4 評価軸: 明示性・決定性・機械検証可能性・コンテキスト消費）に従って記録され、採用と判断された項目があれば実装 Issue に分解される | `intentional-non-adoption.md` §3.5〜3.7 に本書と対応する記録を追加（本コミットで追記）。本判断では採用相当の項目なし（合成は既存 API のまま運用のため実装 Issue 化不要） |
