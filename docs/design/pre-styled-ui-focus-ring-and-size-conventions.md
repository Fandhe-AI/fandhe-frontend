...
| 既定宣言（canonical） | `outline: var(--fandhe-focus-ring-width, 2px) solid var(--fandhe-color-focus-ring, var(--fandhe-color-accent)); outline-offset: var(--fandhe-focus-ring-offset, 2px);` | リテラル値をハードコードせず、太さ・オフセット・色をテーマ側 1 箇所で変更できる。各 `var()` は本イシュー以前の実際の既定値（`2px`・本イシュー以前から存在する `--fandhe-color-accent`）を第 2 引数へフォールバックとして持つ。`Theme::empty()` ベースの既存カスタムテーマは本イシューが新設したトークン（`--fandhe-focus-ring-width`/`--fandhe-focus-ring-offset`/`--fandhe-color-focus-ring`）を定義していないため、フォールバックがないと `outline` 宣言全体が computed-value time に無効となりキーボードフォーカス表示が消え、0.42.0 を「破壊的変更ではない純追加」とする契約と矛盾する（イシュー #1424 レビュー指摘対応。実装は `recipe::focus_ring_declarations` に一元化） |
