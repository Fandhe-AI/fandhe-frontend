...
| 定数 | 値 | 超過時の扱い |
|---|---|---|
| `MAX_PAGE_TEXT_BYTES` | 3,968 バイト（イシュー #2849 で 4,096 から 4,032 へ、イシュー #2862 で 4,032 から 3,968 へ暫定引き下げ、§10-10・§10-12 参照。`MAX_INDEX_BYTES` のハードルール〔§10-5・§10-6〕に抵触せず超過分を吸収する最小限の暫定調整。イシュー #2851 の base 取り込み時点では実測が当時の範囲内に収まり追加の引き下げは不要だった、§10-11 参照） | **決定的に切り詰める**（エラーにしない）。UTF-8 文字境界で切る（`char_indices` で境界を求め、バイト単位切断で不正 UTF-8 を作らない）。切り詰め痕跡の付加文字（`…` 等）は付けない（決定性と単純さを優先する） |
...
### 10-12 イシュー #2862（`pricing-comparison-table`）で再度ハードルールに抵触し、`MAX_PAGE_TEXT_BYTES` を再引き下げて対処
...
5,632 バイト）で FAIL した。§10-10 の指摘（「次回発生時、本定数の
再引き下げを機械的に繰り返さない」）を踏まえ恒久対処（イシュー #3173、
セクション粒度インデックスへの分割）への着手を検討したが、本イシュー
単体のスコープ（Blocks への 1 block 追加）に対して不釣り合いに大きいと
判断し、§10-10 と同型の緊急避難（`MAX_PAGE_TEXT_BYTES` の再引き下げ）を
今回も採った。イシュー #3173 は引き続き優先度を上げて着手することを
推奨する。

- **`MAX_INDEX_BYTES`（`1_703_936`）は変更しない**（§10-6 のハードルールを遵守）。
- `MAX_PAGE_TEXT_BYTES` を `4_032` → `3_968`（−64 バイト、#2849 と同じ
  刻み幅）へ引き下げた。実測（一時的に `MAX_INDEX_BYTES` を極小値へ
  差し替えて `TooLarge` のエラーメッセージから索引全体バイト数を測る
  手法、§10-10 と同型）で、引き下げ後の索引 JSON 全体は
  1,691,830 バイト（`MAX_INDEX_BYTES` の約 99.3%）まで縮小し、必要な
