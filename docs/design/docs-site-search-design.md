...
### 10-13 イシュー #2865（`pricing-seats-split`）で再度ハードルールに抵触し、`MAX_PAGE_TEXT_BYTES` を再度引き下げて対処

イシュー #2865（Blocks に `pricing-seats-split`〔座席数入力付きプラン、
区分 Marketing・カテゴリ Pricing〕を追加）で
`cargo test -p fandhe-frontend-docs-site` が
`SearchIndex(TooLarge { bytes: 1704017, limit: 1703936 })` で FAIL した
（§10-12 の引き下げ後、余裕約 0.3% だった枠を 1 block の追加で使い切った）。

#### 実測

- 超過量: 81 バイト
- `MAX_PAGE_TEXT_BYTES` を `4_000` → `3_968`（−32 バイト）へ引き下げた後の
  索引 JSON 全体バイト: 1,695,144（`MAX_INDEX_BYTES` の約 99.5%）

#### 決定

- `MAX_INDEX_BYTES`（1.625 MiB = `1_703_936`）は変更しない（§10-6 の
  ハードルールを遵守）。
- §10-10・§10-12 と同じ理由により、`MAX_PAGE_TEXT_BYTES` をさらに引き下げる
  対処を継続した。
- **余裕はほぼ枯渇している（約 0.5%）**。§10-12 の推奨を改めて繰り返す:
  次回以降の Blocks 追加（親トラッキング #2730 は継続中）で本定数の
  再引き下げでは吸収しきれない超過が発生する可能性が高く、恒久対処
  （イシュー #3173、セクション粒度インデックスへの分割）の優先度を
  上げて着手することを推奨する。
