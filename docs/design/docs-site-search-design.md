...
- 新規モジュール `crates/docs-site/src/search_index.rs`（`lib.rs` に
  ...
  - `pub const MAX_INDEX_BYTES: usize = 1_441_792;`（#2552 で `1_048_576` から `1_179_648` へ、#2645 で `1_179_648` から `1_310_720` へ、イシュー #2817 で `1_310_720` から引き上げ、§10/§10-4 参照。イシュー #2637・PR #2702（イシュー #2639 Breadcrumbs）の base 取り込み時点でも実測は範囲内）
...
### 10-4 イシュー #2817（careers-split-photo-list）`MAX_INDEX_BYTES` 引き上げ実装記録

Blocks セクションの拡充（イシュー #2730「目的別パーツ拡充」配下、複数の
並行 PR）が進む中、イシュー #2817（`careers-split-photo-list` block
追加）の PR が base（main）取り込み後に `cargo test -p
fandhe-frontend-docs-site` を実行したところ、実サイトの検索インデックス
JSON が 1,313,136 バイトに達し、10-1 で設定した上限（1.25 MiB =
`1_310_720` バイト）を約 2,416 バイト超過して
`SearchIndexError::TooLarge` が発火した。
...
- 10-1 の実測（1,180,096 バイト、新上限比 90.0%）の時点で既に §8
  ...
  不釣り合いに大きく、緊急のビルド復旧が優先される。そのため本 PR でも
...
- 引き上げ幅は §10/§10-1 と同じ +131,072 バイト刻みを踏襲し、1.375 MiB
  ...
  （1,313,136 バイト）は新上限の約 91.1%（1,313,136 / 1,441,792）であり、
  ...
  明記する）。
- **並行 PR との重複に関する注記**: 同じ #2730 配下の並行イシュー #2816
  （`careers-split-accordion`）も独立に同一の超過（実測 1,313,087 バイト）
  へ到達し、同一の目標値（`1_441_792`）へ引き上げる変更を別 PR で行って
  いる（本節と同型の §10-4 記録を持つ）。いずれか一方が先にマージされた
  場合、後発 PR は本節・`search_index.rs` の定数変更が重複コンフリクトに
  なる（値は同一のため解消は容易）。マージ担当は片方の変更のみを採用し、
  節番号の重複（両 PR とも「§10-4」を名乗る）は後発側を §10-5 へ繰り下げる
  形で解消すること。
