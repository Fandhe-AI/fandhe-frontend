...
| `MAX_INDEX_BYTES` | 1,703,936 バイト（1.625 MiB。#2552 で 1 MiB から 1.125 MiB へ、#2645 で 1.125 MiB から 1.25 MiB へ、イシュー #2814 で 1.25 MiB から 1.625 MiB へ引き上げ、§10 参照。イシュー #2637（Menu）・#2639（Breadcrumbs）の base 取り込み時点では実測が 1.25 MiB の範囲内に収まっており追加の引き上げは不要だったが、イシュー #2814（`blog-split-header-grid` block 追加）の base 取り込み時点で 1.25 MiB を超過し引き上げた、§10-4 参照。イシュー #2751（`content-article` block 追加）の base 取り込み時点では実測は 1.625 MiB の範囲内に収まっており、追加の引き上げは不要だった、§10-5 参照） | **fail-closed**。`BuildError::SearchIndexTooLarge { bytes, limit }` を返し、**ページ書き出し前**に打ち切る |
...
- 新規モジュール `crates/docs-site/src/search_index.rs`（`lib.rs` に
  ...
  - `pub const MAX_INDEX_BYTES: usize = 1_703_936;`（#2552 で `1_048_576` から `1_179_648` へ、#2645 で `1_179_648` から `1_310_720` へ、イシュー #2814 で `1_310_720` から `1_703_936` へ引き上げ、§10 参照。イシュー #2637・#2639 の base 取り込み時点でも実測は範囲内だったが、#2814 で超過し引き上げた。#2751 の base 取り込み時点では実測は範囲内であり追加の引き上げは不要だった）
...
### 10-5 イシュー #2751（`content-article`）base 取り込み時点の実測確認

イシュー #2751（Blocks に `content-article`〔1 列の記事本文、区分
Marketing・カテゴリ Content の最初の block〕を追加）を main（10-4 時点で
`MAX_INDEX_BYTES` は 1.625 MiB = `1_703_936`）へ取り込んだ時点で
`cargo test -p fandhe-frontend-docs-site` を実行し実サイトの検索
インデックスサイズを再測した。実測は 1.625 MiB の範囲内（80% 未満）に
収まっており §8 トリガー 1 の再評価基準に到達しなかったため、本 PR
では `MAX_INDEX_BYTES` の追加引き上げを行わない。
...
- **本節を追加した経緯**: 本イシューはもともと base 取り込み前の実装
  ...
  イシュー #2814 が同じ 1.25 MiB 上限を別途超過し、§10-4 の評価
  （固定 +131,072 バイト刻みを踏襲せず、実測が新上限の 80% 未満に
  収まる値として 1.625 MiB へ引き上げ）を経て `1_703_936` へ引き上げ
  済みだった。本 PR を base 取り込みした結果、両者は同じ定数を競合
  して変更する形になったため、§10-1 の再評価契約に従い改めて実測を
  取り直した。本イシュー単独の実測（1,313,125 バイト）は §10-4 の
  引き上げ後上限（1,703,936 バイト）に対し約 77.1% であり、本 PR
  自身の変更を base 取り込み後に再計測しても新たな超過は生じない
  （§10-4 が既に本イシュー相当の増分を見込んだ値まで引き上げ済み
  のため）。したがって本 PR では `MAX_INDEX_BYTES` を独自に変更せず、
  §10-4 の引き上げ結果をそのまま採用する。
