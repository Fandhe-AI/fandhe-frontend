...
| 定数 | 値 | 超過時の扱い |
|---|---|---|
| `MAX_PAGE_TEXT_BYTES` | 4,064 バイト（イシュー #2851 で 4,096 から 4,064 へ暫定引き下げ、§10-10 参照。`MAX_INDEX_BYTES` のハードルール〔§10-5・§10-6〕に抵触せず超過分を吸収する最小限の暫定調整） | **決定的に切り詰める**（エラーにしない）。UTF-8 文字境界で切る（`char_indices` で境界を求め、バイト単位切断で不正 UTF-8 を作らない）。切り詰め痕跡の付加文字（`…` 等）は付けない（決定性と単純さを優先する） |
...
- 新規モジュール `crates/docs-site/src/search_index.rs`（`lib.rs` に
  ...
  - `pub const MAX_PAGE_TEXT_BYTES: usize = 4064;`（イシュー #2851 で `4096` から `4064` へ暫定引き下げ、§10-10 参照）
...
### 10-10 イシュー #2851（`footer-link-columns`）の超過と `MAX_PAGE_TEXT_BYTES` 暫定引き下げ

イシュー #2851（Blocks に `footer-link-columns`〔リンクカラム型 footer、
区分 Marketing・カテゴリ Footer の 3 件目〕を追加）の PR で
`cargo test -p fandhe-frontend-docs-site` が
`SearchIndex(TooLarge { bytes: 1705980, limit: 1703936 })` で FAIL し、
`MAX_INDEX_BYTES`（1.625 MiB = `1_703_936`）を 2,044 バイト超過した。

`MAX_INDEX_BYTES` の追加引き上げは §10-5・§10-6 のハードルールにより
選択肢から外れる。本 block の Rust コードフェンス（3 インスタンス
構成、集約元 7 件の差分統合）を圧縮して超過分を吸収する対処も検討した
が、`crates/docs-site/src/search_index.rs::page_entry` は本文全体を
`MAX_PAGE_TEXT_BYTES`（4,096 バイト）で決定的に切り詰めるため、本 block
のように切り詰め済みのページは本文を多少削っても切り詰め後の `text`
バイト数（4,096 バイトのまま）が変わらず、索引サイズの削減に**寄与しない**
（切り詰め後サイズを実際に下げるには、コードフェンスをごく短い断片へ
書き換える必要があり、実装コードの正確な提示という block の目的を
損なう）。したがって本 PR に閉じた本文圧縮では超過を解消できない。

代わりに、`MAX_PAGE_TEXT_BYTES` を 4,096 から 4,064 バイトへ 32 バイト
引き下げる暫定対処を採った。per-page 上限に張り付いている（切り詰め
済みの）ページが実サイトで約 260 件あり、削減量は最大でも
260 × 32 ≒ 8.3 KiB で、本 PR の超過分（2,044 バイト）を十分な余裕を
持って吸収できる。1 ページあたり 32 バイト（4,096 バイト中 0.8%）の
削減は個々のページの検索精度への影響が無視できる水準であり、§10-5 で
「削減量が乏しく恒久対処にならない」と判断した per-page 上限の**大幅**
引き下げ（4,096 → 3,072、-25%）とは規模が異なる。本節の変更は
恒久対処ではなく、セクション粒度インデックスへの分割（イシュー #3173）
までの最小限の暫定調整であり、`MAX_INDEX_BYTES` のハードルール（§10-5・
§10-6）を回避する意図はない。次回 `SearchIndexError::TooLarge` が
発生した場合も、`MAX_INDEX_BYTES`・`MAX_PAGE_TEXT_BYTES` いずれの
機械的な繰り返し引き下げ・引き上げにも頼らず、イシュー #3173 の実装を
優先して検討すること。
