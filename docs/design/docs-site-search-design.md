...
### 10-11 イシュー #2858（`header-mega-menu`）と #2855（`header-flyout-menu`）の並行合流で再発、`MAX_PAGE_TEXT_BYTES` を再度引き下げて対処

イシュー #2858（Blocks に `header-mega-menu` block を追加する PR）が
...
- **`MAX_INDEX_BYTES`（1.625 MiB = `1_703_936`）は変更しない**（§10-6 の
  ハードルールを遵守）。
- `MAX_PAGE_TEXT_BYTES` を `4_032` → `3_968`（−64 バイト、#2849 実施時と
  同じ引き下げ幅）へ再度引き下げた。索引 JSON 全体で 1,704,988 →
  1,687,354 バイト（−17,634 バイト、必要な 1,052 バイトに対し約 16.8 倍の
  安全余裕）まで縮小した。
- **#2849 §10-10 の「本定数の再引き下げを機械的に繰り返さない」という
  教訓との整合**: 本対処は同じ機構を再度使ってはいるが、恒久対処（選択肢 3、
  セクション粒度インデックスへの分割、追跡: イシュー #3173）の実装は
  本イシュー（レビュー指摘対応）のスコープを大きく超えるため、依然として
  緊急避難としてのみ適用する。索引全体は引き下げ後も `MAX_INDEX_BYTES` の
  約 99.0% に達しており、§8 トリガー 1（80% 超過）を大きく超えたままである。
  次に `TooLarge` が発生した場合、三度目の本定数引き下げではなく
