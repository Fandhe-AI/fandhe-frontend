...
| `MAX_INDEX_BYTES` | 1,441,792 バイト（1.375 MiB。#2552 で 1 MiB から 1.125 MiB へ、#2645 で 1.125 MiB から 1.25 MiB へ、#2751 で 1.25 MiB から 1.375 MiB へ引き上げ、§10 参照。イシュー #2637（Menu）の base 取り込み時点で実測は 1.25 MiB の範囲内に収まっており、追加の引き上げは不要だった。PR #2702（イシュー #2639 Breadcrumbs）の base 取り込み時点でも実測は範囲内に収まっており、追加の引き上げは不要だったが、イシュー #2751（`content-article` block 追加）で実測が 1,313,125 バイトへ達し実際に超過した） | **fail-closed**。`BuildError::SearchIndexTooLarge { bytes, limit }` を返し、**ページ書き出し前**に打ち切る |
...
- 新規モジュール `crates/docs-site/src/search_index.rs`（`lib.rs` に
  ...
  - `pub const MAX_INDEX_BYTES: usize = 1_441_792;`（#2552 で `1_048_576` から `1_179_648` へ、#2645 で `1_179_648` から `1_310_720` へ、#2751 で `1_310_720` から引き上げ、§10 参照。イシュー #2637・PR #2702（イシュー #2639 Breadcrumbs）の base 取り込み時点でも実測は範囲内だったが、#2751（`content-article` block 追加）で実際に超過した）
...
### 10-4 `MAX_INDEX_BYTES` 再引き上げ（#2751）実装記録

イシュー #2751（`content-article` block 追加、区分 Marketing・カテゴリ
Content の最初の block）を実装したところ、`cargo test -p
fandhe-frontend-docs-site` で `SearchIndexError::TooLarge { bytes:
1313125, limit: 1310720 }` が発火し、10-1 時点で引き上げた 1.25 MiB
（`1_310_720`）を実際に超過した。

- §10-1 が予告したとおり、per-page 上限見直し・セクション粒度分割は
  block 1 件の追加という規模に対して不釣り合いに大きいと判断し、
  §10/§10-1 と同じ判断軸で `MAX_INDEX_BYTES` を再度引き上げる対処を
  採った。
- 同じ +131,072 バイト刻みで 1.375 MiB（`1_310_720 + 131_072` =
  `1_441_792`）へ引き上げた。引き上げ後の実測（1,313,125 バイト）は
  新上限の約 91.1%（1,313,125 / 1,441,792）であり、§8 トリガー 1 の
  80% 基準を既に超えている。次回超過時は本節の刻み幅を機械的に
  繰り返すのではなく、§8 トリガー 1 の対応（per-page 上限見直し・
  セクション粒度分割）を実際に検討すること。
