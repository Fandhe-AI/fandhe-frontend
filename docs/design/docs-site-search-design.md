...
- コードブロック（`pre`/`code`）のテキストは**含める**。API 名検索の
  ...
  する。**ただし Blocks ページ（`crate::blocks::block_for_path` が
  `Some` を返すページ）は例外**とし、フェンスコードブロック本文を
  索引テキストから除外する（`page_entry` の `exclude_code_blocks`
  引数、イシュー #2862・§10-13）。Blocks ページの `## Rust コード`
  節は部品実装コードの全文複製であり、ページ本文の大半（実測で
  9 割超）を占める。「総量が支配的でない」という上記の許容根拠は
  Blocks ページには当てはまらないため、API 参照ページ等（本許容根拠が
  妥当なページ）とは別の扱いとする。
...
| 定数 | 値 | 超過時の扱い |
|---|---|---|
| `MAX_PAGE_TEXT_BYTES` | 4,000 バイト（イシュー #2849 で 4,096 から 4,032 へ暫定引き下げ〔§10-10〕、イシュー #2851 の base 取り込み時点では追加の引き下げ不要〔§10-11〕、PR #3272（イシュー #2853・#2855 の並行マージ）で再度超過を検知し 4,032 から 4,000 へ再引き下げ、§10-12 参照。`MAX_INDEX_BYTES` のハードルール〔§10-5・§10-6〕に抵触せず超過分を吸収する最小限の暫定調整。余裕は約 0.3% まで薄くなっている。イシュー #2862（`pricing-comparison-table` block 追加）でも独立に再度超過を検知したが、本定数の再引き下げは採らず Blocks ページのフェンスコードブロック本文を索引テキストから除外する対処で吸収した、§10-13 参照） | **決定的に切り詰める**（エラーにしない）。UTF-8 文字境界で切る（`char_indices` で境界を求め、バイト単位切断で不正 UTF-8 を作らない）。切り詰め痕跡の付加文字（`…` 等）は付けない（決定性と単純さを優先する） |
...
- 新規モジュール `crates/docs-site/src/search_index.rs`（`lib.rs` に
  ...
  - `pub const MAX_PAGE_TEXT_BYTES: usize = 4032;`（イシュー #2849 で `4096` から `4032` へ暫定引き下げ、§10-10 参照。イシュー #2851 では追加の変更なし、§10-11 参照。PR #3272（イシュー #2853・#2855 の並行マージ）で `4032` から `4000` へ再引き下げ、§10-12 参照。イシュー #2862 は独立に再度超過を検知したが本定数は据え置き、§10-13 参照）
...
### 10-13 イシュー #2862（`pricing-comparison-table`）で再度ハードルールに抵触したが、`MAX_PAGE_TEXT_BYTES` の再引き下げは採らず恒久対処へ切り替えた

イシュー #2862（Blocks に `pricing-comparison-table` block を追加）の
実装で `cargo test -p fandhe-frontend-docs-site` が
`SearchIndex(TooLarge { bytes: 1709568, limit: 1703936 })`（超過量
5,632 バイト）で FAIL した。

#### 却下した対処: `MAX_PAGE_TEXT_BYTES` の再引き下げ

§10-10・§10-12 と同型の緊急避難（`4_000` → `3_968` 等の再引き下げ）を
一度は実装したが、この対処自体が §10-10 が明記した「次回発生時、本定数の
再引き下げを機械的に繰り返さない」という自らの方針に反すること（本 PR の
レビューで指摘）を踏まえ、採用しないことにした。この対症療法を続ける
限り、257 ページ超（既に per-page 上限で切り詰められている全ページ）の
索引精度を Blocks 追加のたびに一律で落とし続けることになり、恒久対処
（イシュー #3173、セクション粒度インデックスへの分割）を先送りするだけで
終わる。

#### 採用した対処: Blocks ページのフェンスコードブロック除外

イシュー #3173（セクション粒度インデックスへの分割）のフル実装は本イシュー
単体のスコープ（Blocks への 1 block 追加）に対して不釣り合いに大きいが、
超過の実態を実測したところ、Blocks ページ 1 件（`pricing-comparison-table`）
の Markdown 原稿は 15,901 バイトのうち 14,991 バイト（約 94%）が
`## Rust コード` 節のフェンスコードブロック（部品実装コードの全文複製、
`blocks_code_drift.rs` が原文一致を固定）であることを確認した。これは
Blocks セクション（124 ページ、`crate::blocks::all_blocks()`）全体に
共通する構造であり、§3-3 が「コードブロックは含める」とした根拠
（「API 名検索の実用性を優先し、総量は支配的でない」）は、#957 設計時点
（実測 16,360 バイト）を前提としており Blocks ページには当てはまらない。

このため、`MAX_INDEX_BYTES`・`MAX_PAGE_TEXT_BYTES` のいずれも変更せず
（前者は §10-6 のハードルールを遵守、後者は §10-12 の `4_000` のまま
据え置き）、`search_index::page_entry` に `exclude_code_blocks: bool`
引数を追加し、`crate::build::build_site` が `blocks::block_for_path` で
判定した Blocks ページのみ `true` を渡す形で、フェンスコードブロック
（`crate::markdown::parse_fence` が生成する `pre` 要素の部分木）を索引
テキストから除外するようにした。API 参照ページ（`docs/guides/
component-authoring.md` 等）はこの除外対象にならず、既存の
`real_site_search_index_still_contains_code_block_keywords_after_
highlighting` 回帰テスト（イシュー #1078）が示すとおり従来どおり
コードブロックが索引に含まれる。

- **既存 257 ページ超の索引精度は一切落とさない**（対症療法との決定的な
  違い。Blocks ページ以外は本変更による挙動変化がない）。
- **恒久対処ではなく Blocks ページに特化した除外**である点で
  イシュー #3173（セクション粒度分割）の完全な代替ではないが、Blocks
  ページが今後も索引総量の支配的な増分源であり続ける限り、再発防止として
  §10-10・§10-12 より確実に効く（1 ページあたり数 KB〜十数 KB の削減
  であり、`MAX_PAGE_TEXT_BYTES` の per-page 上限を数十バイト刻みで
  引き下げるより桁違いに大きい安全余裕を生む）。
- Blocks ページ以外（API 参照ページ等）でコードブロックが索引の支配的な
  割合を占める事態が将来生じた場合は、本節と同型の判断（除外対象の拡大、
  またはイシュー #3173 のセクション粒度分割）を再検討する。

### 10-14 イシュー #2856（`header-flyout-menu` の残り領域・状態表示・原稿を仕上げる）PR #3276 は §10-13 の恒久対処後も上限に収まった（本定数の追加引き下げは行わない）

PR #3276 は自ブランチの派生元だった旧 main（`MAX_PAGE_TEXT_BYTES = 4_032`、
Blocks ページのフェンスコードブロック除外〔§10-13〕未導入）に対して単独
計測すると `SearchIndex(TooLarge { bytes: 1705161, limit: 1703936 })`
（超過量 1,225 バイト）を再検知していたが、これは §10-12（PR #3272）が
既に `4_000` へ引き下げ済みだった変更と時期が重なっていただけであり、
PR #3276 側の追加コンテンツそのものが新たな超過を要求したわけではなかった。
base 取り込みで §10-12 の `4_000` および §10-13 のフェンスコードブロック
除外を引き継いだ状態で再計測すると `MAX_INDEX_BYTES` 超過は再発せず、
`cargo test -p fandhe-frontend-docs-site`（`search_index.rs`・
`blocks_contract.rs` を含む）が全件 PASS することを確認した。

#### 決定

- **本定数を `4_000` からさらに引き下げない**（§10-10・§10-12 で明記した
  「次回発生時に本定数の再引き下げを機械的に繰り返さない」方針を実際に
  守った実例。§10-13 のフェンスコードブロック除外という恒久寄りの対処が
  既に効いていることも確認した）。
- 恒久対処（セクション粒度インデックスへの分割、追跡: イシュー #3173）は
