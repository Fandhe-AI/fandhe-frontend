...
- §10-1 の実測（1,180,096 バイト、新上限比 90.0%）の時点で既に §8
  ...
  継続して採用する。ただし今回で対症療法は**最後**とし、per-page 上限
  見直し・セクション粒度分割の本格検討は `out-of-scope-tracking.md` の
  方針に従い実 Issue（イシュー #3173「検索インデックスをセクション粒度へ
  分割する」）として切り出した（本 PR のスコープ外）。
- 引き上げ幅は §10/§10-1 と同じ +131,072 バイト刻みを踏襲し、1.375 MiB
  （`1_048_576 + 393_216` = `1_441_792`）へ引き上げる。引き上げ後の実測
  （1,313,090 バイト）は新上限の約 91.1%（1,313,090 / 1,441,792）であり、
  §8 トリガー 1 の 80% 基準を今回も引き続き超過している。イシュー #2730
  配下で今後も block 追加が続く前提であり、残り余裕は約 128 KiB
  （`1,441,792 - 1,313,090`）しかなく、1 ページあたりの索引テキスト
  上限（`MAX_PAGE_TEXT_BYTES` = 4,096 バイト）を単純に割ると新規ページ
  30 件強で再び上限へ到達する計算になる（#2730 は新規 301 block ページを
  計画しており、桁が 1 つ足りない）。

  **決定（本 PR の対処）と今後の運用ルール**:
  - `MAX_INDEX_BYTES` は本 PR の引き上げ（`1_441_792`）を最終値として
    **固定（ハードルール）** する。次回 `SearchIndexError::TooLarge` が
    発生した場合、この値をこれ以上引き上げてはならない。
  - 機械的な制約として `crates/docs-site/tests/search_index.rs` に
    `max_index_bytes_is_pinned_pending_issue_3173_section_granularity_split`
    を追加し、`MAX_INDEX_BYTES` が `1_441_792` から動いていないことを
    テストで固定する（値を変更する PR はこのテストの意図的な書き換えを
    要求されるため、無自覚な機械的引き上げを防ぐ）。
  - 移行条件: 次回 `TooLarge` が発生した時点で、イシュー #3173
    （セクション別インデックスファイルへの分割、§8 トリガー 1 の恒久
    対応）の実装に着手する。`MAX_INDEX_BYTES`（1 ファイル全体上限という
    概念）自体を撤廃する構成変更であり、上記ピンテストもそのタイミングで
    合わせて置き換える。
  - 期限: 明確な暦日は設定しないが、イシュー #2730（301 block 追加計画）
    の進捗上、残り余裕（約 128 KiB ≒ 30 ページ強相当）を使い切る前に
    イシュー #3173 の着手を開始すること。
