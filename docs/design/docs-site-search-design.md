...
イシュー #2865（`pricing-seats-split` block 追加）も同じフェンスコード
ブロック除外の恩恵を受け、`MAX_PAGE_TEXT_BYTES`（`4_000`）・
`MAX_INDEX_BYTES` のいずれも変更せずに `cargo test -p
fandhe-frontend-docs-site` が PASS することを確認済み（base 取り込み後の
再実測）。
