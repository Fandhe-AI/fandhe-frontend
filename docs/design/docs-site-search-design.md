...
### 10-13 イシュー #2856（`header-flyout-menu` の残り領域・状態表示・原稿を仕上げる）PR #3276 は §10-12 の引き下げ後上限に収まった（本定数の追加引き下げは行わない）

PR #3276 は自ブランチの派生元だった旧 main（`MAX_PAGE_TEXT_BYTES = 4_032`）
に対して単独計測すると `SearchIndex(TooLarge { bytes: 1705161, limit:
1703936 })`（超過量 1,225 バイト）を再検知していたが、これは §10-12
（PR #3272）が既に `4_000` へ引き下げ済みだった変更と時期が重なっていた
だけであり、PR #3276 側の追加コンテンツそのものが新たな超過を要求した
わけではなかった。base 取り込みで §10-12 の `4_000` を引き継いだ状態で
再計測すると `MAX_INDEX_BYTES` 超過は再発せず、`cargo test -p
fandhe-frontend-docs-site`（`search_index.rs`・`blocks_contract.rs` を含む）
が全件 PASS することを確認した。

#### 決定

- **本定数を `4_000` からさらに引き下げない**（PR #3276 のレビュー指摘。
  §10-10・§10-12 で明記した「次回発生時に本定数の再引き下げを機械的に
  繰り返さない」方針を実際に守った初例）。
- 恒久対処（セクション粒度インデックスへの分割、追跡: イシュー #3173）は
  引き続き未着手のまま。§10-12 が指摘したとおり余裕は乏しい（約 0.3%）ため、
  次回の Blocks 追加で本定数の引き下げでは吸収しきれない超過が発生する
  可能性が高く、イシュー #3173 の優先度を上げることを重ねて推奨する。
