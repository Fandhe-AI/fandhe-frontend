...
| 定数 | 値 | 超過時の扱い |
|---|---|---|
| `MAX_PAGE_TEXT_BYTES` | 4,000 バイト（イシュー #2849 で 4,096 から 4,032 へ暫定引き下げ〔§10-10〕、イシュー #2851 の base 取り込み時点では追加の引き下げ不要〔§10-12〕、PR #3272（イシュー #2853・#2855 の並行マージ）で再度超過を検知し 4,032 から 4,000 へ再引き下げ〔§10-13〕、イシュー #2858（`header-mega-menu`）合流時の再超過は本定数の追加引き下げではなく該当ページの本文削減で対処〔§10-11〕。`MAX_INDEX_BYTES` のハードルール〔§10-5・§10-6〕に抵触せず超過分を吸収する最小限の暫定調整。余裕は約 0.3% まで薄くなっている） | **決定的に切り詰める**（エラーにしない）。UTF-8 文字境界で切る（`char_indices` で境界を求め、バイト単位切断で不正 UTF-8 を作らない）。切り詰め痕跡の付加文字（`…` 等）は付けない（決定性と単純さを優先する） |
...
- 新規モジュール `crates/docs-site/src/search_index.rs`（`lib.rs` に
  ...
  - `pub const MAX_PAGE_TEXT_BYTES: usize = 4032;`（イシュー #2849 で `4096` から `4032` へ暫定引き下げ、§10-10 参照。イシュー #2851 では追加の変更なし、§10-12 参照）
...
### 10-11 イシュー #2858（`header-mega-menu`）と #2855（`header-flyout-menu`）の並行合流で再発、今回は `MAX_PAGE_TEXT_BYTES` を引き下げずページ本文を削って対処

イシュー #2858（Blocks に `header-mega-menu` block を追加する PR #3273）が
レビュー指摘対応の一環で `origin/main`（イシュー #2855
`header-flyout-menu` が既にマージ済み）を取り込んだところ、
`cargo test -p fandhe-frontend-docs-site` が
`SearchIndex(TooLarge { bytes: 1704988, limit: 1703936 })`（超過
1,052 バイト）で FAIL した。

#### 実測

- `header-mega-menu`・`header-flyout-menu` はいずれも Rust コードフェンスを
  含む本文が 1 ページ 1 万バイト超であり、`MAX_PAGE_TEXT_BYTES`
  （当時 4,032）の per-page 上限で既に切り詰められていた。#2849 時点の
  257 ページからさらに増加した切り詰め済みページ数のうち、本 2 ページは
  それぞれ**新規ページとして丸ごと** 4,032 バイトずつを索引全体へ追加した
  （既存ページの増分ではなく、2 ページ分の新規追加自体が原因）。
- `header-mega-menu` 単体の P1/P2 レビュー是正（`disabled` 属性追加・
  `REPO` 定数追加等）による本文の増減は、既に per-page 上限で切り詰められて
  いたため索引全体のバイト数に影響しなかった（#2849 §10-10 の教訓と同型）。

#### 決定

- **`MAX_INDEX_BYTES`（1.625 MiB = `1_703_936`）は変更しない**（§10-6 の
  ハードルールを遵守）。
- 当初 `MAX_PAGE_TEXT_BYTES` を `4_032` → `3_968` へ再度引き下げる対処で
  一度実装したが、PR #3273 の AI レビュー（P1）が「§10-10 の『全ページ上限の
  再引き下げの反復を避ける』方針に反する」と指摘したため、この引き下げを
  取り消した（本定数は origin/main が既に到達していた `4_000` のまま据え置く）。
- **`header-mega-menu` 自身のプロースを削っても効果がないことを実測で確認**:
  同ページは Rust コードフェンスを含む本文が `MAX_PAGE_TEXT_BYTES`
  （4,000 バイト）を大きく超えており、既に per-page 上限で切り詰められて
  いる。したがって同ページの Markdown プロースをどれだけ削っても、
  切り詰め後の `text` フィールドは常に上限一杯（4,000 バイト）のままで
  索引全体のバイト数は変化しない（`page_entry`/`truncate_to_byte_limit`
  の仕様上当然の帰結）。
- 代わりに、同じ Header カテゴリで並行合流した 3 ページのうち、
  per-page 上限に達していない `header-floating-pill`（実測 3,063 バイト
  程度、`MAX_PAGE_TEXT_BYTES` に対し十分な余裕あり）の
  `site/blocks/header-floating-pill.md` から、実装詳細を繰り返すだけの
  補足カッコ書き 1 文（パネルのレイアウト実装詳細の説明）を削り、当該
  ページの索引対象本文自体を縮小した。同ページは切り詰められていない
  ため、この削減がそのまま索引全体のバイト数減少に直結する。索引 JSON
  全体は 1,703,985 バイト（超過 49 バイト）から上限内へ戻った。
- **本対処が §10-10/§10-13 と異なる点**: 全ページ一律の per-page 上限
  （`MAX_PAGE_TEXT_BYTES`）を下げるのではなく、超過の原因となった
  3 ページの合流のうち、per-page 上限で切り詰められておらず実際に
  効果のあるページの記述量を削る対処であり、既存の 300 ページ超の索引
  精度には影響しない。ただし本質的には「特定ページの記述を削る」という
  対症療法であることに変わりはなく、恒久対処（選択肢 3、セクション粒度
  インデックスへの分割、追跡: イシュー #3173）を代替するものではない。
  索引全体は対処後も `MAX_INDEX_BYTES` の約 99.7% に達しており、§8
  トリガー 1（80% 超過）を大きく超えたままである。次回発生時はまず
  イシュー #3173 の優先度を上げて着手することを強く推奨する。

### 10-12 イシュー #2851（`footer-link-columns`）は既存の引き下げ後上限に収まった
...
### 10-13 PR #3272（イシュー #2853・#2855 の並行マージ）で再度ハードルールに抵触し、`MAX_PAGE_TEXT_BYTES` を再度引き下げて対処
...
（§10-10 の引き下げ後 3 度目の base 取り込みで発生。§10-12 の
