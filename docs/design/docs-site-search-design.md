...
- 新規モジュール `crates/docs-site/src/search_index.rs`（`lib.rs` に
  ...
  - `pub const MAX_INDEX_BYTES: usize = 1_703_936;`（#2552 で `1_048_576` から `1_179_648` へ、#2645 で `1_179_648` から `1_310_720` へ、イシュー #2814 で `1_310_720` から `1_703_936` へ引き上げ、§10 参照。イシュー #2637・#2639 の base 取り込み時点でも実測は範囲内だったが、#2814 で超過し引き上げた。イシュー #2816 の base 取り込み時点でも実測は範囲内であり追加の引き上げは不要だった、§10-5 参照）
...
### 10-5 イシュー #2816（careers-split-accordion）base 取り込み時点の実測確認
...
10-4 の再評価トリガー文言（機械的な再引き上げではなく per-page 上限
