...
- 4 分割サブタスクの内訳:
  - TASK-6.3a（#65・クローズ済み）: CI `browser-test` ジョブ（`wasm-pack test
    ...
    chromedriver 明示指定）+ `docs/browser-testing.md` を整備。
...
- 本ドキュメントは TASK-6.3d の成果物（検証レポート）であり、main の実体（テスト
  ...
  `docs/perf-browser-report.md`（TASK-11.5c・#88）で既に **Go 判定済み**。本
...
TASK-11.5 側（`docs/perf-browser-report.md`）も Go 判定済みのため、
...
`docs/hydration-api.md` 第 3・6 節の不変条件、および REQ-6 受け入れ基準に対応する
...
- ローカル再現手順は `docs/browser-testing.md` 第 5 節を参照。
...
- **状態注入フォーマット（`data-hydrate-*` エンコード）**: `hydration_browser.rs`
  ...
  自体の製品化は TASK-11.4 系（#81〜#84）のスコープ（`docs/hydration-api.md`
  第 5 節スコープ外表）。
- **性能実証**: 初期ロード・DOM 操作性能の判定は TASK-11.5 側（`docs/perf-browser-report.md`）
  が担当し、本ドキュメントの対象外。
...
- **計測環境の限定**: 本レポートは CI（GitHub ホストランナー・`ubuntu-latest`）
  ...
  `docs/browser-testing.md` 第 5 節の手順で可能だが、本レポート作成時点では
...
- **A03 インジェクション / XSS**: 観点 5（`xss_payload_item_does_not_produce_script_element_in_real_dom`）
  ...
  `raw_html()` は使用していない（`docs/hydration-api.md` 第 6 節不変条件）。
...
- **A05 セキュリティ設定ミス / A08 サプライチェーン**: `browser-test` ジョブは
  ...
  （`docs/browser-testing.md` 第 8 節と整合）。
...
- `docs/browser-testing.md`（TASK-6.3a・#65、検証環境・第 7 節引き継ぎ表）
- `docs/hydration-api.md`（第 3・5・6 節、ハイドレーション API 契約・不変条件）
- `docs/perf-browser-report.md`（TASK-11.5c・#88、性能側の Conditional Go 条件 1 判定）
