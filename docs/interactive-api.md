# rws-interactive 状態管理 API 設計確定（TASK-11.1a）
...
`rws-interactive` の公開 API 表面・モジュール構成・セキュリティ不変条件を**設計と
...
- **TASK-11.1b（#71）**: 本書に従った `rws-interactive` クレートの実装
...
`docs/hydration-api.md`（TASK-6.2a）と同じ書式（ステータス・トレーサビリティ・
...
**先行依存関係**: 本書の設計は `rws-core`（マージ済み、`docs/component-api.md`
第 2 節の凍結表）のみに依存する。`rws-interactive` と `rws-wasm-full`
（TASK-11.2）・ハイドレーション状態注入の実配線（TASK-11.4 #81）との統合は
本書のスコープ外とし、第 5 節に引き継ぎ事項として明記する。

## 2. クレート構成の確定

- **パッケージ名**: `rws-interactive`
- **配置**: `interactive/`
- **edition**: 2021
- **属性**: `#![forbid(unsafe_code)]` + `#![warn(missing_docs)]`
- **依存**: `rws-core`（path 依存）のみ。**外部クレート 0**（PoC-5 の
  `interactive/Cargo.toml` 実績を踏襲。`docs/unsafe-boundary.md` 第 2 節が
  「作成時に `#![forbid(unsafe_code)]` を設定」と予告している内容に合致する）。

依存グラフ上限（REQ-3: 標準サーバー構成 60 件以内・深さ 6 以内、
`docs/dependency-graph-policy.md`）に対し、`rws-interactive` が外部依存 0 を
維持することで、`rws-app`（`docs/app-api.md` 第 2 節）と同様にこの余裕を
消費しないことを設計上の根拠として記録する。

## 3. 公開 API 凍結表

`rws-interactive` は「状態を持つクライアント側コンポーネント」の抽象を提供する。
`docs/component-api.md` が確定した「コンポーネントは `Node` を返す通常の Rust
...
| API | シグネチャ | 役割 |
|-----|-----------|------|
| `Component` | `pub trait Component { type Action; fn update(&mut self, action: Self::Action); fn view(&self) -> rws_core::Node; fn decode_action(name: &str, payload: &str) -> Option<Self::Action>; }` | 状態とその描画・遷移を結ぶ中核トレイト。PoC-5 の `AppState`/`dispatch`/`render` を汎用化する |
...
| `render_for_hydration` | `pub fn render_for_hydration<C: Component + Hydrate>(component: &C) -> rws_core::Node`（本体は TASK-11.1b） | `view()` のルート要素へ `hydration_attrs()` を付与した `Node` を返す SSR 用ヘルパ |

### 3.1 設計方針の要点

- **`view()` は `rws_core::Node` のみを返す**: `Component::view` の戻り値は
  `rws_core::Node`（`docs/component-api.md` 第 2 節で凍結済みの型）のみとし、
  文字列や DOM 型を返さない。これにより `rws_core::render()` の既定エスケープが
...
| # | 判断 | 根拠 |
|---|------|------|
| 1 | `Component` はトレイトとして提供し、`docs/component-api.md`（REQ-5）が定める「コンポーネントは `Node` を返す通常の Rust 関数」という**ステートレスな記述規約とは別の抽象**として位置付ける | REQ-5 はページ/コンポーネント関数の記述方式（マクロ DSL 不使用）を対象とする要件であり、状態を持つクライアント側の状態機械（REQ-11 の対象）とは扱う関心事が異なる。`Component::view` の内部実装は REQ-5 の規約（`Node` を返す通常の Rust 関数として組み立てる）にそのまま従うため、両者は矛盾せず補完関係にある |
| 2 | 文字列 dispatch（`dispatch<C: Component>(component, name, payload) -> bool`）を提供し、型付き `update(Self::Action)` とは別関数にする | WASM 境界（`rws-wasm-full`、TASK-11.2）はブラウザの `data-action`/`data-payload` 属性から文字列を受け取る（PoC-5 実績）。型変換の失敗（`decode_action` が `None` を返す）を状態変更なしで吸収する境界層を `Component` 実装側と分離し、コア状態機械を文字列表現に汚染させない |
| 3 | ハイドレーション属性の命名規約を `HYDRATE_ATTR_PREFIX = "data-hydrate-"` として定数化する | PoC-5（`docs/spec/03-poc/wasm-runtime-split/interactive/src/lib.rs:158-167`）の `data-hydrate-counter`/`data-hydrate-draft`/`data-hydrate-items` 実績を踏襲しつつ、個々の属性名をハードコードせず接頭辞を共有定数として公開し、`rws-wasm-full`（TASK-11.2）・`rws-server` 側で同一の規約を参照できるようにする |
| 4 | `from_hydration_attrs` は改ざんされうるクライアント入力を前提に `Result<Self, HydrateError>` を返し、`unwrap()`/`panic!` を使わない | `.claude/rules/coding-rust.md` のエラーハンドリング規約。PoC-5 の `state_from_hydration_attrs` は数値パース失敗時に `unwrap_or(0)` でフォールバックしていたが、製品版では失敗を型として表現し、フォールバック戦略（既定値に倒すか呼び出し元へエラーを伝播するか）を呼び出し側（`rws-wasm-full`）の選択に委ねる |
...
| `rws-wasm-full`（TASK-11.2 #未定）でのイベント配線・`Closure` 配線との統合 | TASK-11.2 |
| ハイドレーション状態注入の実配線（サーバー Rust ↔ クライアント WASM の end-to-end 結合） | TASK-11.4（#81） |
| `find_attr_values`/`find_nav_targets`（`rws-core` 側のハイドレーション対象特定 API） | TASK-6.2 系（`docs/hydration-api.md` 第 2〜3 節、マージ済み設計） |
...
`core/src/lib.rs` 冒頭に記載された不変条件（REQ-1・REQ-2）を、`rws-interactive`
への制約としてそのまま再掲・固定し、状態管理コア固有の不変条件を追加する。

1. `Component::view()` の出力は `rws_core::Node` のみであり、`rws_core::render()`
   の既定エスケープを必ず経由する。**`rws-interactive` クレート内では
...
6. `#![forbid(unsafe_code)]` によりクレート全体で `unsafe` を機械的に禁止する
   （REQ-2）。`docs/unsafe-boundary.md` 第 2 節の `interactive` 行を「未作成」
   から「作成済み・forbid 設定済み」へ本タスクで更新する。
7. `interactive/Cargo.toml` の `[dependencies]` は `rws-core`（path）のみを
   維持する（外部依存ゼロ、REQ-3）。
...
|--------------------|----------------------|-----------|
| WASM 完全方式でのイベント処理・DOM 操作が `unsafe` を使用せず safe Rust の範囲に収まること | `rws-interactive` は `#![forbid(unsafe_code)]`（第 2 節・第 6 節・不変条件 6） | TASK-11.1b（#71） |
...
| クライアント WASM のイベント処理・DOM 更新を経由した出力にも同一のエスケープ保証が及ぶこと（REQ-1 関連） | `Component::view()` が `rws_core::Node` のみを返す契約（第 6 節・不変条件 1） | TASK-11.1b（#71）・TASK-11.2 |

## 8. 関連文書との整合確認

- `docs/component-api.md` 第 5 節「状態管理（`rws-interactive` クレート）→ 別
  クレート（TASK-5.1 系の対象外）」は本書により解消される。`Component` トレイト
  は REQ-5 の「コンポーネントは通常の Rust 関数」規約と矛盾しないことを
  第 3 節冒頭・第 4 節・判断 1 で明記した。
- `docs/hydration-api.md` 第 5 節「状態注入・`rws-interactive` との統合 →
  TASK-11.4（#81）」・第 2 節「`rws-core` 側には新たに `find_attr_values`/
  `find_nav_targets` を追加する」との役割分担を維持する。本書の `Hydrate`
  トレイトは `rws-wasm-client`/`rws-wasm-full` 側のハイドレーション対象特定
