...
- **TASK-11.3b（本ドキュメント・#80）**: `docs/opt-in-thin-js-glue.md`（オプトイン
  提供ドキュメント・制約事項の明記）
...
（`docs/wasm-full-architecture.md` が TASK-11.1b 未マージ時に採った運用と同一）。
...
本書は `docs/wasm-full-architecture.md`（TASK-11.2a）・`docs/embedding-guide.md`
（TASK-7.1b）・`docs/component-api.md`（TASK-5.1a）と同じ書式（目的とトレーサビ
...
（WASM 完全方式、`docs/wasm-full-architecture.md` 参照）です。`rws-wasm-thin`
...
| `docs/unsafe-boundary.md` の `wasm-thin` 行の更新（`rws-wasm-thin` は `web-sys` 非依存のため `unsafe` を使用しない見込みだが、確定は実装時に行う） | TASK-11.3a（#79） |
