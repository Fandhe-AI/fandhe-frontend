...
`docs/hydration-state-format.md`（TASK-11.4a・#82、確定版）は REQ-11 の受け
...
本書は `docs/hydration-state-format.md`・`docs/interactive-api.md`・
`docs/wasm-full-architecture.md` と同じ書式（ステータス・トレーサビリティ・
...
- 対象は `rws_interactive::Hydrate` トレイトが既に一般化している
  ...
  変更しない**（凍結済み API、`docs/hydration-state-format.md` 第 3 節）。
...
`docs/hydration-state-format.md` 第 8 節の不変条件をそのまま継承し、
...
| 既存の単純な値のみのフォーマット（`docs/hydration-state-format.md`）との後方互換 | `escape_item`/`unescape_item`・`encode_list`/`decode_list` を無変更のまま維持し、`Value` codec からも一切呼び出さない（第 3.3 節・第 4 節・判断 4、回帰テストで固定） |

## 10. 関連文書との整合確認

- `docs/hydration-state-format.md` 第 2 節・第 7 節が本書（イシュー #163）
  への引き継ぎとして明記した記述と整合する。同書自体は改訂せず、本書が
  独立した拡張として参照する形を取る。
- `docs/interactive-api.md` 第 3〜4 節の `Hydrate`・`HYDRATE_ATTR_PREFIX`・
