...
| `motion_view_transition_css.rs`（イシュー #2516） | `#![cfg(feature = "motion")]` のため `--features motion` 指定なしでは 0 件実行（正常）。`cargo test -p fandhe-frontend-pre-styled-ui --features motion --test motion_view_transition_css` で実行し、`view_transition::VIEW_TRANSITION_PRESETS_CSS`（fade/slide/wipe の named view transition CSS プリセット）の golden 一致・`data-fandhe-view-transition` 属性セレクタの fade/slide/wipe 網羅・reduced-motion ブロックの `animation: revert;` グループ化セレクタを確認する。`view_transition.rs` の CSS リテラルを変更したら golden を §5 の通常フローで貼り替える |
