...
| `radial_chart_css.rs` | `charts::radial_chart`（イシュー #2133 で `bar[data-hidden]` state を golden 末尾へ純追加。Cursor Bugbot 指摘是正〔`hidden_categories` によるリング単位非表示・`bar`/`label` の `data-index` 共有識別子追加〕で `label[data-hidden]` state を golden 末尾へ純追加。さらなる Cursor Bugbot 指摘是正〔`track`（リング背景）が非表示リングでも既定グレーのまま残る不具合〕で `track` にも `data-index`/`data-hidden` 伝搬を追加し `track[data-hidden]` state を golden 末尾へ純追加） |
