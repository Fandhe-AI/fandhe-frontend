...
- `max_ms` は外れ値（GC・ランナー負荷等）の有無の参考値とし、`p95_ms`・超過率
  ...
  しない（`docs/perf-browser-harness.md` 第 4 節のとおり、CI 共有ランナーは
...
| metric | samples | mean_ms | p95_ms | max_ms | 16ms 超過率 | 予算 | 判定 |
|--------|---------|---------|--------|--------|-------------|------|------|
| `initial_load` | (未計測) | (未計測) | (未計測) | (未計測) | (対象外) | 300ms 以内 | 保留 |
| `dom_update` | (未計測) | (未計測) | (未計測) | (未計測) | (未計測) | 16ms/フレーム以内 | 保留 |

「16ms 超過率」は `dom_update` について、全サンプル中 16ms を超過した操作の
割合を指す（第 3 節の判定基準に対応）。実測値の記録後、本節を更新し、各行の
「判定」列を Go / Conditional Go / No-Go / 要再計測のいずれかで埋める。
`dom_update` の判定は `mean_ms` のみで行わず、`p95_ms` と超過率を踏まえて
決定する。

## 5. Conditional Go 条件 1 解消判定（TASK-11.5b 完了後に確定）

上記第 4 節の実測値について、`initial_load` の `mean_ms`・`p95_ms` が予算内、
かつ `dom_update` の `p95_ms` が 16ms 以内で 16ms 超過率も無視できない水準
（目安 5%）を超えていなければ、Conditional Go 条件 1（実ブラウザでの正式実証）
は TASK-6.3（ハイドレーション実証・クローズ済み）と合わせて **解消**とし、
`docs/spec/06-roadmap.md` MS-3 完了時の Go/No-Go 確認において本レポートを
根拠資料とする。`dom_update` の `mean_ms` のみが予算内でも `p95_ms`・超過率が
上記基準を満たさない場合は、単純な Go とはせず Conditional Go（要再計測・
要精査）として扱う。
