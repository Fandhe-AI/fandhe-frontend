...
| 装飾系（CSS 主体で実装可能性がある、または既存基盤で実装見込みがあるもの） | splitter（ark・chakra 双方）/ floating-panel（ark・chakra 双方）/ tour | splitter は CSS（`resize`）主体で実装できる可能性がある。floating-panel は positioning（§3.20）・overlay（`crates/wasm-full/src/overlay.rs`）基盤が実装済みで将来実装しやすい。tour は状態機械（ステップ管理・ハイライト対象の同期）が大きく需要待ち。scroll-area（ark・chakra 双方）はイシュー #825 で保留解除・実装済み（`docs/design/component-coverage-map.md` 参照。JS によるスクロール位置追従・thumb drag は同イシューのスコープ外のまま） | 利用要望 issue の起票。floating-panel は既存 positioning/overlay 基盤で決定的に実装できる範囲を確認したうえで着手する |
