...
### 3.25 アプリケーションロジックを内包する UI 部品（Radix `Form` 等）と、headless 層への装飾関心の混入（ユーザー判断 2026-07-25、イシュー #959 系）

- **概要**: Radix UI を第 3 の参照軸として追加した（イシュー #937、
  `docs/design/component-coverage-map.md`）際に、Radix 側の部品には
  「UI の構造・アクセシビリティ」を超えた関心が同居しているものがあることが
  判明した。本節はそれらの取り扱いを 2 つの規則として確定する。ark-ui /
  chakra-ui 由来の判断（§3.22〜§3.24）と同じ評価軸（§2）に立つ。

- **規則 1（非採用）: アプリケーションロジックを内包する部品は採用しない**

  UI コンポーネント層が担うのは **anatomy（構造）・アクセシビリティ
  （WAI-ARIA・キーボード操作）・表示状態（`data-*`）まで**とする。
  バリデーション・送信処理・データ整形・永続化といったアプリケーション
  ロジックを内包する部品は、参照軸に含まれていても実装しない。

  これは新規の原則ではなく、§3.23 が
  「数値・日時整形は UI コンポーネント層の責務外。国際化ライブラリは
  持ち込まない」として FormatByte / FormatNumber / FormatRelativeTime /
  FormatTime を非採用とした判断の一般化である。

  - **対象（確定）**: Radix Primitives `Form`（`form`）。
    Radix `Form` は Field / Label / Control / Message / ValidityState /
    Submit の anatomy を持つが、その本体は**ブラウザ Constraint Validation
    API を用いたバリデーション実行・カスタム検証・エラーメッセージ対応付け・
    送信ハンドリング**であり、規則 1 の「アプリケーションロジック」に当たる。
  - **構造部分は既に充足している**: Radix `Form` のうち UI 構造に相当する
    部分（フィールドとラベル・説明・エラー表示の結び付け、`data-invalid`
    による無効状態の表現）は `crates/headless-ui/src/field.rs` および
    `fieldset.rs` が既に担っている。本節が落とすのは検証・送信の層のみで
    あり、利用者はバリデーションを通常の Rust コードで書き、その結果を
    `field` の状態として渡す。
  - **評価軸との対応**: バリデーション規則を部品内部に持つと、どの入力が
    どの条件で不正になるかがコンポーネント定義からは読み取れなくなる
    （明示性の低下）。検証の実行タイミングがブラウザ API の挙動に依存し、
    SSR 経路と CSR 経路で結果が分岐しうる（決定性の低下）。

- **規則 2（層の割り当て）: 装飾・アニメーション関心を headless 層へ持ち込まない**

  `fandhe-frontend-headless-ui` は Radix Primitives を参照するが、Radix が
  primitives 側に持ち込んでいる**装飾・アニメーション・レイアウト計測の
  関心**（`data-motion` によるアニメーション方向の露出、viewport の寸法
  測定とそれに基づく DOM 出力の変化など）は headless 層へ持ち込まない。
  これらが必要な場合は上層の `fandhe-frontend-pre-styled-ui` の責務として
  設計する（2 層構成の責務境界。`crates/headless-ui/src/lib.rs` の
  クレートレベル rustdoc 参照）。

  規則 2 は非採用の規則ではなく**配置の規則**である。当該関心を持つ部品を
  実装対象から外すのではなく、headless 層には構造とアクセシビリティのみを
  置き、装飾側を pre-styled 層へ寄せる。

  - **適用例**: Radix Primitives `Navigation Menu`（`navigation-menu`、
    `component-coverage-map.md` §5 Part D で実装対象）。同部品を既存
    `nav_list` と区別する要素として挙げられている `viewport` 測定・
    `data-motion` は規則 2 の対象であり、headless 層には置かない。
...
- **再評価トリガー**
  - 規則 1: 「バリデーション結果を UI 部品の外側で組み立てる」書き方が
    ...
    記述量増大の実測、利用者からの具体的な不具合報告）が出た場合。
    再導入するとしても、検証ロジックそのものではなく「検証結果を受け取って
    表示状態へ変換する」層に限定できるかを §4 の手続きで評価する。
  - 規則 2: pre-styled 層に寄せた結果、headless 層だけでは実現できない
...
イシュー #959 で本節へ転記する。**このうち Form は 2026-07-25 のユーザー
判断により保留を解除し、§3.25 規則 1（アプリケーションロジック内包部品）の
意図的非採用へ確定移行した**（下表の該当行を参照。現存する Radix 由来の
保留は 6 件）。本節が記録対象とする行は #735 時点の 36 行
+ Radix 由来 7 行 = 43 行（上表のとおり 36 行の多くは #735 以降の各実装
イシューで既に保留解除・実装済みへ移行済みであり、43 は「現存する保留行数」
ではなく「本節が扱ってきた延べ記録対象数」である点に注意）。イシュー #1676
（親 #1669、Phase 13）で参考サイトを再突合し、Themes 追加候補のうち保留と
判定した 4 群（code-block / prose / toc / checkbox-cards・radio-cards group
版）を本節へ追加した（延べ記録対象は 43 + 4 群）。
...
| Radix 由来（イシュー #937 で新規判定、#959 で本節へ転記） | ~~Form (`form`)~~（**保留解除・意図的非採用へ確定移行、2026-07-25 ユーザー判断。根拠は §3.25 規則 1**） / Direction Provider (`direction-provider`) / Accessible Icon (`accessible-icon`) / Slot (`slot`) / Inset (`inset`) / Radio (`radio`) / Reset (`reset`)（いずれも `docs/design/component-coverage-map.md` §5 Part D 参照） | Form: 保留解除済み（§3.25 規則 1 の意図的非採用。バリデーション・送信はアプリケーションロジックであり UI 層の責務外。構造部分は `field`/`fieldset` が既にカバー）。Direction Provider: RTL/LTR 動的注入 provider 機構が §3.23 の JS ランタイム固有 utilities に類するが個別記録がない。Accessible Icon: **イシュー #1066 で検証完了（2026-07-26）**。Themes/pre-styled 層は `icon`（`IconProps::label`）単体で完全代替、Primitives/headless 層は `role`/`aria_label`（`crates/headless-ui/src/aria.rs`）+ `visually_hidden` の組み合わせで代替可能。結論: 代替可能・実装不要（詳細は `docs/design/component-coverage-map.md` §5 Part D / §9 参照）。Slot: asChild/Slot 相当の要素種別差し替え・props マージ機構が現時点の `fandhe-frontend-headless-ui` に存在しない（再導入の提案はここでは書かない、`.claude/rules/coding-rust.md`）。Inset: layout 系ユーティリティに近いが #716/#724 の layout プリミティブ 5 件には含まれない。Radio: 既存 `radio_group` はグループ前提、単独 Radio ボタンの anatomy 差分が未検証。Reset: **イシュー #1066 で検証完了（2026-07-26）**。`stylesheet`/`theme` mod のリセット範囲は実測でゼロ（要素セレクタへのリセット出力なし）で重複はないが、Radix `Reset` は Slot 依存の asChild 型のため実装可否は Slot 行の保留に従属し保留を維持（詳細は `docs/design/component-coverage-map.md` §5 Part D / §9 参照） | Form: （保留解除済み。§3.25 規則 1 の再評価トリガーを参照）。Direction Provider: provider 機構全般の非採用可否が §3.23/§3.24 へ確定記録された場合、または `dir` 属性の明示的引数渡しで代替可能と判断された場合。Accessible Icon: 充足済み（検証完了 2026-07-26、イシュー #1066）。結論: 代替可能・実装不要。意図的非採用への区分移行はユーザー判断待ち。Slot: 要素差し替え機構の要否が別途の設計検討で明示的に再評価された場合。Inset: layout 系ユーティリティ全般の参照方針が別途確定した場合。Radio: 単独 Radio の要否・anatomy 差分の検証完了時。Reset: 充足済み（検証完了 2026-07-26、イシュー #1066）。結論: 保留維持・実装 issue 起票なし。Slot 行の再評価時に同時再判定する |
| Themes 追加候補の再突合で保留（イシュー #1669/#1676） | code-block（chakra `typography/code-block.md`）/ prose（chakra `typography/prose.md`）/ toc（ark-ui、pin 以降追加・対応 md なし、`docs/design/component-coverage-map.md` §5 Part E）/ checkbox-cards・radio-cards（Radix Themes、group 版。`docs/design/component-coverage-map.md` `forms/checkbox-card.md`・`forms/radio-card.md` 行） | code-block: Shiki/Highlight.js アダプタ前提のシンタックスハイライト部品で、ハイライト処理はアプリロジック寄り（§3.25 規則 1 に触れる可能性）。docs サイト側には外部依存ゼロの `crates/docs-site/src/highlight.rs` が既にある。prose: 生 HTML 断片へ型階層をカスケード適用する部品であり、REQ-1（既定エスケープ、迂回は `raw_html()` 明示オプトインのみ）との関係整理が先行課題。役割分担（要素単位オプトインの `heading`/`text`/… vs `site.css` の `.docs-content` カスケード）は #771 の `crates/pre-styled-ui/src/text.rs` rustdoc のとおり不変。toc: Ark UI が参照一覧 pin（`ab53c6b`）以降に追加した部品（スクロール位置に応じて可視見出しを追跡・強調）。docs サイト側には `layout.rs::toc_inline()` の静的目次と `script.rs` の IntersectionObserver スクロールスパイが既に存在し、スクロール追跡は実行時関心（§3.25 規則 2 の系）のため headless 層には置かない。checkbox-cards・radio-cards: 本行の `checkbox_card`/`radio_card` は単体版、Radix Themes 版は複数選択の**カード群** | code-block: (a) `highlight.rs` を `pre-styled-ui` へ一般化する要否が設計検討で明示的に評価された場合、または (b) ハイライト処理を含まない構造のみ（Root/Header/Content/Code/CopyTrigger の anatomy + 既存 `code`/`clipboard` の合成）で成立する要件が特定された場合。prose: 信頼済みノード木（`raw_html()` を経由しない Markdown → ノード木変換等）に対する型階層カスケードとして REQ-1 を弱めずに設計できることが示された場合、かつ `site.css` との重複適用・詳細度衝突の扱いが設計文書化された場合。toc: (a) docs サイト外の利用要望が出た場合、または (b) 静的 anatomy（Root/Nav/List/Item/Link/Indicator + `aria-current`/`data-active`）のみで成立する設計が示され、追跡配線を `wasm-full` 側へ隔離できると確認された場合。checkbox-cards・radio-cards: 既存 `checkbox_group`/`radio_group`（両層）+ `checkbox_card`/`radio_card` の合成で表現できない要件が特定されるか、利用要望が出た時点で再評価 |
