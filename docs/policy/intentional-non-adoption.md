...
| 装飾系（CSS 主体で実装可能性がある、または既存基盤で実装見込みがあるもの） | scroll-area（ark・chakra 双方）/ floating-panel（ark・chakra 双方）/ tour | scroll-area は CSS（`overflow`）主体で実装できる可能性がある。floating-panel は positioning（§3.20）・overlay（`crates/wasm-full/src/overlay.rs`）基盤が実装済みで将来実装しやすい。tour は状態機械（ステップ管理・ハイライト対象の同期）が大きく需要待ち。splitter（ark・chakra 双方）はイシュー #826 で保留解除・実装済み（headless `crates/headless-ui/src/splitter.rs` + styled `crates/pre-styled-ui/src/splitter.rs`、`docs/design/component-coverage-map.md` 参照） | 利用要望 issue の起票。floating-panel は既存 positioning/overlay 基盤で決定的に実装できる範囲を確認したうえで着手する |
