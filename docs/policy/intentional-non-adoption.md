...
| JS ランタイム固有 utilities のうち静的実装可能なもの | json-tree-view（download-trigger（ark・chakra 双方）は利用要望 issue #828 の起票により保留解除、`headless-ui`/`pre-styled-ui` の `download_trigger` mod として実装済み。詳細は `docs/design/component-coverage-map.md` 該当行参照） | json-tree-view は実装済み `tree_view` mod の派生として実装できる見込みだが需要待ち | 利用要望 issue の起票（実装時は `tree_view`（json-tree-view）・既存静的部品群（実装済みの download-trigger 等）の実装パターンを踏襲する） |
...
| 装飾系（CSS 主体で実装可能性がある、または既存基盤で実装見込みがあるもの） | floating-panel（ark・chakra 双方）/ tour | floating-panel は positioning（§3.20）・overlay（`crates/wasm-full/src/overlay.rs`）基盤が実装済みで将来実装しやすい。tour は状態機械（ステップ管理・ハイライト対象の同期）が大きく需要待ち。scroll-area（ark・chakra 双方）はイシュー #825 で保留解除・実装済み（`docs/design/component-coverage-map.md` 参照。JS によるスクロール位置追従・thumb drag は同イシューのスコープ外のまま）。splitter（ark・chakra 双方）はイシュー #826 で保留解除・実装済み（headless `crates/headless-ui/src/splitter.rs` + styled `crates/pre-styled-ui/src/splitter.rs`、`docs/design/component-coverage-map.md` 参照） | 利用要望 issue の起票。floating-panel は既存 positioning/overlay 基盤で決定的に実装できる範囲を確認したうえで着手する |
