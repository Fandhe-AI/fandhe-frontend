...
| JS ランタイム固有 utilities のうち静的実装可能なもの | download-trigger（ark・chakra 双方） | `a[download]` 属性による静的な部品として代替可能だが需要待ち（**json-tree-view は保留解除済み**: イシュー #829 で `tree_view`（#753）の派生として実装完了。headless `crates/headless-ui/src/json_tree_view.rs` + styled `crates/pre-styled-ui/src/json_tree_view.rs`、`docs/design/component-coverage-map.md` 該当行を実装済みへ更新済み） | 利用要望 issue の起票（実装時は既存静的部品群（download-trigger）の実装パターンを踏襲する） |
