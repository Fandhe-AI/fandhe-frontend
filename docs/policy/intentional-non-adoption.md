...
- **適用例（shadcn/ui 固有部品、イシュー #2006・親トラッキング #2001）**:
  ...
  - **規則 1 の適用例**:
    - **Data Table**（`data-table`、実装 issue #2124）: shadcn の Data Table
      ...
      ロジックは持たない」切り分けである。
    - **会話系 5 部品**（Message #2104 / Bubble #2107 / Attachment #2110 /
      Marker #2113 / Questionnaire #2116）: いずれも AI チャット UI の
      **表示**部品であり、ストリーミング表示・送信・履歴取得・必須回答
      判定等のアプリケーションロジックを内包しない（表示状態のみ
      `data-*` で受ける）。例えば Questionnaire の「次へ」ボタンの
      `data-disabled` は、必須回答かどうかの**判定自体はアプリ側が行い、
      結果だけを部品へ渡す**。Attachment のファイルサイズ表示も整形済み
      文字列を受け取る（§3.23 の数値整形非採用の系）。
  - **規則 2 の適用例**:
