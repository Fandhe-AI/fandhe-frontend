...
| `animation`（fandhe-animation） | `unsafe` を全面禁止 | `#![forbid(unsafe_code)]` を `crates/animation/src/lib.rs` に設定済み（イシュー #2371）。外部依存ゼロのため FFI 境界を持たず、`crates/core/tests/unsafe_boundary.rs` の safe 域（`UNSAFE_ALLOWED_MEMBERS`/`DENY_UNSAFE_FFI_MEMBERS` いずれにも非該当）として `forbid-unsafe` ジョブが自動検証する（イシュー #2372） |
| `wireframe-ui`（fandhe-frontend-wireframe-ui） | `unsafe` を全面禁止 | `#![forbid(unsafe_code)]` を `crates/wireframe-ui/src/lib.rs` に設定済み（イシュー #2603）。依存は `fandhe-frontend-core` への path 依存のみで外部依存ゼロのため FFI 境界を持たず、`crates/core/tests/unsafe_boundary.rs` の safe 域（`UNSAFE_ALLOWED_MEMBERS`/`DENY_UNSAFE_FFI_MEMBERS` いずれにも非該当）として `forbid-unsafe` ジョブが自動検証する（イシュー #2604） |
| `frontend-animation`（fandhe-frontend-animation） | フレームワーク自作コードは safe Rust（`crates/frontend-animation/src/` に自作 `unsafe` ブロック 0 件）。`#![deny(unsafe_code)]` を `crates/frontend-animation/src/lib.rs` に設定済み（`#[wasm_bindgen]` 展開コードが内部で `unsafe` を含むため `forbid` は不採用、`wasm-client`/`wasm-full` と同方針、イシュー #2417）。`unsafe` は `wasm-bindgen`/`web-sys` の FFI 依存クレート内部・自動生成グルーコードに限定して許容。**CI 強制済み**: `crates/core/tests/unsafe_boundary.rs` の `DENY_UNSAFE_FFI_MEMBERS` に登録され、`.github/workflows/ci.yml` の `forbid-unsafe` ジョブが (a) `#![deny(unsafe_code)]` 属性の実在、(b) `crates/frontend-animation/src/` 配下の自作 `unsafe` トークン 0 件、(c) `allow(unsafe_code)` による deny 上書きが 0 件、の 3 点を機械検証する | rAF Driver（`raf_driver::RafDriver`/`AnimationLoop`）・DOM Target（`dom_target::DomTarget`）を実装済み（イシュー #2403/#2517）。`AnimationLoop` の rAF コールバック自己参照は `wasm-client/src/timer.rs` と同じ `Rc<RefCell<..>>` 方式で解決し `unsafe` を要しない。`animate`（`element.animate()` WAAPI 薄いラッパ、イシュー #2398）も安定版 wasm-bindgen のみで完結する duck-typing extern バインディング（`wasm-full/src/nav.rs`「判断10」と同方針）で実装済み。confetti（`canvas_target::CanvasTarget`・`confetti::fire`、canvas 2D 描画・`AnimationLoop` 駆動によるパーティクル発火、イシュー #2533）も追加済み。`fire()` が返す `AnimationLoop` ハンドルの所有権は呼び出し側（`wasm-full` の click ハンドラ）が `Rc<RefCell<HashMap<..>>>` で保持・差し替える設計であり、`step` クロージャ自身が自己の保持スロットを書き換えて drop する構造（use-after-free の温床）は採らない（`confetti.rs` モジュール doc「`AnimationLoop` の所有権契約」節参照）。pointer capture ベースの汎用ドラッグ演算（`drag::DragController`、軸制約・範囲クランプ・離脱速度推定 + spring 復帰）・`reduced_motion::prefers_reduced_motion` も安定版 wasm-bindgen/web-sys のみで完結し自作 `unsafe` ブロック 0 件（イシュー #2535）。`grep -rnE '\bunsafe\s*(fn|impl|trait|\{)' crates/frontend-animation/src/` の結果は 0 件（confetti/drag 追加後も継続確認済み）。layout FLIP（`flip.rs`、イシュー #2518）も座標計測・rAF 再生・DOM 適用のすべてを安定版 wasm-bindgen/web-sys のみで完結させ `unsafe` を追加していない（grep 結果は追加後も 0 件のまま）。text_animation（`text_animation::typewriter_frame`/`scramble_frame`/`play_typewriter`/`play_scramble`、イシュー #2532）も `AnimationLoop`/`RafDriver`（#2403/#2517）をそのまま消費し `unsafe` を追加していない（`grep -rnE '\bunsafe\s*(fn|impl|trait|\{)' crates/frontend-animation/src/` は本モジュール追加後も 0 件のまま確認済み）。presence（`presence::insert_exit_ghost`/`remove_when_settled`、keyed list 削除行の退場ゴースト配置・実測 CSS アニメーション時間経過後のタイマー除去、イシュー #2544）も `Closure::once_into_js`（`confetti.rs`/`layout_flip.rs` と同型の 1 回呼び出し後自己解放パターン）で `setTimeout` コールバックを組み立てるのみで `unsafe` 追加なし（`grep -rnE '\bunsafe\s*(fn|impl|trait|\{)' crates/frontend-animation/src/` は本モジュール追加後も 0 件のまま確認済み）。共有レイアウト遷移（`shared_layout.rs`、イシュー #2536）も `flip.rs` の計測・再生ロジックを再利用するのみで新規の DOM/FFI 呼び出しを追加せず、`grep -rnE '\bunsafe\s*(fn|impl|trait|\{)' crates/frontend-animation/src/` の結果は追加後も 0 件のまま。SVG path 等の残りの実装は Phase 4 の後続 issue で追加する |
...
`core` / `interactive` / `animation` / `wireframe-ui` に `#![forbid(unsafe_code)]` が設定されているため、これらのクレート内での `unsafe` 使用は
...
`frontend-animation`（fandhe-frontend-animation）はイシュー #2417 で crate 雛形を追加し、
イシュー #2403/#2517 で rAF Driver・DOM Target を、イシュー #2398 で `animate`
モジュール（`element.animate()` WAAPI 薄いラッパ。安定版 wasm-bindgen のみで完結する
duck-typing extern バインディング、`wasm-full/src/nav.rs`「判断10」と同方針）を
実装済みですが、
`grep -rnE '\bunsafe\s*(fn|impl|trait|\{)' crates/frontend-animation/src/` の結果も 0 件です。
...
1. **境界の限定**: `unsafe` は WASM バインディング層・FFI 境界に該当するクレート（`wasm-client` / `wasm-full` /
   `frontend-animation` 等）に限定する。`core` / `interactive` / `animation` / `wireframe-ui` への追加は `#![forbid(unsafe_code)]` によりビルド自体が失敗するため、
   構造的に不可能である。
