...
`frontend-animation`（fandhe-frontend-animation）はイシュー #2417 で crate 雛形を追加、
イシュー #2398 で `animate` モジュール（`element.animate()` WAAPI 薄いラッパ。安定版
wasm-bindgen のみで完結する duck-typing extern バインディング、`wasm-full/src/nav.rs`
「判断10」と同方針）を実装済みですが、
`grep -rnE '\bunsafe\s*(fn|impl|trait|\{)' crates/frontend-animation/src/` の結果は引き続き 0 件です。
