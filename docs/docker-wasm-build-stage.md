...
> 検証はいずれも本書のスコープ外です（`docs/wasm-build-integration.md` §7・
> `docs/dist-server-design.md` の先行例と同型の設計契約ドキュメント）。
>
...
>
> すなわち `docs/wasm-build-integration.md` の前置き（#109〜#111 すべて
...
- **`docs/wasm-build-integration.md` との境界**: 同書 §7 は「TASK-10.3 は
...
WASM ビルド経路の 2 系統が並存し、`docs/wasm-build-integration.md` §2 が
...
| 受け入れ条件 | 対応状況 |
|-------------|---------|
| 成果物が作成される | 本書 `docs/docker-wasm-build-stage.md` が本タスクの成果物 |
...
- **`docs/wasm-build-integration.md` 前置きの状態更新（#109〜#111 の
