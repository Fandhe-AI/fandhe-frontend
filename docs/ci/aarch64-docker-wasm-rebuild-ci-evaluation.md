...
> **さらなる前提変更（2026-08-10、ユーザー指示）**: CI の `runs-on` は
> `ubuntu-latest` 単一へ限定され、`ubuntu-24.04-arm` 等の arm 変種を含む
> 他イメージの使用は禁止された（`.claude/rules/ci.md` Runner 方針、
> `crates/xtask/tests/workflow_runner_policy.rs` が機械強制）。したがって
> 本節が示した「arm64 ホステッドランナーによる再評価」経路は現方針では
> 選択肢から外れており、aarch64 CI 常設を再検討する場合は Runner 方針
> そのものの変更から始める必要がある。
