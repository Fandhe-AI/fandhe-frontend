...
- `gate-self-apply` ジョブ（`fw gate` は `cargo check` / `clippy` /
  ...
  衝突が起きない。#2306 は本書執筆時点でまだ CI 実測を経ていない
  （実装 PR は本書と同時にオープンする）ため、分割後の所要時間は本書では
  未確定として扱い、確定次第この節を実測値（run ID 付き）で更新する。
  いずれにせよ本節の二重コンパイル分析（`browser-test` の回帰原因）は
  `gate-self-apply` 系ジョブの分割可否に依存しないため、本イシューの
  スコープ外事項は無い。
