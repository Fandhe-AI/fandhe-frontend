...
- **TASK-5.1c（#32）**: 利用者向けチュートリアル `docs/component-authoring.md` の作成
...
（本書は設計判断の記録、`docs/component-authoring.md` は使い方の解説）。

本書は `docs/dependency-graph-policy.md`（TASK-3.3a）と同じく `docs/` 直下のフラット
...
| # | 判断 | 根拠 |
|---|------|------|
| 1 | マクロ DSL（`view!`/`rsx!`/`html!` 相当）を提供しない | PoC-1 が特定した差別化空白 D。PoC-2 の依存グラフ実測（マクロ DSL 構成 202 件/深さ14 → 純 Rust 構成 52 件/深さ5、`docs/dependency-graph-policy.md` 第 2 節）。REQ-5 概要・詳細 |
...
7. `core/Cargo.toml` の `[dependencies]` は常に空を維持する（外部依存ゼロ）。
   ...
   60 件・深さ 6 の制約（`docs/dependency-graph-policy.md`）も維持する。
