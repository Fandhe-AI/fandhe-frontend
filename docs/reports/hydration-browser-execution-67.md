...
> **注記（#433 改名）**: 本レポートは旧名称時代の実測記録です。`wasm-client/` 等のクレート配置はルート直下から #442 で `crates/wasm-client/` 等へ移設されます（新旧対応は `docs/design/framework-naming.md` 参照）。以下の記録中のパスは当時のまま残しています。
