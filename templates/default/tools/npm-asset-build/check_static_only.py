...
def validate_exempt_entries(entries: Iterable[object]) -> dict[ExemptKey, str]:
    ...
    呼び出し元: load_allowlist（allowlist.toml 読み込み時の検証）と
    tools/npm-asset-build/apply_exempt.py（半自動追記時の再検証）の両方から
    呼ばれる、検証規則の単一の実装源（ドリフト防止のため意図的に切り出している）。
    """
...
def load_allowlist(path: str | None) -> dict[ExemptKey, str]:
    """allowlist.toml を読み込み、`validate_exempt_entries()` で検証した
    ExemptKey -> reason の免除表を返す。

    ファイル I/O・TOML パース（tomllib）の責務のみを担い、エントリ検証は
    `validate_exempt_entries()` に委譲する（検証規則の二重実装を避ける）。
    """
    if path is None:
        return {}
    ...
    return validate_exempt_entries(data.get("exempt", []))
