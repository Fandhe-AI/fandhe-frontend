...
クレート・領域名を使う: `core` / `interactive` / `app` / `server` / `dist-server`（fandhe-frontend-dist-server: 単一バイナリ配布サーバー） / `docs-site`（fandhe-frontend-docs-site: docs サイトジェネレータ） / `headless-ui`（fandhe-frontend-headless-ui: headless UI コンポーネント層、イシュー #520/#522） / `pre-styled-ui`（fandhe-frontend-pre-styled-ui: pre-styled UI コンポーネント層、イシュー #520/#546） / `wasm-client` / `wasm-full` / `xtask` / `spec` / `claude`（.claude 体系） / `global`（横断）
