...
- 本方針（`runs-on` は `ubuntu-latest` リテラル単一）は同じく `crates/xtask/tests/workflow_runner_policy.rs`（`workflows_run_only_on_ubuntu_latest`）が `.github/workflows/*.yml` の非コメント `runs-on` 行に対して fail-closed に機械強制する。値が空になる block sequence 形・`labels:` mapping 形も「1 行から runner を確定できない」ため FAIL とする。キーと `:` の間の空白（`runs-on : windows-latest`。YAML では合法）も正規化して検査する（正規化を怠ると空白 1 つで検知を回避できる fail-open になるため。PR #1301 の codex レビュー P0 指摘）。reusable workflow の呼び出しジョブ（job-level `uses:`）は `runs-on` を持たず本テストの射程外であり、下記 codex-review 例外はその射程外で成立している（本規則は OS・イメージ選択に関するもので、既承認の codex 例外を撤回しない）
