...
- **`docs-site.yml` の verify ステップ契約（イシュー #944/#951/#957/#1016/#1017/#1018/#1021/#1022/#2088/#2607）**: `site/**` の
  ...
  ブランドマーク `icon::brand`（代わりに使える既存アイコンがないため
  新規追加、`icon.rs` は計 24 種）へフォールバックする §11.4 からの意図的な
...
- **crates.io 公開用 release ワークフロー（イシュー #514/#513）**: `.github/workflows/release.yml` は `workflow_dispatch` 起点で単一クレートを crates.io へ公開する。既公開バージョン検証ステップは `command -v curl` で存在チェックしてから sparse index（`https://index.crates.io`）を取得するため、curl 未導入 runner では環境エラーとして明示停止する（自動インストールは行わない）。`cargo package`/`cargo publish`（dry-run 含む）は `https://index.crates.io`・`https://static.crates.io` への到達性を前提とし、到達不可の場合は環境エラーとして扱う（他の crates.io バージョン依存ワークフローと同様、テストの弱体化で対処しない）。`CARGO_REGISTRY_TOKEN` はリポジトリ Secrets からのみ供給し、`mode: publish` を選択したステップの `env:` にのみ限定注入する（ログへは出力しない）。誤操作対策として `mode` の既定値は `dry-run-only`（安全側）とし、実公開は明示的に `mode: publish` を選ぶ運用とする。**バンプ PR と同時の crates.io 公開（同時公開フロー、ユーザー決定 2026-08-10、イシュー #1306）**: codex-review 導入後、`templates/app/wasm/Cargo.lock` の再生成をバンプ先バージョン公開まで後続 PR へ先送りする従来運用は stale lock の P1 検知でマージ不能になるため、条件（dry-run 先行確認・`mode: publish` 実行前に「未公開バージョン起因で構造的に fail するもの」〔`version-bump-guard` は含まない。公開前は green 必須〕を除く CI 全 green と codex-review findings が既知の stale lock 指摘のみであることの確認・依存順公開・同一 PR 内での lock 再生成・公開完了確認後に限る `version-bump-guard` の `version-bump-exempt` 免除・公開後の force-push 等の禁止・`mode: publish` 明示選択の維持）を満たす限り、バンプ PR が open のまま当該 PR ブランチを ref とした先行公開を許容する。詳細・残存リスク（reject されても公開済みバージョンは crates.io に残る）は `docs/ci/version-bump-publish-order-gap.md` §10 を正とし、本節では二重管理しない
...
- **ruleset 更新の運用手順（ジョブ追加・改名・削除・新規 workflow 追加時）**: ① `.github/workflows/*.yml` を変更する → ② `cargo test -p xtask`（`workflow_required_checks_manifest.rs`）が FAIL するので `.github/required-status-checks.json` を追随更新する（新規 workflow ファイルは分類表〔`JOB_ENUMERATED_WORKFLOW_FILES` / `REUSABLE_WORKFLOW_FILES` / `EXCLUDED_WORKFLOW_FILES`〕への追加も必要）→ ③ **マージ前**に `gh api repos/Fandhe-AI/fandhe-frontend/rulesets/<id>` で GET した ruleset の `required_status_checks` ルールの `parameters.required_status_checks` を、更新後のマニフェストの配列で差し替えて `gh api --method PUT` する（他のルール・`bypass_actors` 等は変更しない）→ ④ `setup-repo-guards` の 3 軸スイープ（`strict_required_status_checks_policy: false` / `bypass_actors` 空 / 全 required エントリが `integration_id` で束縛済み）を確認する → ⑤ 当該 PR の `dep-version-check` ジョブ（`check-ruleset-sync`）が PASS することを確認する → ⑥ マージする。**注意**: PUT からマージまでの間、他の open PR は新 context をまだ報告しないため「Expected」のまま停止する（当該 PR をマージ後、他 PR は `main` を pull/merge して追随する）。PUT 後・マージ前に別 PR が先にマージされると、main push 側の `check-ruleset-sync` が一時的に FAIL する（当該 PR のマージで解消する）
