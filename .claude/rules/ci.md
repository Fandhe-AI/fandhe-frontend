...
- **`required-check-skip-mirror.yml`（paths フィルタ付きチェックの required 化、implement-issue-tree autoMerge G0 対応）**: `musl-smoke.yml`（ジョブ `x86_64 musl startup smoke (REQ-9)`）と `image-size.yml`（ジョブ `Docker image size (REQ-9)`）は `pull_request.paths` フィルタを持つため、対象パス（Dockerfile / Cargo.toml / Cargo.lock / crates/** / static/** / .dockerignore + 各ワークフロー自身）に変更がない PR ではワークフロー自体が起動せずチェックが報告されない。GitHub の ruleset （`main-protection`）で未報告のチェックを required 指定すると当該 PR が永久にマージ不能になるため、GitHub 公式の「paths-ignore を鏡像にした補完ワークフロー」パターンで `.github/workflows/required-check-skip-mirror.yml` を追加し、対象パスに変更がない PR でのみ起動して同名ジョブ（`x86_64 musl startup smoke (REQ-9)` / `Docker image size (REQ-9)`）の success を報告する。`paths-ignore` は本物 2 ワークフローの `pull_request.paths` の和集合と厳密に同期させ、片方でも `paths` を変更した場合は必ずこのファイルの `paths-ignore` へも反映する。ジョブ名は ruleset が名前で識別するため本物側と完全一致を維持し、本物側のジョブ名を変更する場合はこの鏡像ファイルと ruleset `main-protection`（required_status_checks）の双方を同時更新する（`.github/workflows/required-check-skip-mirror.yml` 冒頭コメント参照）。
