...
/// 表示すること（§2.2）。
#[wasm_bindgen_test]
fn click_navigation_scrolls_to_top_on_new_entry() {
    let window = web_sys::window().expect("window must exist");
    let document = window.document().expect("document must exist");
    ...
    assert!(
        root.query_selector("[data-testid=\"item-detail\"]")
            .unwrap()
            .is_some(),
        "テスト前提条件: 遷移先の DOM が描画されていること"
    );
    assert_eq!(
...
#[wasm_bindgen_test]
fn popstate_with_encoded_state_restores_saved_scroll_position() {
    let window = web_sys::window().expect("window must exist");
    let document = window.document().expect("document must exist");
    ...
    assert!(
        root.query_selector("[data-testid=\"item-list\"]")
            .unwrap()
            .is_some(),
        "テスト前提条件: popstate によりルートが再解決・再描画されていること"
    );
    assert!(
        (window.scroll_y().unwrap() - 500.0).abs() < 1.0,
...
#[wasm_bindgen_test]
fn popstate_with_null_or_invalid_state_falls_back_to_top() {
    let window = web_sys::window().expect("window must exist");
    let document = window.document().expect("document must exist");
