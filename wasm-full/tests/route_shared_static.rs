...
//!    `rws_app::routes::title` を直接呼ぶ構造（検証 1）そのものが保証する。
    ...
    for pattern in ROUTE_PATTERN_LITERALS {
        assert!(
            ...
            "{ssr_path:?} にルートパターンリテラル `.route({pattern}` が再定義されている。\
             rws_app::routes の単一定義（イシュー #407）に反する"
        );
    }
    for title in PAGE_TITLE_LITERALS {
        assert!(
            ...
            "{ssr_path:?} にページタイトルリテラル {title} が再定義されている。\
             rws_app::routes::title の単一定義（イシュー #407）に反する"
        );
    }
    assert!(
        stripped.contains("rws_app::routes"),
        "{ssr_path:?} が rws_app::routes を参照していない（共有ルート定義を経由していない）"
    );
}

/// 検証 1c: `wasm-full/src/nav.rs` も同様に、ルートパターン・タイトル
/// リテラルを再定義せず `rws_app::routes` を参照していることを固定する。
#[test]
fn wasm_full_nav_does_not_redefine_route_literals_and_references_shared_routes() {
    ...
    assert!(
        stripped.contains("rws_app::routes"),
        "{nav_path:?} が rws_app::routes を参照していない（共有ルート定義を経由していない）"
    );
}

/// 検証 2: server・wasm-full 双方のルーティングエンジンが同一クレート
/// （`rws_app::router::Router`、`app/src/routes.rs` 経由）に収束しており、
/// `rws-server` 独自の `Router` 実体（旧 `server/src/router.rs`）が
...
fn server_router_module_is_a_reexport_shim_not_a_duplicate_engine() {
    ...
    assert!(
        ...
        "{router_path:?} に `struct Router` の独自定義が存在する。\
         エンジンは rws_app::router へ一本化されている必要がある（イシュー #407 案 B-1）"
    );
    assert!(
        stripped.contains("pub use rws_app::router"),
        "{router_path:?} が rws_app::router を再エクスポートしていない"
    );
}
