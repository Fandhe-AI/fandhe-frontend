...
//! `set_inner_html` で行う。
//!
//! # `rws-wasm-client` への一本化（イシュー #375、Bugbot 指摘対応）
//!
//! `loader_error_view` / `resolve_list_node` / `resolve_detail_node` は
//! `rws-wasm-client`（`wasm-client/src/lib.rs`）の実装と完全に同一のロジック
//! （fail-closed・`Loader::Error` 値を一切受け取らない構造的保証）であり、
//! `rws-wasm-full` は既に `rws-wasm-client` に workspace path 依存している
//! （`wasm-full/Cargo.toml`）。将来のセキュリティ・挙動修正が本モジュールと
//! `wasm-client` 側の 2 箇所に分岐しないよう、本モジュールは実装を持たず
//! `rws-wasm-client` の公開関数をそのまま再エクスポートする一本化窓口とする。
pub use rws_wasm_client::{loader_error_view, resolve_detail_node, resolve_list_node};
