...
├── wireframe-ui/  # fandhe-frontend-wireframe-ui: blocks.pm 参照のワイヤーフレーム UI コンポーネント層（SSR 専用・非インタラクティブ、headless-ui/pre-styled-ui 非依存、docs/design/wireframe-ui-architecture.md 参照。雛形は #2603 で追加、#2604 で CI・依存グラフ運用へ組み込み済み、#2605 で共通基盤 API（`Size`・共通型・モノクロトークン・`wireframe_css()`）追加済み）
