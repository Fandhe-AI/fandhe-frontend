...
├── wasm-full/     # fandhe-frontend-wasm-full: CSR/ハイドレーション フルセット（配線群別 feature 18 件 + 別枠 feature 5 件〔position/stagger/view-transitions/view-transition-name/animate〕+ scope feature 16 件、既定 on、#2326/#2327/#2396/#2397/#2398/#2400/#2515/#2520。`default-features = false` 利用者の移行手順は `docs/guides/wasm-full-features.md`）
...
`frontend-animation` の辺は宣言していない（`crates/wasm-full/Cargo.toml` の
依存は `optional = true` の既定 off であり、`fw structure` が突き合わせる
`cargo metadata` の既定解決〔`resolve.nodes[].deps`〕には現れないため。
宣言すると非対称違反になる）。
