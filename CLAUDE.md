...
│   ├── wireframes.md            # Wireframes セクショントップページ原稿（`fandhe-frontend-wireframe-ui` の部品ページ索引、イシュー #2607。掲載予定 45 部品〔Phase 1〜8〕のリンクなし箇条書き。イシュー #2617 で Annotation が「掲載済み」節へリンク付きで追加された最初の部品になった。イシュー #2611 で Grid（Phase 1「レイアウト骨格」）・イシュー #2612 で Divider・イシュー #2610 で Stack が続いた）
│   ├── wireframes/              # 部品ページ原稿（`/wireframes/<kebab>/` 1 ページ = 部品 1 件、イシュー #2607。イシュー #2617 で最初の原稿（`annotation.md`）が加わり `.gitkeep` を削除した。イシュー #2611 で `grid.md`・イシュー #2612 で `divider.md`・イシュー #2610 で `stack.md` が続いた。レジストリは `crates/docs-site/src/wireframes/mod.rs`、登録の正は `site/nav.toml`、三方突合は `crates/docs-site/tests/wireframes_nav.rs`）
