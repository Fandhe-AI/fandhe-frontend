...
│   ├── wireframes.md            # Wireframes セクショントップページ原稿（`fandhe-frontend-wireframe-ui` の部品ページ索引、イシュー #2607。掲載予定 49 部品〔Phase 1〜8〕のリンクなし箇条書き。イシュー #2617 で Annotation が「掲載済み」節へリンク付きで追加された最初の部品になった）
│   ├── wireframes/              # 部品ページ原稿（`/wireframes/<kebab>/` 1 ページ = 部品 1 件、イシュー #2607。イシュー #2617 で最初の原稿（`annotation.md`）が加わり `.gitkeep` を削除した。イシュー #2612 で `divider.md` が加わり原稿 2 件目になった。レジストリは `crates/docs-site/src/wireframes/mod.rs`、登録の正は `site/nav.toml`、三方突合は `crates/docs-site/tests/wireframes_nav.rs`）
...
├── wireframe-ui/  # fandhe-frontend-wireframe-ui: blocks.pm 参照のワイヤーフレーム UI コンポーネント層（SSR 専用・非インタラクティブ、headless-ui/pre-styled-ui 非依存、docs/design/wireframe-ui-architecture.md 参照。雛形は #2603 で追加、#2604 で CI・依存グラフ運用へ組み込み済み、#2605 で共通基盤 API（`Size`・共通型・モノクロトークン・`wireframe_css()`）追加済み、#2606 で SVG ラインアートアイコン基盤（`icon.rs`、21 種、`Node` スロット規約は `docs/design/wireframe-ui-architecture.md` §11）追加済み、#2617 で最初の個別部品（`annotation.rs`、Phase 2「テキスト・注釈」）追加済み、#2612 で `divider.rs`（Phase 1「レイアウト骨格」、`Orientation` の最初の消費者）追加済み）
