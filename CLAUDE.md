...
├── wireframe-ui/  # fandhe-frontend-wireframe-ui: blocks.pm 参照のワイヤーフレーム UI コンポーネント層（SSR 専用・非インタラクティブ、headless-ui/pre-styled-ui 非依存、docs/design/wireframe-ui-architecture.md 参照。雛形は #2603 で追加、#2604 で CI・依存グラフ運用へ組み込み済み、#2605 で共通基盤 API（`Size`・共通型・モノクロトークン・`wireframe_css()`）追加済み、#2606 で SVG ラインアートアイコン基盤（`icon.rs`、21 種、`Node` スロット規約は `docs/design/wireframe-ui-architecture.md` §11）追加済み、#2617 で最初の個別部品（`annotation.rs`、Phase 2「テキスト・注釈」）追加済み、#2611 で Phase 1「レイアウト骨格」の `grid.rs`（列数固定グリッド配置、blocks.pm 対応なしの独自追加部品）追加済み、#2612 で同 Phase の `divider.rs`（`Orientation` の最初の消費者）追加済み、#2615 で Phase 2 の `paragraph.rs`（複数行本文プレースホルダー）追加済み）
