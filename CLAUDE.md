...
| `ci.md` | CI 規約（GitHub ホステッドランナー既定・codex-review の codex ジョブのみ self-hosted 例外・共有 CARGO_TARGET_DIR 対策・ツール前提の明示） |
