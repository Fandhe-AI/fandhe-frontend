//! `fandhe-frontend-wasm-thin`: REQ-11（`docs/spec/04-requirements.md`）が定める「薄い JS
//! グルー方式」のオプトイン実装クレート（TASK-11.3、親イシュー #78）。
//!
//! WASM 完全方式（`fandhe-frontend-wasm-full`、既定実装）はイベント配線・DOM 更新まで
...
//!   純粋 Rust。`fandhe_frontend_interactive::Component`/`Hydrate` の任意の実装を束縛して
...
//!   `fandhe_frontend_interactive::AppState` に束縛した具象エクスポートを同梱する。
...
//!    [`fandhe_frontend_interactive::Component::view`] → `fandhe_frontend_core::render()` の既定
...
//!    `fandhe_frontend_interactive::dispatch` の仕様どおり no-op とし、ハイドレーション
//!    失敗は panic せず `Err`（[`fandhe_frontend_interactive::HydrateError`]）を返す。
...
///
/// [`fandhe_frontend_interactive::Component`] を実装した任意のコンポーネント `C` を
...
impl<C: Component> ThinRuntime<C> {
    ...
    ///
    /// `fandhe_frontend_interactive::Component::view` が返す `fandhe_frontend_core::Node` を
    /// `fandhe_frontend_core::render()` に通す（不変条件 1）。JS グルーはこの戻り値を
    /// そのまま `innerHTML` へ設定する契約（不変条件 2）。
    pub fn html(&self) -> String {
        fandhe_frontend_core::render(&self.component.view())
    }

    /// WASM 境界の `(name, payload)` 文字列でアクションを適用し、
    /// 再描画後の HTML を返す。
    ///
    /// 未知のアクション名は `fandhe_frontend_interactive::dispatch` の仕様に従い
    /// no-op（状態を変更しない）。この場合も現在の状態を再描画した HTML を
    /// 返す（呼び出し側から見て「アクション適用後の最新表示」を返す契約を
    /// 一貫させるため）。
    pub fn apply(&mut self, name: &str, payload: &str) -> String {
        fandhe_frontend_interactive::dispatch(&mut self.component, name, payload);
        self.html()
    }
...
impl<C: Component + Hydrate> ThinRuntime<C> {
    ...
    ///
    /// 属性はクライアント側で改ざんされうる入力のため、`fandhe_frontend_interactive::
...
/// を `fandhe_frontend_interactive::AppState` に束縛した具象エクスポートをここに用意する。
...
pub mod demo {
    ...
    /// 文字列を渡す想定（`fandhe_frontend_interactive::dispatch` の契約に一致）。
    #[wasm_bindgen]
    ...
    /// 表現）。`fandhe_frontend_interactive::AppState::from_hydration_attrs` は
