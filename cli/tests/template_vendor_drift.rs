...
const VENDORED_CRATE_SOURCE_MANIFESTS: &[(&str, &str)] = &[
    ("fandhe-frontend-core", "core/Cargo.toml"),
    ("fandhe-frontend-app", "app/Cargo.toml"),
    // イシュー #411（PR #428）で app テンプレートへ追加された vendor クレート。
    // いずれも正本は publish = false（crates.io 未公開）であり、canary の
    // 監視対象に含める。
    ("fandhe-frontend-interactive", "interactive/Cargo.toml"),
    ("fandhe-frontend-wasm-client", "wasm-client/Cargo.toml"),
];
