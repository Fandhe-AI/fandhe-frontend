...
- **状態は共通型で表現**: `Active`/`Disabled` の `data-*`（同文書 §5「表示状態
  ...
  と `Disabled` を併用する部品は、本モジュールより先に main へマージされた
  `select`（イシュー #2624）が最初の実消費者、`switch`（イシュー #2627）が
  2 例目、`checkbox`（イシュー #2625）が 3 例目、`radio`（イシュー #2626）が
  4 例目、`textarea`（イシュー #2623）が 5 例目、`slider`（イシュー #2628）
  が 6 例目であり、本部品は Select・Switch・Checkbox・Radio・Textarea・
  Slider に続く 7 例目の実消費者です。
