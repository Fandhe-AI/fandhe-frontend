...
（1 枚または複数枚の組）を置く 2 列 feature セクションです。

4 つの静的インスタンスを縦に並べています。インスタンス A はアイコン付き
カード一覧（4 件・2 列）+ 縦長画像 1 枚、インスタンス B はアイコン付きの
行一覧（3 件）+ 正方形画像 1 枚（この 1 件だけ DOM 順が「画像 → 一覧」）、
インスタンス C は数値を主役にしたカード（2 件）+ ボタン行 + 正方形画像
1 枚、インスタンス D は定義リスト（4 件）+ 画像 4 枚の 2×2 グリッドです。
