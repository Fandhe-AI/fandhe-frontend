...
- 同意チェックボックスは SSR 初期状態（未チェック）を描画するのみです。docs サイトは JS ハイドレーションを行わないため、チェックボックスを操作してもチェックマークの表示は切り替わりません（表示の同期は利用者側の状態管理・ハイドレーションで行います）。プライバシーポリシーへのリンクは遷移先を持たない死にリンクになるため出力せず、同意文言はテキストのみにしています。
