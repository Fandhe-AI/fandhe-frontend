...
状態機械 `Popover` が管理します。content は自由な `children` を受け取るため、
`fandhe-frontend-pre-styled-ui` の `field`/`input` mod と組み合わせてフォーム
内包パターンも表現できます（下記 Examples 節「Dimensions form」参照）。
