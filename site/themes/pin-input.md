...
`root` の既定 CSS は `label` を `control` の上に積む `flex-direction: column` のため、`control()`/`separator()`/`control()` を `root` の直下にそのまま並べると縦積みになります。3-3 等の横並びパターンにするには、これらを横並び用のラッパー要素（例: `<div style="display: flex; align-items: center;">`）でまとめてから `root` の子に渡してください。専用のグループ用パートは提供していません（個別ボックスデザインではラッパーの `display: flex` だけで同じ視覚効果を再現できるため）。
