...
`collapse-content` は折りたたみ詳細を表現しますが、closed 時に headless 層
が出力する `hidden` 属性は `display: none` を伴うため、opacity フェード
遷移が実際に見えるのはクライアントランタイムが `hidden` を外す前後で
`data-state` を切り替える場合のみです。SSR 単独では即時表示・即時非表示に
なります。
