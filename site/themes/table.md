...
表形式データを表示する部品です。TableVariant（Line/Outline）で外枠・区切り線を、size（Xs〜Xl）でセルの padding/font-size を、striped（bool）で縞模様表示を、sticky_header（bool）で column-header（th）の position: sticky 表示を切り替えます。scroll_area（chakra Table.ScrollArea 相当）で root をスクロール枠に包み、sticky_header と組み合わせて見出し行を固定できます。column_header は scope="col" を関数側で固定し呼び出し側の偽装を除去します。
