//! `fandhe-frontend-wasm-client` 側の三モード（SSR/SSG/CSR）整合テスト（イシュー #375）。
//!
...
//! （`fandhe_frontend_core::render(&fandhe_frontend_app::list_page(...))`）が SSR 出力へ埋め込まれる
//! ことを固定するが、実際の `fandhe-frontend-wasm-client` の公開関数
//! （[`fandhe_frontend_wasm_client::render_list_page_html`] 等）は使っていない
//! （同ファイルのコメント「CSR を模した直接呼び出し（fandhe-frontend-wasm-client 相当）」
//! 参照）。
//!
//! 本ファイルは、イシュー #375 で `fandhe-frontend-wasm-client` を `fandhe_frontend_app::Loader`
//! 経由へ移行したことに伴い、**実際の `fandhe-frontend-wasm-client` 公開関数**を SSR
//! （[`fandhe_frontend_server::ssr::respond`]）・SSG（[`fandhe_frontend_server::ssg::generate`]）の
//! 出力と native で直接突き合わせる。`fandhe-frontend-server` を dev-dependency として
//! 追加している（`wasm-client/Cargo.toml` 参照。ワークスペース内 path 依存
//! かつ `fandhe-frontend-server` 自体は外部依存ゼロのため REQ-3 の依存グラフ計測
//! （Normal のみ対象）に影響しない）。
//!
...
use fandhe_frontend_wasm_client::{render_detail_page_html, render_list_page_html};
use std::fs;
...
/// REQ-6 中核: `fandhe-frontend-wasm-client` の CSR 用一覧 HTML（[`render_list_page_html`]）
...
fn wasm_client_list_html_is_embedded_in_ssr_output() {
    ...
    assert!(
        ...
        "SSR body should embed the exact fandhe-frontend-wasm-client render_list_page_html() output"
    );
}

/// 詳細ページ版。`fandhe_frontend_app::demo_items()` の全 id について、CSR
/// （[`render_detail_page_html`]）の出力が SSR 応答本文へそのまま埋め込まれる
/// ことを固定する。
#[test]
fn wasm_client_detail_html_is_embedded_in_ssr_output_for_every_item() {
    for item in fandhe_frontend_app::demo_items() {
        ...
        assert!(
            ssr_response.body.contains(&csr_html),
            "SSR detail body for id {:?} should embed the exact fandhe-frontend-wasm-client \
...
/// REQ-6: SSG（`generate`）が書き出す静的ファイルにも、`fandhe-frontend-wasm-client` の
...
/// SSR・SSG・CSR（`fandhe-frontend-wasm-client`）のいずれの経路でも既定エスケープされ、
...
fn xss_payload_is_escaped_across_ssr_ssg_and_wasm_client_csr_paths() {
    ...
    // CSR（fandhe-frontend-wasm-client 実関数）。`<img src=x onerror=alert(1)>` は
