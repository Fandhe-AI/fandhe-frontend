...
/// `RWS_BIND_ADDR` に環境変数経由で明示指定したポート番号が、実際の
...
fn bind_addr_env_switches_port() {
    ...
    for attempt in 1..=MAX_PORT_RETRIES {
        ...
        assert_eq!(
            ...
            200,
            "dist-server must accept connections on the RWS_BIND_ADDR-specified port"
        );
...
/// `RWS_BIND_ADDR` に指定したホスト部（`127.0.0.1` 以外のループバック
...
fn bind_addr_env_switches_address() {
    ...
    assert_eq!(
        host, "127.0.0.2",
        "listening address host must match the RWS_BIND_ADDR-specified host: {reported_addr}"
    );
    ...
    assert_eq!(
        status_code(&response),
        200,
        "dist-server must accept connections on the RWS_BIND_ADDR-specified host"
    );
