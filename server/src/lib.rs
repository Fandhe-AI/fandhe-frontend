...
//!   （TASK-7.2 系）。
//! - [`ssr`]: rws-app のページ関数を分岐なく呼び、パスをステータス・
//!   Content-Type・既定エスケープ済み HTML 文字列へ変換する SSR コア
//!   （TASK-6.1c）。`server/src/main.rs`（CLI エントリ）・`rws-dist-server`
//!   （HTTP 配信）の両方から共有される。
...
//! `rws-app` の既定エスケープ済み出力をそのまま扱うのみ）。`router` の解決で
//! 得た [`router::Params`] を画面へ出力する際は、呼び出し元が必ず
//! `rws_core::text` / `rws_core::el` の attrs 経由で既定エスケープを通すこと
